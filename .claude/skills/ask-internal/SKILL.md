---
name: ask-internal
description: Answer questions about Mahoraga internal stuff (setup, features, architecture, design decisions) by reading the private internal-docs submodule and the codebase. Use for "how do I X", "where is Y", "what is the deal with Z", or any question that mixes ops/setup knowledge with code knowledge. Can execute steps with per-command confirmation.
...
- NEVER edit Mahoraga code or docs in response to an ask-internal question. The skill answers; it does not write files.
...
> The setup runbook (`setup/dogfood-profile.md:23`) says to delete `~/.cache/mahoraga/dogfood`, but the actual code path in `packages/cli/src/cleanup.ts:47` removes `~/.local/share/mahoraga/dogfood`. The doc looks stale. Recommend updating it.
...
- Modify Mahoraga code or docs from this skill.
