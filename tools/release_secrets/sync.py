...
DEFAULT_REPO = "connectserverlab-del/mahoraga"
REPO_ROOT = Path(__file__).resolve().parents[2]
DEFAULT_ENV_FILE = REPO_ROOT / ".env.production"

RELEASE_WORKFLOW_FILES = (
    Path(".github/workflows/build-mahoraga.yml"),
    Path(".github/workflows/release-mahoraga.yml"),
...
ALLOWLIST: tuple[SecretSpec, ...] = (
    ...
    SecretSpec(
        "MAHORAGA_CONFIG_URL",
        ("release-mahoraga.yml", "release-server.yml"),
    ),  # Mahoraga server inline production config URL.
    SecretSpec(
        "POSTHOG_API_KEY",
        ("release-mahoraga.yml", "release-server.yml", "release-extensions.yml"),
    ...
    SecretSpec(
        "SENTRY_DSN",
        ("release-mahoraga.yml", "release-server.yml"),
    ),  # Mahoraga server inline Sentry DSN.
    SecretSpec(
        "AGENT_RUNNER_JWT_SECRET",
        ("release-mahoraga.yml", "release-server.yml"),
    ),  # Optional Mahoraga server remote-agent JWT signing secret.
    SecretSpec(
        "ESIGNER_USERNAME",
        (
            "build-mahoraga.yml",
    ...
    SecretSpec(
        "MAHORAGA_AGENT_V2_KEY",
        ("release-mahoraga.yml", "release-extensions.yml"),
    ),  # Mahoraga agent extension signing key.
    SecretSpec(
        "MAHORAGA_CONTROLLER_KEY",
        ("release-extensions.yml",),
    ),  # Mahoraga controller extension signing key.
    SecretSpec(
        "BUGREPORTER_KEY",
        ("release-extensions.yml",),
    ...
    SecretSpec(
        ...
        ("build-mahoraga.yml", "release-extensions.yml"),
