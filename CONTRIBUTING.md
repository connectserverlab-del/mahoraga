...
Hey there! Thanks for your interest in Mahoraga. Whether you're fixing bugs, adding features, improving docs, or just poking around the code, we're glad you're here.

Mahoraga is a monorepo with two main parts:
...
4. Select `packages/mahoraga-agent/dist/`
5. Press Agent icon from extensions toolbar to open the agent panel

**For detailed setup, architecture, and code standards, see [Agent Contributing Guide](packages/mahoraga-agent/CONTRIBUTING.md).**
...
uv run mahoraga build --preset debug --chromium-src /path/to/chromium/src

# Release build (for production; add --no-sign/--no-upload as needed)
uv run mahoraga build --preset release --chromium-src /path/to/chromium/src

# See every pipeline step and preset switch
uv run mahoraga build --list
uv run mahoraga build --help
```

The build typically takes 1-3 hours on modern hardware (M4 Max, Ryzen 9, etc.).

**For the build system architecture and more invocations, see [packages/mahoraga/bos_build/README.md](packages/mahoraga/bos_build/README.md).**
...
- Agent: [packages/mahoraga-agent/CLAUDE.md](packages/mahoraga-agent/CLAUDE.md)
...
│   │
│   └── mahoraga-agent/        # Chrome extension
│       ├── src/
...
Found a bug? [Open an issue](https://github.com/connectserverlab-del/mahoraga/issues/new) with:
- Clear description
- Steps to reproduce
- Expected vs actual behavior
- Screenshots/videos
- Environment details (OS, browser version, Mahoraga version)

### 💡 Suggest Features

Have an idea? [Share it here](https://github.com/connectserverlab-del/mahoraga/issues/99) or chat with us on [Discord](https://discord.gg/YKwjt5vuKr).
...
- **[GitHub Issues](https://github.com/connectserverlab-del/mahoraga/issues)** - Bug reports and features
- **[GitHub Discussions](https://github.com/connectserverlab-del/mahoraga/discussions)** - General questions
...
Questions? Hit us up on [Discord](https://discord.gg/YKwjt5vuKr) or [Twitter](https://twitter.com/mahoraga_ai).
