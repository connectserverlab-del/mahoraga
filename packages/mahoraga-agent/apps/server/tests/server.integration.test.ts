...
import { MOCK_MAHORAGA_RESPONSE_TEXT } from '../src/lib/clients/llm/mock-language-model'
...
describe('HTTP Server Integration Tests', () => {
  beforeAll(async () => {
    config = await ensureMahoraga()

    mcpClient = new Client({
      name: 'mahoraga-integration-test-client',
      version: '1.0.0',
    })
  ...
  describe('Chat endpoint', () => {
    it(
      'streams a mocked chat response for Mahoraga provider requests in test mode',
      async () => {
        ...
        assert.ok(
          fullResponse.includes(MOCK_MAHORAGA_RESPONSE_TEXT),
          'Should include the mocked Mahoraga chat response',
        )
