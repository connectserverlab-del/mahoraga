...
beforeEach(() => {
  lastCall = null
  nextResult = null
  delete process.env.MAHORAGA_ACPX_PROBE_TIMEOUT_MS
})
...
describe('probeAcpAgent — input shape', () => {
  ...
  it('honours MAHORAGA_ACPX_PROBE_TIMEOUT_MS when in the [1000, 120000] range', async () => {
    process.env.MAHORAGA_ACPX_PROBE_TIMEOUT_MS = '90000'
    nextResult = baseProbeResult()
    await probeAcpAgent({ agentId: 'claude' })
    expect(lastCall?.timeoutMs).toBe(90_000)
  })

  it('ignores MAHORAGA_ACPX_PROBE_TIMEOUT_MS when below the floor', async () => {
    process.env.MAHORAGA_ACPX_PROBE_TIMEOUT_MS = '999'
    nextResult = baseProbeResult()
    await probeAcpAgent({ agentId: 'claude' })
    expect(lastCall?.timeoutMs).toBe(120_000)
  })

  it('ignores MAHORAGA_ACPX_PROBE_TIMEOUT_MS when above the ceiling', async () => {
    process.env.MAHORAGA_ACPX_PROBE_TIMEOUT_MS = '300000'
    nextResult = baseProbeResult()
    await probeAcpAgent({ agentId: 'claude' })
    expect(lastCall?.timeoutMs).toBe(120_000)
  })
