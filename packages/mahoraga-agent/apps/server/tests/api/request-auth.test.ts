...
describe('request auth', () => {
  it('accepts loopback and extension origins', () => {
    ...
    expect(isTrustedAppOrigin('chrome-extension://mahoraga')).toBe(true)
    expect(isTrustedAppOrigin('moz-extension://mahoraga')).toBe(true)
  })
  ...
  it('allows requests from trusted origins', async () => {
    ...
    const res = await app.request('http://localhost/agents/status', {
      headers: { Origin: 'chrome-extension://mahoraga' },
    })
