...
const INLINE_ENV_STUBS: Record<string, string> = {
  MAHORAGA_CONFIG_URL: 'https://server-build-test.invalid/config',
...
describe('server build', () => {
  ...
  const binaryPath = resolve(
    rootDir,
    `dist/prod/server/.tmp/binaries/mahoraga-server-${target.id}${target.ext}`,
  )
  const stagedBinaryPath = resolve(
    rootDir,
    `dist/prod/server/${target.id}/resources/bin/mahoraga_server${target.ext}`,
  )
  const zipPath = resolve(
    rootDir,
    `dist/prod/server/mahoraga-server-resources-${target.id}.zip`,
  )
  const tempDir = mkdtempSync(join(tmpdir(), 'mahoraga-build-test-'))
