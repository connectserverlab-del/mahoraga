...
# Cleanup script for Mahoraga test resources
# Kills any running test processes and removes orphaned temp directories

set -e

echo "Cleaning up Mahoraga test resources..."
...
# Kill orphaned test browser processes (matches only Mahoraga launched with
# a test user-data-dir — never the user's dev Mahoraga)
orphan_pids=$(pgrep -f 'mahoraga-test-' 2>/dev/null || true)
...
TEMP_DIR="${TMPDIR:-/tmp}"
temp_dirs=$(find "$TEMP_DIR" -maxdepth 1 -name "mahoraga-test-*" -type d 2>/dev/null | wc -l | tr -d ' ')

if [ "$temp_dirs" -gt 0 ]; then
  echo "  Removing $temp_dirs orphaned temp directories"
  find "$TEMP_DIR" -maxdepth 1 -name "mahoraga-test-*" -type d -exec rm -rf {} + 2>/dev/null || true
fi
