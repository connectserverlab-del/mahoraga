...
import { TEST_PORTS } from '@mahoraga/shared/constants/ports'

const DEFAULT_BINARY_PATH =
  process.env.MAHORAGA_BINARY ??
...
function resolveFixedPort(
  testEnvName:
    | 'MAHORAGA_TEST_CDP_PORT'
    | 'MAHORAGA_TEST_SERVER_PORT'
    | 'MAHORAGA_TEST_EXTENSION_PORT',
  baseEnvName:
    | 'MAHORAGA_CDP_PORT'
    | 'MAHORAGA_SERVER_PORT'
    | 'MAHORAGA_EXTENSION_PORT',
): number | undefined {
  const testPort = parsePort(process.env[testEnvName], testEnvName)
  if (testPort !== undefined) {
    return testPort
  }
  if (process.env.MAHORAGA_TEST_USE_ENV_PORTS === 'true') {
...
export async function createTestRuntimePlan(): Promise<TestRuntimePlan> {
  ...
  const userDataDir = mkdtempSync(join(tmpdir(), 'mahoraga-test-'))
  const headless = process.env.MAHORAGA_TEST_HEADLESS !== 'false'
  const extraArgs = parseExtraArgs(process.env.MAHORAGA_TEST_EXTRA_ARGS)
