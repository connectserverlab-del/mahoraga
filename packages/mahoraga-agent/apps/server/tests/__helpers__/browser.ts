 ...
 *
 * Low-level Mahoraga process management.
 * Use setup.ts:ensureMahoraga() for the full test environment.
 */
import type { ChildProcess } from 'node:child_process'
import { spawn, spawnSync } from 'node:child_process'
import { rmSync } from 'node:fs'

const TEST_USER_DATA_PREFIX = 'mahoraga-test-'
...
function shouldLogBrowserOutput(): boolean {
  return (
    process.env.CI === 'true' || process.env.MAHORAGA_TEST_DEBUG === 'true'
  )
}
...
function killOrphanedTestBrowsers(
  ...
  // Matches only Mahoraga processes launched with a test user-data-dir
  // (e.g., /var/folders/.../mahoraga-test-XXXX). Never matches a dev
  // Mahoraga run from ~/Library/Application Support/Mahoraga.
...
export async function spawnBrowser(
  ...
  console.log(`Starting Mahoraga on CDP port ${config.cdpPort}...`)
  const browserProcess = spawn(
    ...
    [
      ...
      // Match the supported dev/test launch path and keep legacy Mahoraga
      ...
      // Mahoraga tests still need Chromium's remote debugging flag here.
      `--remote-debugging-port=${config.cdpPort}`,
      `--mahoraga-mcp-port=${config.serverPort}`,
      `--mahoraga-extension-port=${config.extensionPort}`,
  ...
  browserProcess.on('error', (error) => {
    console.error('Failed to start Mahoraga:', error)
  })
...
/** Stops the shared Mahoraga test process and removes its temp profile. */
export async function killBrowser(): Promise<void> {
  ...
  console.log('Shutting down Mahoraga...')
  ...
  console.log('Mahoraga stopped')
  killOrphanedTestBrowsers('Killed dangling Mahoraga test processes')
