...
export async function spawnServer(config: ServerConfig): Promise<ServerState> {
  ...
  console.log(`Starting Mahoraga Server on port ${config.serverPort}...`)
  const configDir = mkdtempSync(join(tmpdir(), 'mahoraga-server-config-'))
