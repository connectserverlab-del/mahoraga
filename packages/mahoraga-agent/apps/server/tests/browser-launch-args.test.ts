...
async function captureBrowserArgs(headless: boolean): Promise<string[]> {
  const tempDir = mkdtempSync(join(tmpdir(), 'mahoraga-launch-args-'))
  const argsPath = join(tempDir, 'args.txt')
  const binaryPath = join(tempDir, 'mahoraga-fake')
  await Bun.write(
    binaryPath,
    [
      '#!/bin/sh',
      'args_file="$MAHORAGA_TEST_ARGS_FILE.tmp"',
      ': > "$args_file"',
      'for arg in "$@"; do',
      '  printf "%s\\n" "$arg" >> "$args_file"',
      'done',
      'mv "$args_file" "$MAHORAGA_TEST_ARGS_FILE"',
      'sleep 60',
    ].join('\n'),
  )
  ...
  const originalArgsFile = process.env.MAHORAGA_TEST_ARGS_FILE
  process.env.MAHORAGA_TEST_ARGS_FILE = argsPath
  try {
    await spawnBrowser({
      ...
      binaryPath,
      userDataDir: mkdtempSync(join(tmpdir(), 'mahoraga-test-')),
      headless,
      extraArgs: [],
    })
    ...
    if (originalArgsFile === undefined) {
      delete process.env.MAHORAGA_TEST_ARGS_FILE
    } else {
      process.env.MAHORAGA_TEST_ARGS_FILE = originalArgsFile
    }
    await killBrowser()
...
describe('spawnBrowser', () => {
  ...
  it('uses the dev dock icon without a headless flag in headed mode', async () => {
    ...
    expect(args).toContain('--mahoraga-dock-icon=dev')
