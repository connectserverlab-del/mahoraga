...
mock.module('../../src/lib/mahoraga-dir', () => ({
  getMahoragaDir: () => join(homedir(), '.mahoraga-test'),
}))
...
describe('createLanguageModel — ACP providers', () => {
  ...
  it('adds the user-supplied command to agentRegistryOverrides for acp-custom', async () => {
    ...
    // Built-in tier-2 overrides (Mahoraga-pinned npx commands) are
  ...
  it('falls back to Mahoraga-pinned npx commands for built-in agents when no bundled bun is available', async () => {
    ...
    // Mahoraga controls the version range instead of deferring to
  ...
  it('falls back to getMahoragaDir()/workspaces/<type> when no path or providerId is set', async () => {
    await createLanguageModel(baseConfig() as never)
    expect(lastBuildArgs?.workspacePath).toBe(
      join(homedir(), '.mahoraga-test', 'workspaces', 'claude-code'),
    )
  })
  ...
  it('expands a leading $HOME token in acpFixedWorkspacePath', async () => {
    ...
    await createLanguageModel({
      ...baseConfig(),
      acpFixedWorkspacePath: '$HOME/mahoraga-workspaces/harness-claude-1',
    } as never)
    expect(lastBuildArgs?.workspacePath).toBe(
      `${homedir()}/mahoraga-workspaces/harness-claude-1`,
    )
  })
    ...
    const servers = [
      {
        ...
        headers: [{ name: 'X-Mahoraga-Scope-Id', value: 'conv-mcp-1' }],
      },
    ]
    await createLanguageModel({
      ...baseConfig(),
