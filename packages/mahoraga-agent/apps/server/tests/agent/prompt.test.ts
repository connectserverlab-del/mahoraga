...
describe('workspace gating (P11)', () => {
  ...
  describe('without workspace selected', () => {
    ...
    it('documents output-only reads for Mahoraga-generated files', () => {
      const prompt = buildRegular({
      ...
      expect(prompt).toContain('Mahoraga-generated output files')
...
describe('mode-aware framing', () => {
  ...
  it('chat mode with a selected workspace does not advertise workspace tools when only output reads are registered', () => {
    const prompt = buildChatMode({
      workspaceDir: '/tmp/mahoraga-workspace',
      generatedOutputReadAvailable: true,
    })
    expect(prompt).toContain('### Browser Output Files')
    expect(prompt).not.toContain('<workspace>')
...
//
// Why: The shipped Mahoraga Soul/Memory feature is unshipped. Soul can still
...
// see browser tools prefixed with `mcp.mahoraga.*` rather than the bare
...
// steers the agent toward Mahoraga MCP tools for browser work over its
...
describe('acp tool namespace section', () => {
  it('is absent when acpMode is unset', () => {
    ...
    expect(prompt).not.toContain('mcp.mahoraga.<name>')
  })
  ...
  it('renders once when acpMode is true', () => {
    ...
    expect(prompt).toContain('mcp.mahoraga.<name>')
    expect(prompt).toContain('mcp.mahoraga.navigate')
