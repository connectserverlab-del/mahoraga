...
import { createBrowserOutputFileAccess } from '@mahoraga/browser-mcp/output-file'
import { registerBrowserTools } from '@mahoraga/browser-mcp/register'
import { BROWSER_TOOLS } from '@mahoraga/browser-mcp/registry'
import {
  getToolOutputDir,
  TOOL_OUTPUT_DIR_MODE,
  TOOL_OUTPUT_FILE_MODE,
} from '@mahoraga/browser-mcp/tool-output-dir'
import {
  defineTool,
  executeTool,
  textResult,
...
async function withMahoragaDir<T>(run: () => Promise<T>): Promise<T> {
  ...
  const mahoragaDir = mkdtempSync(join(tmpdir(), 'mahoraga-output-test-'))
...
`,
  ...
  it('writes large direct diffs to a Mahoraga output markdown file', async () => {
    await withMahoragaDir(async () => {
      const fake = createFakeServer()
  ...
  it('writes large URL-change act readbacks to a Mahoraga output file', async () => {
    await withMahoragaDir(async () => {
      const fake = createFakeServer()
  ...
  it('caps large read results and writes the full content to a Mahoraga output file', async () => {
    await withMahoragaDir(async () => {
      const fake = createFakeServer()
  ...
  it('prints a page to a Mahoraga output PDF file', async () => {
    await withMahoragaDir(async () => {
      const fake = createFakeServer()
  ...
  it('downloads a file via a ref click into the Mahoraga output dir', async () => {
    await withMahoragaDir(async () => {
      const fake = createFakeServer()
  ...
  it('writes very large snapshots to a Mahoraga output markdown file', async () => {
    await withMahoragaDir(async () => {
      const fake = createFakeServer()
  ...
  it('writes very long unbroken snapshots to a Mahoraga output markdown file', async () => {
    await withMahoragaDir(async () => {
      const fake = createFakeServer()
...
describe('buildBrowserToolSet', () => {
  ...
  it('allows no-workspace filesystem readback for AI SDK downloads', async () => {
    await withMahoragaDir(async () => {
      ...
      expect(readResult.text).toContain('mahoraga,1')
    })
  })
