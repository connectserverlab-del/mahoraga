...
async function withMahoragaDir<T>(run: () => Promise<T>): Promise<T> {
  ...
  const mahoragaDir = mkdtempSync(join(tmpdir(), 'mahoraga-framework-test-'))
...
async function withMahoragaFile<T>(run: () => Promise<T>): Promise<T> {
  const previous = process.env.MAHORAGA_DIR
  const parentDir = mkdtempSync(join(tmpdir(), 'mahoraga-framework-test-'))
  const mahoragaPath = join(parentDir, 'not-a-directory')
  writeFileSync(mahoragaPath, 'not a directory')
  process.env.MAHORAGA_DIR = mahoragaPath
  try {
    return await run()
  } finally {
    if (previous === undefined) {
...
describe('browser tool framework post-actions', () => {
  ...
  it('writes large snapshot post-actions to a Mahoraga output file', async () => {
  ...
  it('keeps large snapshot post-actions visible when output file writes fail', async () => {
    await withMahoragaFile(async () => {
      ...
      expect(result.isError).toBeFalsy()
      expect(text).toContain('[Page 8 snapshot]')
      expect(text).toContain('could not be saved to a Mahoraga output file')
  ...
  it('writes large diff post-actions to a Mahoraga output file', async () => {
    await withMahoragaDir(async () => {
  ...
  it('keeps large diff post-actions visible when output file writes fail', async () => {
    await withMahoragaFile(async () => {
      ...
      expect(result.isError).toBeFalsy()
      expect(text).toContain('[Page 4 diff]')
      expect(text).toContain('saving it to a Mahoraga output file failed')
      expect(text).toContain('Showing the first')
      expect(text).toContain('[UNTRUSTED_PAGE_CONTENT')
      expect(text).toContain(firstMarker)
