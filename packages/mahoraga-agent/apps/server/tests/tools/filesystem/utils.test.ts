...
import { getToolOutputDir } from '../../../src/lib/mahoraga-dir'
import {
  detectLineEnding,
  isBinaryPath,
  normalizeToLF,
...
describe('filesystem path boundaries', () => {
  ...
  beforeEach(async () => {
    ...
    mahoragaDir = join(
      tmpdir(),
      `fs-boundary-mahoraga-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    )
    ...
    await mkdir(mahoragaDir, { recursive: true })
  })
  ...
  it('accepts Mahoraga tool output paths under the dedicated output directory', async () => {
  ...
  it('rejects relative Mahoraga tool output paths', async () => {
    await expect(resolveBrowserToolOutputPath('snapshot.md')).rejects.toThrow(
      'absolute Mahoraga tool output path',
    )
  })

  it('rejects sibling Mahoraga state paths outside the output directory', async () => {
    const outputDir = await getToolOutputDir()
    ...
    await expect(resolveBrowserToolOutputPath(siblingPath)).rejects.toThrow(
      'outside Mahoraga tool output',
    )
  })

  it('rejects symlinked Mahoraga tool output roots', async () => {
    ...
    await expect(resolveBrowserToolOutputPath(outputPath)).rejects.toThrow(
      'Mahoraga tool output directory must be a real directory',
    )
  })
})
