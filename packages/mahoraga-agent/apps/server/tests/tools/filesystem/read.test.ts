...
} from '@mahoraga/browser-mcp/output-file'
import { getToolOutputDir } from '@mahoraga/browser-mcp/tool-output-dir'
import { read as browserRead } from '@mahoraga/browser-mcp/tools/read'
...
let tmpDir: string
let mahoragaDir: string
let previousMahoragaDir: string | undefined
...
beforeEach(async () => {
  previousMahoragaDir = process.env.MAHORAGA_DIR
  mahoragaDir = join(
    tmpdir(),
    `fs-read-mahoraga-${Date.now()}-${Math.random().toString(36).slice(2)}`,
  )
...
afterEach(async () => {
  ...
  await rm(mahoragaDir, { recursive: true, force: true })
  if (previousMahoragaDir === undefined) {
    delete process.env.MAHORAGA_DIR
  } else {
    process.env.MAHORAGA_DIR = previousMahoragaDir
  }
})
...
describe('filesystem_read', () => {
  ...
  it('reads Mahoraga-generated output files', async () => {
  ...
  it('rejects absolute Mahoraga state paths outside generated outputs', async () => {
    await getToolOutputDir()
    const statePath = join(mahoragaDir, 'config.json')
    await writeFile(statePath, '{}')

    const result = await exec({ path: statePath })
    expect(result.isError).toBe(true)
    expect(result.text).toContain('outside Mahoraga tool output')
  })

  it('reads Mahoraga-generated output files without a workspace', async () => {
    const outputDir = await getToolOutputDir()
    const outputPath = join(outputDir, 'snapshot.md')
  ...
  it('rejects unregistered Mahoraga-generated output files without a workspace', async () => {
    const outputDir = await getToolOutputDir()
    const outputPath = join(outputDir, 'snapshot.md')
  ...
  it('rejects relative paths without a workspace', async () => {
    ...
    expect(result.text).toContain('Mahoraga-generated tool output')
  })

  it('rejects Mahoraga state paths outside generated outputs without a workspace', async () => {
    await getToolOutputDir()
    const statePath = join(mahoragaDir, 'config.json')
    await writeFile(statePath, '{}')
    const noWorkspaceExec = createReadExec()
