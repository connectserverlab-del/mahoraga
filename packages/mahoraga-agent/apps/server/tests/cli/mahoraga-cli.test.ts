...
import { cleanupMahoraga, ensureMahoraga } from '../__helpers__/index'
...
function buildCli(): void {
  tmpDir = mkdtempSync(join(tmpdir(), 'mahoraga-cli-runtime-'))
  cliBinary = join(tmpDir, 'mahoraga-cli')
  ...
  if (result.status !== 0) {
    throw new Error(
      `failed to build mahoraga-cli\nstdout:\n${result.stdout}\nstderr:\n${result.stderr}`,
    )
  }
}

/** Runs the built CLI against the Mahoraga test server without reading user config. */
...
function runJson(args: string[]): Record<string, unknown> {
  ...
  if (result.status !== 0) {
    throw new Error(
      `mahoraga-cli ${args.join(' ')} exited ${result.status}\nstdout:\n${result.stdout}\nstderr:\n${result.stderr}`,
    )
  }
  try {
    ...
    throw new Error(
      `mahoraga-cli ${args.join(' ')} returned invalid JSON: ${error instanceof Error ? error.message : String(error)}\nstdout:\n${result.stdout}`,
    )
  }
}

describe('mahoraga-cli runtime commands', () => {
  beforeAll(async () => {
    buildCli()
    const config = await ensureMahoraga()
    serverUrl = `http://127.0.0.1:${config.serverPort}`
  })

  afterAll(async () => {
    if (!process.env.KEEP_BROWSER) {
      await cleanupMahoraga()
    }
    if (tmpDir) {
