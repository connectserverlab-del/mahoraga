...
const originalHeadless = process.env.MAHORAGA_TEST_HEADLESS
const runtimePlans: TestRuntimePlan[] = []

afterEach(() => {
  if (originalHeadless === undefined) {
    delete process.env.MAHORAGA_TEST_HEADLESS
  } else {
    process.env.MAHORAGA_TEST_HEADLESS = originalHeadless
  }
...
async function resolveHeadless(value: string | undefined): Promise<boolean> {
  if (value === undefined) {
    ...
    process.env.MAHORAGA_TEST_HEADLESS = value
  }
