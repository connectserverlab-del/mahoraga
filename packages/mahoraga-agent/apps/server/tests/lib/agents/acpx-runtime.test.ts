...
describe('AcpxRuntime', () => {
  ...
  it('uses acpx/runtime to ensure a session and stream a turn', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'mahoraga-acpx-runtime-'))
    const stateDir = await mkdtemp(join(tmpdir(), 'mahoraga-acpx-state-'))
    ...
    expect(events).toEqual([
      {
        type: 'status',
        text: 'Requested model is stored on the Mahoraga agent, but this acpx/runtime version does not expose public model control. Using adapter default.',
      },
      {
        type: 'text_delta',
  ...
  it('uses the shared harness workspace as the default cwd and composes the ACPX run prompt', async () => {
    const mahoragaDir = await mkdtemp(
      join(tmpdir(), 'mahoraga-acpx-mahoraga-'),
    )
    const stateDir = await mkdtemp(join(tmpdir(), 'mahoraga-acpx-state-'))
    tempDirs.push(mahoragaDir, stateDir)
    const calls: Array<{ method: string; input: unknown }> = []
    ...
    const expectedCwd = join(mahoragaDir, 'agents', 'harness', 'workspace')
  ...
  it('uses selected cwd in the runtime fingerprint', async () => {
    ...
    const stateDir = await mkdtemp(join(tmpdir(), 'mahoraga-acpx-state-'))
    const selected = await mkdtemp(join(tmpdir(), 'mahoraga-acpx-selected-'))
    tempDirs.push(mahoragaDir, stateDir, selected)
    const calls: Array<{ method: string; input: unknown }> = []
    const runtime = new AcpxRuntime({
      mahoragaDir,
      stateDir,
  ...
  it('surfaces a clear error when selected cwd no longer exists', async () => {
    ...
    const stateDir = await mkdtemp(join(tmpdir(), 'mahoraga-acpx-state-'))
    tempDirs.push(mahoragaDir, stateDir)
    const missingCwd = join(mahoragaDir, 'missing-workspace')
    const calls: Array<{ method: string; input: unknown }> = []
    const runtime = new AcpxRuntime({
      mahoragaDir,
      stateDir,
  ...
  it('loads history from the latest runtime-state session key', async () => {
    ...
    await sessionStore.save(
      makeSessionRecord({
        ...
        cwd: join(mahoragaDir, 'agents', 'harness', 'workspace'),
  ...
  it('shows only the user request for persisted Mahoraga-wrapped prompts', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'mahoraga-acpx-runtime-'))
    const stateDir = await mkdtemp(join(tmpdir(), 'mahoraga-acpx-state-'))
    tempDirs.push(cwd, stateDir)
    const timestamp = '2026-04-28T20:00:00.000Z'
      ...
      messages: [
        {
          User: {
            ...
            content: [
              {
                Text: `<role>
You are Mahoraga - a browser agent with full control of a Chromium browser through the Mahoraga MCP server.

Use the Mahoraga MCP server for all browser tasks, including browsing the web, interacting with pages, inspecting browser state, and managing tabs, windows, bookmarks, and history.
</role>
...
open &lt;example.com&gt;
  ...
  it('strips the inner formatUserMessage envelope from history payloads', async () => {
    ...
    // `buildMahoragaAcpPrompt` runs `escapePromptTagText` over the
...
&lt;/USER_QUERY&gt;
  ...
  describe('unwrapMahoragaAcpUserMessage', () => {
    it('returns clean text for input that has no envelope', () => {
      expect(unwrapMahoragaAcpUserMessage('hello')).toBe('hello')
    })

    it('handles empty input', () => {
      expect(unwrapMahoragaAcpUserMessage('')).toBe('')
    })
...
&lt;/USER_QUERY&gt;
      ...
      expect(unwrapMahoragaAcpUserMessage(wrapped)).toBe('look at example')
    })
...
just inner
      ...
      expect(unwrapMahoragaAcpUserMessage(innerOnly)).toBe('just inner')
    })
...
just outer
      ...
      expect(unwrapMahoragaAcpUserMessage(outerOnly)).toBe('just outer')
    })

    it('strips an arbitrary single-line role envelope', () => {
      const wrapped = `<role>You are running inside Mahoraga through an ACP adapter.</role>

<user_request>
Need another report this time as pdf, a comparison between both yahoo and google reports you created...
</user_request>`
      expect(unwrapMahoragaAcpUserMessage(wrapped)).toBe(
        'Need another report this time as pdf, a comparison between both yahoo and google reports you created...',
      )
    })

    it('strips the ACPX runtime envelope when it wraps persisted history', () => {
      const wrapped = `<mahoraga_acpx_runtime version="2026-05-02.v1">
You are Mahoraga, an ACPX browser agent.

Skill root: /tmp/runtime-skills
</mahoraga_acpx_runtime>

<user_request>
new runtime prompt
</user_request>`
      expect(unwrapMahoragaAcpUserMessage(wrapped)).toBe('new runtime prompt')
    })
...
&lt;/USER_QUERY&gt;
      ...
      const once = unwrapMahoragaAcpUserMessage(wrapped)
      const twice = unwrapMahoragaAcpUserMessage(once)
    ...
    it('round-trips formatUserMessage output back to the user typed text', () => {
      ...
      // Mirror what acpx-runtime.ts's buildMahoragaAcpPrompt does
...
${escapeForPrompt(formatted)}
      ...
      expect(unwrapMahoragaAcpUserMessage(wrapped)).toBe(userText)
    })
    ...
    it('preserves user-typed angle-brackets via the entity decode', () => {
      ...
      // produces after formatUserMessage + buildMahoragaAcpPrompt.
      const wrapped = `<role>
You are Mahoraga - a browser agent with full control of a Chromium browser through the Mahoraga MCP server.
...
&lt;/USER_QUERY&gt;
  ...
  it('continues the turn when runtime config control is unavailable', async () => {
    const calls: Array<{ method: string; input: unknown }> = []
    const runtime = new AcpxRuntime({
      cwd: '/tmp/mahoraga-acpx-runtime',
      stateDir: '/tmp/mahoraga-acpx-state',
  ...
  it('configures Mahoraga MCP and wraps turns with browser instructions', async () => {
    const calls: Array<{ method: string; input: unknown }> = []
    const runtime = new AcpxRuntime({
      cwd: '/tmp/mahoraga-acpx-runtime',
      stateDir: '/tmp/mahoraga-acpx-state',
      mahoragaServerPort: 9321,
      runtimeFactory: (options) => {
        calls.push({ method: 'createRuntime', input: options })
        return createFakeAcpRuntime(calls)
      },
    ...
    expect(calls[0]?.input).toMatchObject({
      mcpServers: [
        ...
        },
        {
          ...
          headers: [
            { name: 'X-Mahoraga-Agent-Id', value: 'agent-1' },
            { name: 'X-Mahoraga-Session-Id', value: 'main' },
          ],
        },
      ],
    })
  ...
  it('injects AGENT_HOME without CLAUDE_CONFIG_DIR into Claude ACP command resolution', async () => {
    ...
    // Spawn must go through Mahoraga's own npx command for the official
  ...
  it('prepends the bundled native CLI directory to host ACP adapter commands', async () => {
    ...
    const resourcesDir = await mkdtemp(
      join(tmpdir(), 'mahoraga-acpx-resources-'),
    )
    tempDirs.push(mahoragaDir, stateDir, resourcesDir)
...
async function createLatestRuntimeStateForTest(input: {
  ...
  const latest = {
    ...
    cwd: join(input.mahoragaDir, 'agents', 'harness', 'workspace'),
    agentHome: join(
      input.mahoragaDir,
      'agents',
      'harness',
