...
describe('resolveLLMConfig', () => {
  ...
  it('defaults ChatGPT OAuth providers to GPT-5.5', async () => {
    const mahoragaId = 'mahoraga-id'
    const dir = mkdtempSync(join(tmpdir(), 'mahoraga-llm-config-test-'))
    tempDirs.push(dir)
    const handle = initializeDb({
      dbPath: join(dir, 'db', 'mahoraga.sqlite'),
    })
    initializeOAuth(handle.db, mahoragaId)
    new OAuthTokenStore(handle.db).upsertTokens(mahoragaId, 'chatgpt-pro', {
