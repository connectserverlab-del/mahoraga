...
MCP server and AI agent loop powering Mahoraga browser automation. This is the core backend — it connects to Chromium via CDP, exposes 53+ MCP tools, and runs the AI agent that interprets natural language into browser actions.
...
┌──────────────────────────────────────────────────────────────────────┐
│                         MCP Clients                                  │
│           (Agent UI, Claude Code, Gemini CLI, mahoraga-cli)         │
└──────────────────────────────────────────────────────────────────────┘
                                │
...
| **Other** | `mahoraga_info`, `handle_dialog`, `wait_for`, `download`, `export_pdf`, `output_file`, `nudges` |
...
- A running Mahoraga instance (for CDP connectivity)
...
bun run test:integration    # Full integration tests (requires running Mahoraga)
```
...
Bump `packages/mahoraga-agent/apps/server/package.json` and the matching `bun.lock` entry in a PR, merge it, then tag the merged commit:

```bash
git tag -a agent-server/v0.0.123 -m "Mahoraga Server - v0.0.123"
