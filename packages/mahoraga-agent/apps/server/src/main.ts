...
import { EXIT_CODES } from '@mahoraga/shared/constants/exit-codes'
...
export class Application {
  ...
  async start(): Promise<void> {
    logger.info(`Starting Mahoraga Server v${VERSION}`)
    ...
    try {
      await createHttpServer({
        ...
        browser,
        browserSession,
        mahoragaId: identity.getMahoragaId(),
    ...
    try {
      await writeServerConfig({
        ...
        mahoraga_version: this.config.instanceMahoragaVersion,
        chromium_version: this.config.instanceChromiumVersion,
        mahoraga_id: identity.getMahoragaId(),
      })
    } catch (error) {
    ...
    // Boot self-heal for the MCP integration. First drops Mahoraga
    // from any agent no longer in the curated surface, then repairs
    // every remaining agent's Mahoraga MCP URL against the proxy URL
    ...
    // `MAHORAGA_MCP_PUBLIC_URL`; otherwise we'd rewrite every agent
    ...
    const publicMcpUrl = process.env.MAHORAGA_MCP_PUBLIC_URL
    ...
    if (!publicMcpUrl) {
      logger.debug(
        'MCP manager URL reconcile skipped — MAHORAGA_MCP_PUBLIC_URL not set',
      )
    }
  ...
  private async initCoreServices(): Promise<void> {
    ...
    const mahoragaId = identity.getMahoragaId()
    logger.info('Mahoraga ID initialized', {
      mahoragaId: mahoragaId.slice(0, 12),
    ...
    Sentry.setUser({ id: mahoragaId })
    Sentry.setContext('mahoraga', {
      client_id: this.config.instanceClientId,
      install_id: this.config.instanceInstallId,
      mahoraga_version: this.config.instanceMahoragaVersion,
      chromium_version: this.config.instanceChromiumVersion,
