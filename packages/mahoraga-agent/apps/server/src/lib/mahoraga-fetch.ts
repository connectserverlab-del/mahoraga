 ...
 *
 * Custom fetch for Mahoraga gateway requests.
 * Adds X-Mahoraga-ID header for credit tracking,
...
export function createMahoragaFetch(mahoragaId: string): typeof fetch {
  return (async (url: RequestInfo | URL, options?: RequestInit) => {
    const headers = new Headers(options?.headers)
    headers.set('X-Mahoraga-ID', mahoragaId)
