 ...
 *
 * Singleton accessor for the `@mahoraga/agent-mcp-manager` bound API.
 * The workspaceDir is pinned to `<getMahoragaDir()>/mcp-manager` so
 * the manifest of which agents Mahoraga has installed itself into
 * lives next to the rest of the Mahoraga state and travels with the
 * install.
 *
...
import { type BoundApi, bind } from '@mahoraga/agent-mcp-manager'
import { getMahoragaDir } from '../mahoraga-dir'

/**
 * Server-name Mahoraga registers itself under for agents that speak
 * MCP over HTTP natively (Claude Code, Codex, Cursor, OpenCode,
 * Antigravity, VS Code, Zed). Stdio-only agents, when supported, get
 * a separate entry under `MAHORAGA_MCP_STDIO_SERVER_NAME` below.
 */
export const MAHORAGA_MCP_SERVER_NAME = 'mahoraga'

/**
 * Server-name Mahoraga registers itself under for stdio-only agents.
 * The spec wraps `npx mcp-remote <url>` so a stdio client can speak
 * to the Mahoraga HTTP MCP endpoint. Kept as a separate manifest
 ...
 * earlier installs.
 */
export const MAHORAGA_MCP_STDIO_SERVER_NAME = 'mahoraga-stdio'

let cached: BoundApi | null = null

export function getMcpManagerWorkspaceDir(): string {
  return join(getMahoragaDir(), 'mcp-manager')
}
