 ...
 *
 * Boot-time self-heal for the Mahoraga MCP integration.
 *
 * Two managed manifest entries carry Mahoraga's server spec:
 *   - `mahoraga` (HTTP spec for HTTP-native agents)
 *   - `mahoraga-stdio` (stdio spec wrapping `npx mcp-remote <url>`)
 *
 * When Mahoraga restarts on a different port every agent config that
 * previously linked to Mahoraga still points at the stale URL. The
 ...
 * agent's config off disk and reports which still carry a Mahoraga
...
import type { McpHttpSpec, McpStdioSpec } from '@mahoraga/agent-mcp-manager'
...
function rebuildSpec(
  ...
  if (serverName === MAHORAGA_MCP_STDIO_SERVER_NAME) {
    return {
      transport: 'stdio',
      command: 'npx',
...
/**
 * Repairs every still-present Mahoraga entry to point at `currentUrl`.
...
export async function reconcileUrl(
  ...
  logger.info('MCP manager reconciled Mahoraga URL', {
 ...
 * Mahoraga from any agent no longer in the curated surface (Gemini,
