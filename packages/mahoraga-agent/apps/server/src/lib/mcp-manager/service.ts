...
import {
  ...
  MAHORAGA_MCP_STDIO_SERVER_NAME,
  getMcpManager,
} from './manager'
import type {
  InstallAgentResult,
...
/**
 * The curated harness set Mahoraga surfaces in the Integrations
...
/**
 * The two server-names Mahoraga manages in the manifest. Every
 ...
 */
const MAHORAGA_SERVER_NAMES: readonly string[] = [
  MAHORAGA_MCP_SERVER_NAME,
  MAHORAGA_MCP_STDIO_SERVER_NAME,
]
...
function planFor(agentId: AgentId, currentUrl: string): AgentServerPlan {
  ...
  if (!supportsHttp) {
    ...
    return { serverName: MAHORAGA_MCP_STDIO_SERVER_NAME, spec }
  }
  const spec: McpHttpSpec = { transport: 'http', url: currentUrl }
  return { serverName: MAHORAGA_MCP_SERVER_NAME, spec }
}

/**
 * Detects the curated agents on disk and reports Mahoraga's link
...
export async function listAgents(
  ...
  const [detectedRaw, links] = await Promise.all([
    detect(),
    mgr.listLinks({ serverNames: [...MAHORAGA_SERVER_NAMES] }),
  ])
  ...
  for (const id of CURATED_AGENTS) {
    ...
    // An existing Mahoraga link means a working install regardless of
...
/**
 * Install Mahoraga into the given agent's config. Idempotent: a
 ...
 * `mahoraga` and later re-routed to stdio by the upstream catalog
...
export async function installInto(
  ...
  logger.info('Installed Mahoraga MCP into agent', {
    agent: agentId,
    serverName,
  })
  return { success: true }
...
/**
 * Uninstall Mahoraga from the given agent's config. Tries every
 * server name Mahoraga manages because the same agent may be linked
 * under either `mahoraga` (http) or `mahoraga-stdio` depending on
...
export async function uninstallFrom(
  ...
  for (const serverName of MAHORAGA_SERVER_NAMES) {
    try {
      await mgr.disconnect({ serverName, agent: agentId, removeIfLast: true })
      logger.info('Uninstalled Mahoraga MCP from agent', {
        agent: agentId,
        serverName,
      })
    } catch (err) {
  ...
  if (foreignError) {
    return {
      success: false,
      message:
        'Cannot remove a user-edited entry. Please remove Mahoraga from this agent manually and try again.',
    }
  }
  return { success: true }
}
...
/**
 * Cleans any pre-existing Mahoraga link for `agentId` under server
...
export function humaniseInstallError(err: unknown): {
  ...
  if (err instanceof ForeignEntryError) {
    return {
      message:
        "Cannot replace a user-edited entry. Please remove Mahoraga from this agent's config manually and try again.",
      status: 409,
    }
  }
  ...
  if (err instanceof UnsupportedTransportError) {
    return {
      message: `This agent does not support Mahoraga's MCP transport. ${err.message}`,
      status: 400,
    }
  }
...
/**
 * Startup self-heal. Disconnects Mahoraga from any agent that is no
 ...
 * them linked would strand a Mahoraga entry in a config the panel can
...
export async function cleanupNonCuratedLinks(): Promise<AgentId[]> {
  ...
  try {
    links = await mgr.listLinks({ serverNames: [...MAHORAGA_SERVER_NAMES] })
  } catch (err) {
