...
export interface BuildAcpxProviderOptions {
   ...
   * `undefined`. Mahoraga ships ACP agents in `approve-all` so read +
   ...
   * subdirectory under `getMahoragaDir()` so it sits next to the rest
   * of the Mahoraga state and gets cleaned up with the install.
   */
  stateDir?: string
}
...
function defaultStateDir(): string {
  return join(getMahoragaDir(), 'acpx-state')
}
