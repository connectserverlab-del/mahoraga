...
export const MAHORAGA_ACPX_OPERATING_PROMPT_VERSION = '2026-05-02.v1'
...
export function resolveAgentRuntimePaths(input: {
  ...
  const harnessDir = join(input.mahoragaDir, 'agents', 'harness')
...
/** Writes built-in Mahoraga runtime skills and returns their stable names. */
...
/** Builds stable Mahoraga-managed instructions for Claude/Codex ACP turns. */
export function buildAcpxRuntimePromptPrefix(input: {
  agent: AgentDefinition
  paths: AgentRuntimePaths
  skillNames: string[]
}): string {
  return `<mahoraga_acpx_runtime version="${MAHORAGA_ACPX_OPERATING_PROMPT_VERSION}">
...
Mahoraga has made runtime skills available for this ACPX session.
Skill root: ${input.paths.runtimeSkillsDir}
Available skills: ${input.skillNames.join(', ')}
When a task calls for one of these skills, read its SKILL.md from that root and follow it.

When the user asks you to remember, save feedback, store a preference, or update memory in this Mahoraga ACPX context, use the Mahoraga memory skill.
...
export function buildMahoragaAcpPrompt(
  prefix: string,
  message: string,
): string {
  return `${prefix}
