...
import { DEFAULT_PORTS } from '@mahoraga/shared/constants/ports'
import {
...
export class AcpxRuntime implements AgentRuntime {
  ...
  private readonly mahoragaDir: string
  private readonly resourcesDir: string | null
  private readonly stateDir: string
  private readonly mahoragaServerPort: number
  ...
  constructor(options: AcpxRuntimeOptions = {}) {
    ...
    this.mahoragaDir = options.mahoragaDir ?? getMahoragaDir()
    this.resourcesDir = options.resourcesDir ?? null
    this.stateDir =
      options.stateDir ??
      process.env.MAHORAGA_ACPX_STATE_DIR ??
      join(this.mahoragaDir, 'agents', 'acpx')
    this.mahoragaServerPort =
      options.mahoragaServerPort ?? DEFAULT_PORTS.server
  ...
  async send(
    ...
    const runtime = this.getRuntime({
      ...
      useMahoragaMcp: prepared.useMahoragaMcp,
      mahoragaMcpHost: prepared.mahoragaMcpHost,
      agentId: input.agent.id,
      sessionId: input.sessionId,
    })
  ...
  private getRuntime(input: {
    ...
    const mcpHost = input.mahoragaMcpHost ?? '127.0.0.1'
    ...
    const key = JSON.stringify({
      ...
      useMahoragaMcp: input.useMahoragaMcp,
      mahoragaMcpHost: mcpHost,
    ...
    const runtime = this.runtimeFactory({
      ...
      agentRegistry: createMahoragaAgentRegistry({
        commandEnv: input.commandEnv,
        resourcesDir: this.resourcesDir,
        mahoragaDir: this.mahoragaDir,
      }),
      mcpServers: input.useMahoragaMcp
        ? createMahoragaMcpServers(this.mahoragaServerPort, mcpHost, {
            agentId: input.agentId,
            sessionId: input.sessionId,
          })
        : [],
    ...
    logger.debug('Agent harness acpx runtime created', {
      ...
      permissionMode: input.permissionMode,
      nonInteractivePermissions: input.nonInteractivePermissions,
      mahoragaServerPort: this.mahoragaServerPort,
      mahoragaMcpHost: mcpHost,
      commandIdentity: input.commandIdentity,
      useMahoragaMcp: input.useMahoragaMcp,
      agentId: input.agentId,
...
function userContentToText(content: AcpxUserContent): string {
  if ('Text' in content) return unwrapMahoragaAcpUserMessage(content.Text)
...
/**
 * Strip the Mahoraga ACP envelopes from a user-message text so HTTP
 ...
 */
export function unwrapMahoragaAcpUserMessage(raw: string): string {
  if (!raw) return raw
  let text = raw

  // Order matters: the outer envelope is added AFTER
  // `escapePromptTagText` runs over the inner formatUserMessage
  // payload (see buildMahoragaAcpPrompt). So once the outer
...
function stripOuterRuntimeEnvelope(value: string): string {
  const match = value.match(
    /^<mahoraga_acpx_runtime\b[\s\S]*?<\/mahoraga_acpx_runtime>\n\n<user_request>\n([\s\S]*?)\n<\/user_request>$/,
  )
  return match ? match[1] : value
}
...
function createAcpxEventStream(
  ...
  return new ReadableStream<AgentStreamEvent>({
    start(controller) {
      const run = async () => {
        ...
        logger.info('Agent harness acpx session ensured', {
          ...
          adapter: input.agent.adapter,
          sessionKey: prepared.runtimeSessionKey,
          mahoragaSessionKey: input.sessionKey,
    ...
    cancel() {
      void activeTurn?.cancel({ reason: 'Mahoraga stream cancelled' })
    },
  })
}

function createMahoragaMcpServers(
  mahoragaServerPort: number,
  host: string,
  ...
  return [
    {
      type: 'http',
      name: 'mahoraga',
      url: `http://${host}:${mahoragaServerPort}/mcp`,
      headers: [],
    },
    ...
    // right stream via TurnRegistry.pushEvent.
    {
      type: 'http',
      name: 'nudge',
      url: `http://${host}:${mahoragaServerPort}/mcp/nudge`,
      headers: [
        { name: 'X-Mahoraga-Agent-Id', value: turnIdentity.agentId },
        { name: 'X-Mahoraga-Session-Id', value: turnIdentity.sessionId },
      ],
    },
  ]
}

function createMahoragaAgentRegistry(input: {
  commandEnv: Record<string, string>
  ...
  return {
    ...
    resolve(agentName) {
      ...
      if (lower === 'claude' || lower === 'codex') {
        const launch = resolveMahoragaHostAcpAdapterCommand({
          adapter: lower,
 ...
 * to discover package bins. Packaged macOS builds prefer Mahoraga's
 * bundled Bun so adapter package execution doesn't depend on host
 * `npx` or the app launch environment.
 */
function resolveMahoragaHostAcpAdapterCommand(input: {
