...
Durable, promoted memory for this Mahoraga ACPX agent.
...
export const RUNTIME_SKILLS: Record<string, string> = {
  mahoraga: `---
name: mahoraga
description: Use Mahoraga MCP tools for browser automation.
---

# Mahoraga MCP

Use Mahoraga MCP for browser work.
...
- Write Mahoraga memory only under $AGENT_HOME.
...
- Do not use native Claude project memory, native CLI memory, or workspace files for Mahoraga memory.
...
Both namespaces are always on the wire whenever Mahoraga is running. Do not try to install anything.

## Decision

When a turn needs a service:

1. If the system prompt's Connected apps block lists the service, use the Strata flow (discover -> get_action_details -> execute_action) under \`mahoraga/*\`.
...
Pass the exact display name from the Mahoraga catalog. Proper-case, spaces preserved. Wrong casing yields a 400.
