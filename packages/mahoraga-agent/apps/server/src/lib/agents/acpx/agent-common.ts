...
import {
  MAHORAGA_ACPX_OPERATING_PROMPT_VERSION,
  buildAcpxRuntimePromptPrefix,
  buildMahoragaAcpPrompt,
  ensureAgentHome,
  ensureRuntimeSkills,
  ensureUsableCwd,
...
export interface MahoragaManagedContext {
...
/** Builds the common Mahoraga-managed home, skills, cwd, and prompt prefix for Claude/Codex. */
export async function prepareMahoragaManagedContext(
  input: PrepareAcpxAgentContextInput,
): Promise<MahoragaManagedContext> {
...
/** Finalizes Mahoraga-managed prep into the uniform adapter context consumed by AcpxRuntime. */
export async function finishMahoragaManagedContext(input: {
  input: PrepareAcpxAgentContextInput
  paths: AgentRuntimePaths
  skillNames: string[]
  promptPrefix: string
  ...
  const runtimeSessionKey = deriveRuntimeSessionKey({
    ...
    promptVersion: MAHORAGA_ACPX_OPERATING_PROMPT_VERSION,
  ...
  return {
    cwd: input.paths.effectiveCwd,
    runtimeSessionKey,
    runPrompt: buildMahoragaAcpPrompt(input.promptPrefix, input.input.message),
    commandEnv: input.commandEnv,
    commandIdentity,
    useMahoragaMcp: true,
    mahoragaMcpHost: input.mahoragaMcpHost,
  }
}
