...
export interface RpcRouterDeps {
  ...
  /** Scope id attached as `X-Mahoraga-Scope-Id` for local MCP attribution. */
  scopeId: string
  /** Provider id forwarded as `X-Mahoraga-Agent-Id` for audit. */
  agentId: string
}
  ...
  try {
    res = await fetch(base, {
      ...
      headers: {
        ...
        'X-Mahoraga-Scope-Id': deps.scopeId,
        'X-Mahoraga-Agent-Id': deps.agentId,
      },
      body: JSON.stringify({
        jsonrpc: '2.0',
        id: frame.id,
