...
import { createMahoragaFetch } from '../../mahoraga-fetch'
...
function createMahoragaModel(config: ResolvedLLMConfig): LanguageModel {
  if (!config.baseUrl) throw new Error('Mahoraga provider requires baseUrl')
  const { baseUrl, apiKey, model, upstreamProvider, mahoragaId } = config
...
const PROVIDER_FACTORIES: Record<string, ProviderFactory> = {
  ...
  [LLM_PROVIDERS.MAHORAGA]: createMahoragaModel,
...
export function createLLMProvider(config: ResolvedLLMConfig): LanguageModel {
  if (shouldUseMockMahoragaLLM(config)) {
    return createMockMahoragaLanguageModel()
  }
