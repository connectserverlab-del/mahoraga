...
import { OAUTH_CALLBACK_PORT } from '@mahoraga/shared/constants/ports'
...
export class OAuthTokenManager {
  ...
  async handleCallback(
    ...
    this.store.upsertTokens(this.mahoragaId, flow.provider, tokens)
    this.pendingFlows.delete(state)
    ...
    while (Date.now() < deadline) {
      ...
      try {
        ...
        if (data.access_token) {
          ...
          this.store.upsertTokens(this.mahoragaId, providerId, tokens)
  ...
  async refreshIfExpired(provider: string): Promise<StoredOAuthTokens | null> {
    const tokens = this.store.getTokens(this.mahoragaId, provider)
  ...
  private async executeRefresh(
    ...
    if (!tokens.refreshToken) {
      this.store.deleteTokens(this.mahoragaId, provider)
      throw new Error(
    ...
    this.store.upsertTokens(this.mahoragaId, provider, refreshed)
    return refreshed
  }
  ...
  storeTokens(
    ...
    this.store.upsertTokens(this.mahoragaId, provider, tokens)
  }

  getTokens(provider: string): StoredOAuthTokens | null {
    return this.store.getTokens(this.mahoragaId, provider)
  }

  getStatus(provider: string) {
    return this.store.getStatus(this.mahoragaId, provider)
  }
