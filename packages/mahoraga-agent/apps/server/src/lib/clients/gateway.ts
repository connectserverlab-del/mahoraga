...
export interface MahoragaConfig {
  providers: Provider[]
}
...
export async function fetchMahoragaConfig(
  configUrl: string,
  mahoragaId?: string,
): Promise<MahoragaConfig> {
  logger.debug('Fetching Mahoraga config', { configUrl, mahoragaId })
  ...
  if (mahoragaId) {
    headers['X-Mahoraga-ID'] = mahoragaId
  }
  ...
  try {
    ...
    const config = (await response.json()) as MahoragaConfig
    ...
    logger.info('✅ Mahoraga config fetched', {
    ...
    logger.error('❌ Failed to fetch Mahoraga config', {
      configUrl,
...
/**
 * Get LLM config from a provider in the Mahoraga config
 * @param config - Mahoraga config containing providers
...
export async function fetchCredits(
  ...
  const url = new URL(`/credits/${mahoragaId}`, gatewayBaseUrl).href
