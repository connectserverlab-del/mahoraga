...
import { EXTERNAL_URLS } from '@mahoraga/shared/constants/urls'
import { LLM_PROVIDERS } from '@mahoraga/shared/schemas/llm'
...
import { getMahoragaDir } from '../lib/mahoraga-dir'
import { createMahoragaFetch } from '../lib/mahoraga-fetch'
import {
  createMockMahoragaLanguageModel,
...
function defaultAcpWorkspacePath(
  ...
  const base = join(getMahoragaDir(), 'workspaces', providerType)
 ...
 * `$HOME/mahoraga-workspaces/...` as a placeholder because the
...
async function createAcpLanguageModel(
  ...
  // shipped binary resolves; otherwise the Mahoraga-pinned
  ...
  // acpx's built-in registry. This keeps Mahoraga in control of the
 ...
 * defaultMode: "dontAsk"`), which silently auto-denies the Mahoraga MCP
...
function createMahoragaFactory(
  config: ResolvedAgentConfig,
): (modelId: string) => unknown {
  if (!config.baseUrl) throw new Error('Mahoraga provider requires baseUrl')
  const { baseUrl, apiKey, upstreamProvider, mahoragaId } = config
  const mahoragaFetch = mahoragaId
    ? createMahoragaFetch(mahoragaId)
  ...
  logger.debug('Creating OpenAI-compatible provider for Mahoraga')
  return createOpenAICompatible({
    name: 'mahoraga',
    baseURL: baseUrl,
    ...(apiKey && { apiKey }),
...
const PROVIDER_FACTORIES: Record<string, ProviderFactory> = {
  ...
  [LLM_PROVIDERS.MAHORAGA]: createMahoragaFactory,
...
export async function createLanguageModel(
  ...
  if (shouldUseMockMahoragaLLM(config)) {
    return { model: createMockMahoragaLanguageModel() }
  }
