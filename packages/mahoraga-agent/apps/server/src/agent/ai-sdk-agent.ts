...
import { AGENT_LIMITS } from '@mahoraga/shared/constants/limits'
import type { BrowserContext } from '@mahoraga/shared/schemas/browser-context'
...
export class AiSdkAgent {
  ...
  static async create(config: AiSdkAgentConfig): Promise<AiSdkAgent> {
    ...
    // this branch. The spawned agent dials Mahoraga's own /mcp route
...
function withoutReservedBrowserToolNames(
  ...
  if (skipped.length > 0) {
    logger.warn(
      'External MCP tools skipped due to Mahoraga tool name collision',
      {
        toolNames: skipped,
      },
    )
