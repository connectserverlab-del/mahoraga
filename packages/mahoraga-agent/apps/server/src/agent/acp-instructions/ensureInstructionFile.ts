 ...
 *
 * Plants and refreshes the Mahoraga-managed instruction file inside
...
export interface EnsureInstructionFileOptions {
   ...
   * `.mahoraga-tmp` sibling and renames into place.
   */
...
async function defaultWriteFileAtomic(
  ...
  const fs = await import('node:fs/promises')
  const tmp = join(dirname(path), `.${path.split('/').pop()}.mahoraga-tmp`)
