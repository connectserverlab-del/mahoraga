...
import type { Browser } from '@mahoraga/browser-core/browser'
...
export interface ChatServiceDeps {
  ...
  /** Port the Mahoraga server bound to. Forwarded into the ACP MCP
   *  bridge so the spawned agent can dial back into /mcp. */
  serverPort: number
  /** Mahoraga resources directory. Threaded into ACP-backed config
...
export class ChatService {
  ...
  async processMessage(
    ...
    const llmConfig = await resolveLLMConfig(request, this.deps.mahoragaId)
    ...
    const agentConfig: ResolvedAgentConfig = {
      ...
      mahoragaId: this.deps.mahoragaId,
      ...
      if (!request.userWorkingDir) {
        contextChanges.push(
          [
            ...
            'filesystem_read can only read Mahoraga-generated output files returned in this session.',
