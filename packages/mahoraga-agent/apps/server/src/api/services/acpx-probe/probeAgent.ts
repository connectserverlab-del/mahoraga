...
export interface ServerAcpxProbeInput {
  ...
  timeoutMs?: number
  /**
   * Mahoraga resources directory. When set, the probe prefers the
...
function resolveTimeout(requested?: number): number {
  const envValue = Number(process.env.MAHORAGA_ACPX_PROBE_TIMEOUT_MS)
  if (
...
export async function probeAcpAgent(
  ...
  if (!command && agentId) {
    const launcher = resolveAcpSpawnCommand({
      agentType: agentId,
      mahoragaDir: input.mahoragaDir ?? getMahoragaDir(),
