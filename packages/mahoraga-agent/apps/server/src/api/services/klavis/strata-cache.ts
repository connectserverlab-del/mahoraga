...
function keyOf(mahoragaId: string, normalized: string): string {
...
export class KlavisStrataCache {
  ...
  async getOrFetch(
    ...
    const key = keyOf(mahoragaId, normalized)
    ...
    const inflight = this.fetch(client, mahoragaId, servers, normalized)
  ...
  invalidate(mahoragaId: string): void {
    const prefix = `${mahoragaId}|`
    let dropped = 0
  ...
  private async fetch(
    ...
    const result = await client.createStrata(mahoragaId, [...servers])
    return {
