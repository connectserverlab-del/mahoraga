...
export async function createHttpServer(config: HttpServerConfig) {
  const { port, host = '0.0.0.0', mahoragaId } = config
  const { onShutdown } = config

  const tokenManager = mahoragaId
    ? initializeOAuth(getDb(), mahoragaId)
    : null
  if (!mahoragaId) shutdownOAuth()

  const klavis = new KlavisService({ mahoragaId })
  klavis.start()
  ...
  const remoteHermes =
    mahoragaId && INLINED_ENV.AGENT_RUNNER_JWT_SECRET
  ...
  const app = createApiRoutes({
    ...
    gatewayBaseUrl: INLINED_ENV.MAHORAGA_CONFIG_URL
      ? new URL(INLINED_ENV.MAHORAGA_CONFIG_URL).origin
      : undefined,
    klavis,
    remoteHermes,
    tokenManager,
