...
/** Composes the Mahoraga HTTP API from the existing route factories. */
export function createApiRoutes(deps: CreateApiRoutesDeps) {
  ...
  return (
    new Hono<Env>()
      ...
      .route(
        '/test-provider',
        createProviderRoutes({ mahoragaId, resourcesDir }),
      )
      .route('/acpx/probe', createAcpxProbeRoutes({ resourcesDir }))
      .route('/refine-prompt', createRefinePromptRoutes({ mahoragaId }))
...
function protectedAgentRoutes(
  ...
  return new Hono<Env>().use('/*', requireTrustedAppOrigin()).route(
    ...
    routes ??
      createAgentRoutes({
        mahoragaServerPort: config.port,
