...
export const MANAGED_MCP_SERVERS_HEADER = 'X-Mahoraga-Managed-Mcp-Servers'
...
/** Creates the Hono routes that expose Mahoraga as a request-scoped MCP server. */
export function createMcpRoutes(deps: McpRouteDeps) {
  ...
  app.post('/', async (c) => {
    const scopeId = c.req.header('X-Mahoraga-Scope-Id') || 'ephemeral'
    metrics.log('mcp.request', { scopeId })
    const source = c.req.query('source') || undefined
    const includeStructuredContent = c.req.query('structured') === '1'

    const defaultWindowId = parseOptionalNumber(
      c.req.header('X-Mahoraga-Default-Window-Id'),
    )
    const defaultTabGroupId =
      c.req.header('X-Mahoraga-Default-Tab-Group-Id') ?? undefined
