 ...
 * with X-Mahoraga-Agent-Id + X-Mahoraga-Session-Id headers
 ...
 * architecturally; the Mahoraga-side substitutions are (1) using the
...
// at @mahoraga/browser-mcp/register re-type the same call signature
...
export const NUDGE_AGENT_ID_HEADER = 'X-Mahoraga-Agent-Id'
export const NUDGE_SESSION_ID_HEADER = 'X-Mahoraga-Session-Id'
...
const inputSchema = {
  appName: z
    ...
    .min(1)
    .describe(
      'The display name of the toolkit to connect, e.g. "Linear", "Gmail", "Slack". Match the casing of the Mahoraga catalog (proper-case, space-separated).',
    ),
  reason: z
    .string()
    .min(1)
...
export function createNudgeMcpRoute(deps: NudgeMcpRouteDeps) {
  ...
  return new Hono().post('/', async (c) => {
    ...
    const server = new McpServer(
      { name: 'mahoraga-nudge', version: '0.0.1' },
      {
        instructions:
        ...
        if (!agentId) {
          ...
          return {
            ...
            content: [
              {
                type: 'text' as const,
                text: 'Nudge tool called without an X-Mahoraga-Agent-Id header. The connect card cannot be attached to the current conversation.',
              },
            ],
          }
        }
