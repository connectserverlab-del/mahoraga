...
function normalizeTrustedOrigin(value: string): string | null {
  try {
    ...
    if (normalized === 'null' || url.host.length === 0) {
      logger.warn('Ignoring invalid MAHORAGA_TRUSTED_ORIGINS entry', {
        value,
      })
      return null
    }
    ...
    if (normalized !== value) {
      logger.warn('Normalized MAHORAGA_TRUSTED_ORIGINS entry to origin', {
        value,
        normalized,
      })
    }
    ...
    logger.warn('Ignoring invalid MAHORAGA_TRUSTED_ORIGINS entry', { value })
    return null
  }
}

function buildAllowedOrigins(): Set<string> {
  const fromEnv = (process.env.MAHORAGA_TRUSTED_ORIGINS ?? '')
    .split(',')
