import {
  ...
  type SidecarConfig,
} from '@mahoraga/shared/schemas/sidecar-config'
...
const ServerConfigSchema = z.object({
  ...
  instanceMahoragaVersion: z.string().optional(),
...
/** Loads Mahoraga server config from the sidecar JSON file passed by Chromium. */
...
function parseCliArgs(argv: string[]): ConfigResult<ParsedCliArgs> {
  ...
  try {
    program
      ...
      .description('Mahoraga Unified Server - MCP + Agent')
      .version(VERSION)
...
function projectServerConfig(
  ...
  const result = ServerConfigSchema.safeParse({
    ...
    instanceMahoragaVersion: sidecar.instance.mahoraga_version,
    instanceChromiumVersion: sidecar.instance.chromium_version,
    aiSdkDevtoolsEnabled: process.env.MAHORAGA_AI_SDK_DEVTOOLS === 'true',
  })
  if (!result.success) {
    return {
      ok: false,
