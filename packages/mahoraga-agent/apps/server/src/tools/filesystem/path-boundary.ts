...
import { getMahoragaDir, getToolOutputDir } from '../../lib/mahoraga-dir'
...
export async function isMahoragaStatePath(
  inputPath: string,
): Promise<boolean> {
  if (!isAbsoluteInput(inputPath)) return false

  const stateRoot = resolve(getMahoragaDir())
...
function assertAbsoluteMahoragaOutputInput(inputPath: string): void {
  if (!isAbsoluteInput(inputPath)) {
    throw new Error('Path must be an absolute Mahoraga tool output path.')
  }
}
...
/** Resolves a Mahoraga-generated output file without exposing sibling app state. */
export async function resolveBrowserToolOutputPath(
  inputPath: string,
): Promise<string> {
  assertAbsoluteMahoragaOutputInput(inputPath)
  const outputRoot = await getToolOutputDir()
  const candidate = resolve(inputPath)
  const canonical = await realpath(candidate)
  if (!isPathInside(outputRoot, canonical)) {
    throw new Error('Path is outside Mahoraga tool output.')
  }
  return canonical
}
