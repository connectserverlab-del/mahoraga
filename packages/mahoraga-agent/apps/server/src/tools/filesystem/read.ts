...
import { wrapUntrusted } from '@mahoraga/browser-mcp/tools/trust-boundary'
import { tool } from 'ai'
import { z } from 'zod'
import {
  executeWithMetrics,
...
const NO_WORKSPACE_READ_ERROR =
  'No workspace selected. filesystem_read can only read Mahoraga-generated tool output files by absolute path.'
...
function assertAllowedGeneratedOutputPath(
  ...
  if (!allowedOutputPaths.has(resolvedPath)) {
    throw new Error(
      'filesystem_read can only read Mahoraga-generated tool output files returned in this session.',
    )
  }
}
...
async function resolveGeneratedOutputPath(
  ...
  if (!(await isMahoragaStatePath(inputPath))) {
...
async function resolveReadPath(
  ...
  try {
    ...
    if (error instanceof Error && (await isMahoragaStatePath(inputPath))) {
...
export function createReadTool(cwd?: string, options: ReadToolOptions = {}) {
  ...
  return tool({
    description: cwd
      ? `Read a file from the filesystem. Returns text content with line numbers, or image data for image files. Text reads are limited to ${MAX_READ_LINES} lines and ${MAX_READ_CHARS} characters per call. Use offset and limit to paginate through large files.${supportsGeneratedOutputs ? ' Also accepts absolute Mahoraga-generated output file paths returned by browser tools.' : ''}`
      : `Read Mahoraga-generated tool output files by absolute path. Returns text content with line numbers, or image data for image files. Text reads are limited to ${MAX_READ_LINES} lines and ${MAX_READ_CHARS} characters per call. Use offset and limit to paginate through large files.`,
    inputSchema: z.object({
      path: z
        .string()
        .describe(
          cwd
            ? `File path relative to the selected workspace${supportsGeneratedOutputs ? ', or an absolute Mahoraga-generated output path returned by a browser tool' : ''}`
            : 'Absolute Mahoraga-generated tool output path returned by a browser tool',
        ),
      offset: z
        .number()
        .optional()
