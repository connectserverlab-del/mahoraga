...
func TestLoadMahoragaID(t *testing.T) {
	tmp := t.TempDir()
	t.Setenv("HOME", tmp)

	// No server.json → empty
	if got := loadMahoragaID(); got != "" {
		t.Errorf("loadMahoragaID() = %q, want empty", got)
	}

	// server.json without mahoraga_id → empty
	dir := filepath.Join(tmp, ".mahoraga")
	...
	if got := loadMahoragaID(); got != "" {
		t.Errorf("loadMahoragaID() = %q, want empty (no mahoraga_id field)", got)
	}

	// server.json with mahoraga_id → returns it
	data, _ = json.Marshal(map[string]any{
		"server_port":  9100,
		"url":          "http://127.0.0.1:9100",
		"mahoraga_id": "test-uuid-1234",
	})
	os.WriteFile(filepath.Join(dir, "server.json"), data, 0644)

	if got := loadMahoragaID(); got != "test-uuid-1234" {
		t.Errorf("loadMahoragaID() = %q, want %q", got, "test-uuid-1234")
	}
}

func TestLoadOrCreateInstallID(t *testing.T) {
	tmp := t.TempDir()
	configDir := filepath.Join(tmp, "mahoraga-cli")
...
func TestResolveDistinctID_PrefersMahoragaID(t *testing.T) {
	tmp := t.TempDir()
	t.Setenv("HOME", tmp)
	t.Setenv("XDG_CONFIG_HOME", tmp)

	// Write server.json with mahoraga_id
	dir := filepath.Join(tmp, ".mahoraga")
	os.MkdirAll(dir, 0755)
	data, _ := json.Marshal(map[string]any{"mahoraga_id": "server-uuid"})
	os.WriteFile(filepath.Join(dir, "server.json"), data, 0644)
