...
import (
	...
	"mahoraga-cli/analytics"
	"mahoraga-cli/cmd/raw"
	"mahoraga-cli/config"
	"mahoraga-cli/mcp"
	"mahoraga-cli/output"
	"mahoraga-cli/update"
...
var rootCmd = &cobra.Command{
	Use:           "mahoraga-cli",
	Short:         "Browser control CLI for Mahoraga",
	Long:          "mahoraga-cli — command-line interface for controlling Mahoraga via MCP",
	SilenceUsage:  true,
	SilenceErrors: true,
	Run:           runRoot,
}
...
func init() {
	...
	rootCmd.PersistentFlags().StringVarP(&serverURL, "server", "s", defaultServerURL(), "Mahoraga server URL")
...
func explicitPageID(changed bool, page int) (int, error) {
	...
	return 0, fmt.Errorf("page id is required: pass -p/--page <id> from `mahoraga-cli open --json | jq -r .page` or `mahoraga-cli tabs --json`")
}
...
//
// Mahoraga writes a discovery file at runtime, but normal commands intentionally
// ignore it so a saved URL is not silently overridden by another running server.
func defaultServerURL() string {
	if env := normalizeServerURL(os.Getenv("MAHORAGA_URL")); env != "" {
		return env
	}
...
func validateServerURL(raw string) (string, error) {
	...
	return "", fmt.Errorf(
		"Mahoraga server URL is not configured.\n\n" +
			"  Open Mahoraga Settings > Mahoraga MCP and copy the Server URL.\n" +
			"  Save it with:       mahoraga-cli init <Server URL>\n" +
			"  Example:            mahoraga-cli init http://127.0.0.1:9000/mcp\n" +
			"  If Mahoraga is closed:  mahoraga-cli launch\n" +
			"  If not installed:        download from https://mahoraga.com",
	)
}
