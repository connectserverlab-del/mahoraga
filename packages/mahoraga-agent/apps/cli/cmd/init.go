...
func init() {
	cmd := &cobra.Command{
		Use:   "init [url]",
		Short: "Configure the Mahoraga server connection",
		Long: `Set up the CLI by providing the MCP server URL from Mahoraga.

Open Mahoraga → Settings → Mahoraga MCP to find your Server URL.
...
You can provide the full URL or just the port number:
  ...
  mahoraga-cli init 9000

Modes:
  mahoraga-cli init <url>    Non-interactive (full URL or port number)
  mahoraga-cli init          Interactive prompt`,
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			default:
				fmt.Println()
				bold.Println("Mahoraga CLI Setup")
				fmt.Println()
				fmt.Println("Open Mahoraga → Settings → Mahoraga MCP")
			...
			fmt.Println()
			dim.Println("Try: mahoraga-cli health")
			dim.Println("     mahoraga-cli tabs")
		},
	}
