...
func TestCommandName(t *testing.T) {
	tests := []struct {
		...
		{"known command", []string{"health"}, "mahoraga-cli health"},
		{"diff command", []string{"diff"}, "mahoraga-cli diff"},
		{"unknown command", []string{"nonexistent"}, "unknown"},
		{"subcommand", []string{"bookmark", "search"}, "mahoraga-cli bookmark search"},
		{"strata subcommand", []string{"strata", "check"}, "mahoraga-cli strata check"},
		{"known with extra args", []string{"snap", "extra"}, "mahoraga-cli snapshot"},
	}
...
func TestNpmPackageExposesBosAlias(t *testing.T) {
	...
	if pkg.Bin["mahoraga-cli"] != "bin/mahoraga-cli.js" {
		t.Fatalf("mahoraga-cli bin = %q", pkg.Bin["mahoraga-cli"])
	}
	if pkg.Bin["bos"] != "bin/mahoraga-cli.js" {
...
func TestInstallScriptsCreateBosAlias(t *testing.T) {
	...
	if !strings.Contains(string(shellScript), `"${INSTALL_DIR}/bos"`) {
		t.Fatal("install.sh does not create bos next to mahoraga-cli")
	}
	...
	if !strings.Contains(string(powerShell), `"bos.exe"`) {
		t.Fatal("install.ps1 does not create bos.exe next to mahoraga-cli.exe")
	}
}
...
func TestDefaultServerURLUsesEnvBeforeConfig(t *testing.T) {
	...
	t.Setenv("MAHORAGA_URL", "http://127.0.0.1:9115/mcp")
...
func TestDefaultServerURLIgnoresMahoragaServerJSON(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	...
	serverDir := filepath.Join(home, ".mahoraga")
...
func TestValidateServerURLExplainsManualInit(t *testing.T) {
	...
	msg := err.Error()
	if !strings.Contains(msg, "mahoraga-cli init <Server URL>") {
