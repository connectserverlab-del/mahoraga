...
func TestMain(m *testing.M) {
	serverURL = os.Getenv("MAHORAGA_URL")
	if serverURL == "" {
	...
	tmpDir, err := os.MkdirTemp("", "mahoraga-cli-test-*")
	if err != nil {
		fmt.Fprintf(os.Stderr, "Failed to create temp dir: %v\n", err)
		os.Exit(1)
	}

	cliBinary = filepath.Join(tmpDir, "mahoraga-cli")
...
func TestVersion(t *testing.T) {
	...
	if !strings.Contains(r.Stdout, "mahoraga-cli") {
		t.Errorf("expected version output to contain 'mahoraga-cli', got: %s", r.Stdout)
	}
}
...
func TestPageLifecycle(t *testing.T) {
	...
	openData := runJSON(t, "open", fixtureURL(`<title>Example Fixture</title><main><h1>Example</h1><p>Hello from Mahoraga.</p><button>Submit</button></main>`))
