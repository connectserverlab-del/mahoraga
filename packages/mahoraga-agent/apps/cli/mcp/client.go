...
func (c *Client) connect(ctx context.Context) (*sdkmcp.ClientSession, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("cannot connect to Mahoraga at %s: %w%s", c.BaseURL, err, connectionSetupInstructions())
	}
	return session, nil
}
...
// Health checks Mahoraga-compatible REST health endpoints.
...
func connectionSetupInstructions() string {
	return "\n\n" +
		...
		"  Run once with:      mahoraga-cli --server <Server URL> health\n" +
