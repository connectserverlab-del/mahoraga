#!/usr/bin/env bash
#
# Install mahoraga-cli — downloads the latest release binary for your platform.
#
# Usage:
#   curl -fsSL https://cdn.mahoraga.com/cli/install.sh | bash
#
#   # Or with options:
#   curl -fsSL https://cdn.mahoraga.com/cli/install.sh | bash -s -- --version 0.1.0 --dir /usr/local/bin

set -euo pipefail

CDN_BASE="https://cdn.mahoraga.com/cli"
BINARY="mahoraga-cli"
INSTALL_DIR="${HOME}/.mahoraga/bin"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --help)
      ...
      echo "  --dir       Install directory (default: ~/.mahoraga/bin)"
      exit 0
      ;;
...
echo "Installing mahoraga-cli v${VERSION}..."
...
if ! echo "$PATH" | tr ':' '\n' | grep -qx "$INSTALL_DIR"; then
  echo ""
  echo "Add mahoraga-cli to your PATH:"
  echo ""
...
echo ""
echo "Run 'mahoraga-cli --help' or 'bos --help' to get started."
