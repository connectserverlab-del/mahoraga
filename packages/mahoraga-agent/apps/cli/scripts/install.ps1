#
# Install mahoraga-cli for Windows — downloads the latest release binary.
#
# Usage (PowerShell — save and run):
#   Invoke-WebRequest -Uri "https://cdn.mahoraga.com/cli/install.ps1" -OutFile install.ps1
#   .\install.ps1
#   .\install.ps1 -Version "0.1.0" -Dir "C:\tools\mahoraga"
#
# Usage (one-liner, uses env vars for options):
#   & { $env:MAHORAGA_VERSION="0.1.0"; irm https://cdn.mahoraga.com/cli/install.ps1 | iex }
#
...
$CdnBase = "https://cdn.mahoraga.com/cli"
$Binary = "mahoraga-cli"

# When piped via irm | iex, param() is ignored — fall back to env vars
if (-not $Version) { $Version = $env:MAHORAGA_VERSION }
if (-not $Dir) { $Dir = if ($env:MAHORAGA_DIR) { $env:MAHORAGA_DIR } else { "$env:LOCALAPPDATA\mahoraga-cli\bin" } }
...
Write-Host "Installing mahoraga-cli v$Version..."
...
$TmpDir = Join-Path ([System.IO.Path]::GetTempPath()) ("mahoraga-cli-install-" + [System.IO.Path]::GetRandomFileName())
...
Write-Host ""
Write-Host "Run 'mahoraga-cli --help' or 'bos --help' to get started."
