...
const (
	DefaultManifestURL     = "https://cdn.mahoraga.com/cli/latest/manifest.json"
	DefaultCheckTTL        = 24 * time.Hour
	DefaultHTTPTimeout     = 2 * time.Second
	DefaultDownloadTimeout = 5 * time.Minute
	SkipCheckEnv           = "MAHORAGA_SKIP_UPDATE_CHECK"
	InstallMethodEnv       = "MAHORAGA_INSTALL_METHOD"
)
...
func (m *Manager) Apply(ctx context.Context, result *CheckResult) error {
	if result == nil || !result.UpdateAvailable || result.Asset == nil {
		return fmt.Errorf("mahoraga-cli is already up to date")
	}
...
func FormatNotice(currentVersion, latestVersion string) string {
	notice := fmt.Sprintf(
		"Update available: mahoraga-cli v%s (current v%s)",
		latestVersion,
		currentVersion,
	)
	...
	case "npm":
		notice += "\nRun `npm update -g mahoraga-cli` to upgrade."
	case "brew", "homebrew":
		notice += "\nRun `brew upgrade mahoraga-cli` to upgrade."
	default:
		notice += "\nRun `mahoraga-cli update` to upgrade."
	}
