...
func TestManagerCheckNow(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{
			...
			"tag":"mahoraga-cli-v9.9.9",
			"assets":{
				"` + runtimePlatformKey(t) + `":{
					"filename":"mahoraga-cli_9.9.9_test.tar.gz",
					"url":"https://cdn.example.com/cli/v9.9.9/mahoraga-cli_9.9.9_test.tar.gz",
...
func TestAutomaticEnabledSkipsForPackageManagerInstall(t *testing.T) {
	t.Setenv("MAHORAGA_INSTALL_METHOD", "npm")
	...
	if manager.AutomaticEnabled() {
		t.Fatal("AutomaticEnabled() = true, want false when MAHORAGA_INSTALL_METHOD=npm")
	}
}

func TestAutomaticEnabledAllowsNormalInstall(t *testing.T) {
	t.Setenv("MAHORAGA_INSTALL_METHOD", "")
	...
	if !manager.AutomaticEnabled() {
		t.Fatal("AutomaticEnabled() = false, want true when MAHORAGA_INSTALL_METHOD is empty")
	}
}
