...
describe('MOCK_MAHORAGA_IMPORT_SOURCES fixture', () => {
  it('ships mock import sources with stable ids', () => {
    expect(MOCK_MAHORAGA_IMPORT_SOURCES.map((source) => source.id)).toEqual([
      'chrome-work',
      'chrome-personal',
      'edge-default',
    ])
    expect(DEFAULT_MAHORAGA_IMPORT_SOURCE_ID).toBe('chrome-work')
  })

  it('uses the Chromium contract source shape', () => {
    for (const source of MOCK_MAHORAGA_IMPORT_SOURCES) {
...
describe('source selection helpers', () => {
  it('finds the selected source by contract id', () => {
    expect(
      selectedSourceById(MOCK_MAHORAGA_IMPORT_SOURCES, 'chrome-personal')
        ?.displayName,
  ...
  it('selects the first source with default items when the current source vanished', () => {
    expect(
      ...
      selectedSourceId: MOCK_MAHORAGA_IMPORT_SOURCES[0].id,
  ...
  it('falls back to supported items when recommended items are empty', () => {
    expect(
      selectableItemsForSource({
        ...MOCK_MAHORAGA_IMPORT_SOURCES[0],
        recommendedItems: [],
      }),
    ).toEqual(MOCK_MAHORAGA_IMPORT_SOURCES[0].supportedItems)
  })

  it('sanitizes explicit item selections against supported items in source order', () => {
    expect(
      sanitizeImportSelection(MOCK_MAHORAGA_IMPORT_SOURCES[0], [
        'extensions',
        'history',
        'bookmarks',
        'history',
  ...
  it('filters unsupported selected items and returns empty for no matches', () => {
    expect(
      sanitizeImportSelection(MOCK_MAHORAGA_IMPORT_SOURCES[2], [
        'extensions',
        'autofill',
        'history',
      ]),
  ...
  it('builds the Chromium start-import request for one source', () => {
    expect(startImportRequestFor(MOCK_MAHORAGA_IMPORT_SOURCES[0])).toEqual({
  ...
  it('builds the Chromium start-import request for a sanitized subset', () => {
    expect(
      startImportRequestFor(MOCK_MAHORAGA_IMPORT_SOURCES[0], [
        'extensions',
        'history',
      ]),
    ).toEqual({
  ...
  it('does not build a start-import request for empty explicit selections', () => {
    ...
    expect(
      startImportRequestFor(MOCK_MAHORAGA_IMPORT_SOURCES[2], ['extensions']),
    ).toBeNull()
  })
