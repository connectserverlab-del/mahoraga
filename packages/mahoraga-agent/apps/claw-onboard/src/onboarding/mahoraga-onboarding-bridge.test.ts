...
import type { MahoragaOnboardingState } from './mahoraga-onboarding-api'
import { MahoragaOnboardingMessage } from './mahoraga-onboarding-api'
import { createMahoragaOnboardingBridge } from './mahoraga-onboarding-bridge'
...
describe('createMahoragaOnboardingBridge', () => {
  it('sends Chromium messages through the real chrome bridge', () => {
    ...
    const bridge = createMahoragaOnboardingBridge()
    ...
    expect(sent).toEqual([
      [MahoragaOnboardingMessage.PAGE_READY, undefined],
      [MahoragaOnboardingMessage.REFRESH_SOURCES, undefined],
      [
        MahoragaOnboardingMessage.START_IMPORT,
        [{ sourceId: 'source-0', items: ['history'] }],
      ],
      [MahoragaOnboardingMessage.COMPLETE, undefined],
    ])
  })
  ...
  it('does not send an empty explicit import item request', () => {
    ...
    createMahoragaOnboardingBridge().startImport({
      sourceId: 'source-0',
      items: [],
    })
  ...
  it('installs and cleans up the window state receiver it owns', () => {
    installWindow()
    const states: MahoragaOnboardingState[] = []
    const bridge = createMahoragaOnboardingBridge({ chrome: null })
    const cleanup = bridge.registerReceiver((state) => states.push(state))

    expect(window.mahoragaOnboarding).toBeDefined()
    window.mahoragaOnboarding?.receiveState({
      apiVersion: 1,
      status: 'ready',
      sources: [],
    })
    ...
    expect(window.mahoragaOnboarding).toBeUndefined()
  })
  ...
  it('emits mock ready and import success states without chrome.send', () => {
    ...
    bridge.pageReady()
    bridge.startImport({
      sourceId: MOCK_MAHORAGA_IMPORT_SOURCES[0].id,
      items: MOCK_MAHORAGA_IMPORT_SOURCES[0].recommendedItems,
    })
    ...
    expect(states.at(-1)?.progress?.completedItems).toEqual(
      MOCK_MAHORAGA_IMPORT_SOURCES[0].recommendedItems,
    )
    expect(states[2]?.progress).toMatchObject({
      currentSourceId: MOCK_MAHORAGA_IMPORT_SOURCES[0].id,
      currentSourceName: MOCK_MAHORAGA_IMPORT_SOURCES[0].displayName,
      completedSources: 0,
      totalSources: 1,
    })
    expect(states[2]?.results).toEqual([
      {
        sourceId: MOCK_MAHORAGA_IMPORT_SOURCES[0].id,
        displayName: MOCK_MAHORAGA_IMPORT_SOURCES[0].displayName,
        status: 'importing',
      },
    ])
