...
import {
  ...
  type MahoragaImportStatus,
  type MahoragaOnboardingState,
} from './mahoraga-onboarding-api'
import {
  type MahoragaOnboardingBridge,
  createMahoragaOnboardingBridge,
} from './mahoraga-onboarding-bridge'
...
const TOTAL_STEPS = 3
const MAHORAGA_MCP_PAGE_URL = 'chrome://newtab/#/mcp'

const initialOnboardingState: MahoragaOnboardingState = {
...
export function importPhaseFor(status: MahoragaImportStatus): ImportPhase {
...
export function openMahoragaMcpPage() {
  window.location.assign(MAHORAGA_MCP_PAGE_URL)
}

/** Completes onboarding and leaves standalone mock onboarding when needed. */
export function finishMahoragaOnboarding(bridge: MahoragaOnboardingBridge) {
  bridge.complete()
  if (bridge.isMock) openMahoragaMcpPage()
}
...
export function OnboardingV2() {
  ...
  const [bridge] = useState(() => createMahoragaOnboardingBridge())
  const [onboardingState, setOnboardingState] =
    useState<MahoragaOnboardingState>(initialOnboardingState)
