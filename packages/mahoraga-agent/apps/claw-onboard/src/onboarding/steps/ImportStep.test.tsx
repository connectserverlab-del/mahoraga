...
import { MAHORAGA_ONBOARDING_API_VERSION } from '../mahoraga-onboarding-api'
import {
  importItemLabel,
...
function readyState(
  overrides: Partial<MahoragaOnboardingState> = {},
...
function render(
  phase: ImportPhase,
  state: MahoragaOnboardingState = readyState(),
  formValues: Partial<OnboardingFormValues> = {},
  unpicked = false,
): string {
...
describe('ImportStep', () => {
  it('renders the picker, the Keychain notice, and an Import button in picker phase', () => {
    ...
    for (const item of MOCK_MAHORAGA_IMPORT_SOURCES[0].supportedItems) {
  ...
  it('checks recommended items and leaves non-recommended supported items unchecked', () => {
    const source = MOCK_MAHORAGA_IMPORT_SOURCES[1]
  ...
  it('renders the success card and continue CTA in imported phase', () => {
    const html = render(
      ...
      readyState({
        status: 'succeeded',
        progress: {
          completedItems: MOCK_MAHORAGA_IMPORT_SOURCES[0].recommendedItems,
          totalItems: 7,
          completedSources: 1,
          totalSources: 1,
        },
