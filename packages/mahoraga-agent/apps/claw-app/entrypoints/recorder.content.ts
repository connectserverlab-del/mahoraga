 ...
 *
 * window.__mahoragaClawReplayInstalled is a re-injection guard
...
export default defineContentScript({
  ...
  main() {
    type Marked = typeof window & { __mahoragaClawReplayInstalled?: boolean }
    if ((window as Marked).__mahoragaClawReplayInstalled) return
    ;(window as Marked).__mahoragaClawReplayInstalled = true
...
function startRecorder(config: RecorderConfig): ActiveRecorder {
    ...
    try {
      void chrome.runtime
        ...
        .catch((err) => {
          ...
          console.warn(
            '[mahoraga-claw replay] sendMessage to background failed',
            err,
          )
        })
    } catch (err) {
      // eslint-disable-next-line no-console
      console.warn('[mahoraga-claw replay] send threw', err)
    }
  }
  ...
  try {
    ...
    // eslint-disable-next-line no-console
    console.info('[mahoraga-claw replay] recorder online', {
      sessionId,
      tabPageId,
    })
  } catch (err) {
    // eslint-disable-next-line no-console
    console.warn('[mahoraga-claw replay] rrweb.record threw', err)
    return {
      config,
      flushNow: () => {},
      stop: () => {},
