...
import { resolveMahoragaServerBaseUrl } from '@/modules/api/mahoraga-ports'
import {
...
export default defineBackground(() => {
  ...
  async function poll(): Promise<void> {
    let resp: Response
    try {
      const cockpitOrigin = await resolveMahoragaServerBaseUrl()
    ...
    for (const entry of diff.changed) {
      ...
      try {
        ...
        console.warn('[mahoraga-claw replay] restart message failed', {
    ...
    for (const entry of diff.added) {
      ...
      try {
        ...
        // eslint-disable-next-line no-console
        console.warn('[mahoraga-claw replay] inject failed', {
          tabId: entry.chromeTabId,
          error: err instanceof Error ? err.message : String(err),
        })
      }
  ...
  chrome.webNavigation.onCommitted.addListener(async (details) => {
    ...
    try {
      ...
      // eslint-disable-next-line no-console
      console.warn('[mahoraga-claw replay] reinject failed', {
        tabId: details.tabId,
        error: err instanceof Error ? err.message : String(err),
      })
    }
  ...
  async function postRecorderEvents(msg: RecorderMessage): Promise<void> {
    ...
    try {
      ...
      // eslint-disable-next-line no-console
      console.warn('[mahoraga-claw replay] events POST failed', {
