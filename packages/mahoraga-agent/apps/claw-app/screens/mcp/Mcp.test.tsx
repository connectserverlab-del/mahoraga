...
function getConnectionsHookResult() {
  return (
    connectionsHookState()[connectionsHookResultKey] ?? {
      data: {
        connections: mcpMahoragaConnections,
      },
      isPending: false,
      isError: false,
    }
...
mock.module('@/modules/api/connections.hooks', () => ({
  ..._connectionsHooks,
  useMahoragaConnections: Object.assign(() => getConnectionsHookResult(), {
...
describe('Mcp (editorial)', () => {
  ...
  it('does NOT render the removed CLI snippet block', () => {
    ...
    // registered server name (`mahoraga` legacy or `BrowserClaw`
    // post-rename).
  ...
  it('does NOT render the removed floating footer paragraph', () => {
    const html = renderApp()
    expect(html).not.toContain('Hermes and OpenClaw run inside Mahoraga')
  })
  ...
  it('does NOT render the removed marketing subtitle from the old HeroCard', () => {
    ...
    expect(html).not.toContain(
      'Add Mahoraga as an MCP server in your AI agent',
    )
