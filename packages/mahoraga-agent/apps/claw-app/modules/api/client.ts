 ...
 * @mahoraga/claw-server/server. That export field has
 ...
 *   1. Mahoraga `mahoraga.server.server_port` pref
 *   2. ?apiUrl=… on window.location (dev launcher publishes this)
 *   3. sessionStorage cache of (2)
 *   4. VITE_MAHORAGA_CLAW_API_URL from the dev watcher
 *   5. standalone BrowserClaw port on 127.0.0.1
 *
 * The lazy Proxy awaits the base URL at the terminal `$get`/`$post`
 * call so Mahoraga's callback-based pref API can feed Hono without
 * changing call sites.
 */

import type { AppType } from '@mahoraga/claw-server/server'
...
export async function resolveApiBaseUrl(): Promise<string> {
  return resolveMahoragaServerBaseUrl(apiBaseUrlSourcesFromWindow())
}
...
/** Creates a Hono-compatible proxy that awaits Mahoraga port resolution at request time. */
