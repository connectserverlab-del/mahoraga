...
function installMahoragaPrefs(values: Record<string, unknown>) {
  Object.defineProperty(globalThis, 'chrome', {
    ...
    value: {
      ...
      mahoraga: {
        getPref(
          name: string,
          callback: (pref: chrome.mahoraga.PrefObject) => void,
        ) {
          callback({
            key: name,
...
describe('Mahoraga managed port resolution', () => {
  it('prefers the Mahoraga server port pref for API traffic', async () => {
    installMahoragaPrefs({ 'mahoraga.server.server_port': 9511 })
  ...
  it('prefers the Mahoraga proxy port pref for MCP traffic', async () => {
    installMahoragaPrefs({ 'mahoraga.server.proxy_port': 9512 })
  ...
  it('keeps a valid proxy port pref when startup health is not ready yet', async () => {
    installMahoragaPrefs({ 'mahoraga.server.proxy_port': 9000 })
    const requests = installFetchRecorder({
  ...
  it('routes Hono API calls through the Mahoraga server port pref', async () => {
    installMahoragaPrefs({ 'mahoraga.server.server_port': 9511 })
    const requests = installFetchRecorder()
