...
async fn run_connections_case() -> anyhow::Result<()> {
    ...
    let not_installed = service
        .connect_mahoraga(Harness::Antigravity, MCP_URL)
        .await?;
    ...
    let initial = service.list_mahoraga_connections().await?;
    ...
    let claude = service
        .connect_mahoraga(Harness::ClaudeCode, MCP_URL)
        .await?;
    ...
    assert_eq!(
        claude.message,
        "Mahoraga registered as an MCP server in Claude Code."
    );

    let codex = service.connect_mahoraga(Harness::Codex, MCP_URL).await?;
    let zed = service.connect_mahoraga(Harness::Zed, MCP_URL).await?;
    ...
    let configured = service.list_mahoraga_connections().await?;
    ...
    let disconnected = service.disconnect_mahoraga(Harness::Codex).await?;
    assert!(!disconnected.installed);
    assert_eq!(disconnected.message, "Mahoraga unregistered from Codex.");
    assert!(!fs::read_to_string(path_for(&paths, AgentId::Codex)?)?.contains("BrowserClaw"));
    let after_disconnect = service.list_mahoraga_connections().await?;
...
async fn assert_legacy_manifest_migration(
    ...
    let listed = service.list_mahoraga_connections().await?;
    let claude = listed
        .iter()
