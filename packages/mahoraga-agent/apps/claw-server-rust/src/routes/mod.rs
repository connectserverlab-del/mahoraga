...
async fn connections_list(State(state): State<AppState>) -> AppResult<Json<Value>> {
    Ok(Json(json!({
        "connections": state.harness.list_mahoraga_connections().await?
    })))
}
...
async fn connections_connect(
    ...
    let result = state
        .harness
        .connect_mahoraga(harness, &state.config.public_mcp_url())
        .await?;
...
async fn connections_disconnect(
    ...
    let harness = Harness::from_str(&harness)?;
    let result = state.harness.disconnect_mahoraga(harness).await?;
    Ok(Json(serde_json::to_value(result)?))
}
