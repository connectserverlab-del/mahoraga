...
use mahoraga_cdp::{CdpClient, ConnectOptions, ReconnectPolicy};
use mahoraga_core::{
...
pub struct BrowserService {
    ...
    session: Arc<RwLock<Option<Arc<mahoraga_core::BrowserSession>>>>,
...
impl BrowserService {
    ...
    pub async fn session(&self) -> Option<Arc<mahoraga_core::BrowserSession>> {
        self.session.read().await.clone()
    }

    #[doc(hidden)]
    pub async fn connect_once_for_testing(&self) -> Result<(), mahoraga_cdp::CdpError> {
            ...
            match CdpClient::connect(opts).await {
                Ok(client) => {
                    ...
                    *self.session.write().await = Some(session);
                    ...
                    debug!(epoch, "connected to Mahoraga CDP");
