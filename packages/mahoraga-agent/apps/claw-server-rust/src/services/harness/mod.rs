...
pub const MAHORAGA_MCP_SERVER_NAME: &str = "BrowserClaw";
...
impl HarnessService {
        ...
        match result {
            Ok(config_path) => {
                ...
                Ok(ConnectionState {
                    ...
                    message: format!("Mahoraga registered as an MCP server in {harness}."),
                })
            }
    ...
    pub async fn disconnect_mahoraga(&self, harness: Harness) -> AppResult<ConnectionState> {
        let _guard = self.mutex.lock().await;
        let agent = harness.agent_id();
        let manager = self.manager.clone();
        let workspace_dir = self.workspace_dir.clone();
        let result = tokio::task::spawn_blocking(move || {
            with_legacy_manifest_migration(&workspace_dir, || {
                manager.disconnect(DisconnectInput::new(MAHORAGA_MCP_SERVER_NAME, agent))
            })
        ...
        match result {
            Ok(summary) => {
                ...
                Ok(ConnectionState {
                    ...
                    agent_id: agent,
                    config_path: None,
                    message: format!("Mahoraga unregistered from {harness}."),
                })
            }
    ...
    pub async fn list_mahoraga_connections(&self) -> AppResult<Vec<ConnectionState>> {
        let _guard = self.mutex.lock().await;
        let manager = self.manager.clone();
        let workspace_dir = self.workspace_dir.clone();
        ...
        let (links_result, installed_result) = tokio::task::spawn_blocking(move || {
            let links = with_legacy_manifest_migration(&workspace_dir, || {
                manager.list_links(ListLinksFilter {
                    server_names: Some(vec![MAHORAGA_MCP_SERVER_NAME.to_string()]),
                    agents: None,
                })
            });
