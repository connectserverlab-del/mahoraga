 ...
 * `VITE_MAHORAGA_CLAW_API_URL` when dev-watch selects a random port.
 */
...
import { bootstrapMahoragaBrowser } from './lib/browser-bootstrap'
...
async function start(): Promise<void> {
  ...
  // Attach to the Mahoraga Chromium so MCP `tools/call` dispatches
  // hit a real browser. The bootstrap soft-fails when Mahoraga is
  ...
  // wire shape until the user restarts the cockpit with Mahoraga
  // up. Reattach on transient drops is the CdpBackend's job (we
  // pass `exitOnReconnectFailure: false` so it does not kill the
  // process).
  const bootstrap = await bootstrapMahoragaBrowser()
  if (bootstrap) {
    setBrowserSession(bootstrap.session)
    logger.info('cockpit attached to mahoraga browser', {
      cdpPort: env.cdpPort,
    })
    ...
    // No browser attached (Mahoraga not reachable): still flush
