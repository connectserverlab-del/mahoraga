...
import { MAHORAGA_MCP_SERVER_NAME, publicMcpUrl } from '../shared/mcp-url'
...
export async function connectMahoragaToHarness(
  harness: Harness,
  ...
  try {
    ...
    logger.info('connected mahoraga to harness', {
      harness,
      agent: agentId,
    ...
    return {
      ...
      message: `Mahoraga registered as an MCP server in ${harness}.`,
    }
  } catch (err) {
...
export async function disconnectMahoragaFromHarness(
  harness: Harness,
): Promise<ConnectionState> {
  const agentId = HARNESS_TO_AGENT_ID[harness]
  const mgr = getMcpManager()
  try {
    ...
    logger.info('disconnected mahoraga from harness', {
      harness,
      agent: agentId,
    ...
    return {
      ...
      installed: false,
      agentId,
      message: `Mahoraga unregistered from ${harness}.`,
    }
  } catch (err) {
 ...
 */
export async function listMahoragaConnections(): Promise<ConnectionState[]> {
  const mgr = getMcpManager()
  ...
  try {
    links = await mgr.listLinks({
      serverNames: [MAHORAGA_MCP_SERVER_NAME],
    })
  } catch (err) {
    logger.warn('listMahoragaConnections listLinks failed', {
  ...
  try {
    ...
    logger.warn('listMahoragaConnections isInstalled failed', {
      error: err instanceof Error ? err.message : String(err),
    })
...
function failure(
  ...
  if (err instanceof AgentNotInstalledError) {
    logger.info('mahoraga connect target not installed', {
      harness,
      agent: err.agent,
  ...
  if (err instanceof ForeignEntryError) {
    logger.warn('mahoraga harness entry exists but was not written by us', {
      harness,
  ...
  logger.warn('mahoraga connect operation failed', {
    harness,
    op,
    error: message,
  })
