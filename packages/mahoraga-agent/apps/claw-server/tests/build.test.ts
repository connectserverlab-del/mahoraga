...
function getNativeTarget(): { id: string; ext: string; stagedName: string } {
  ...
  return {
    id: `${os}-${cpu}`,
    ext,
    stagedName: `mahoraga-claw-server${ext}`,
  }
}
...
describe('claw server build', () => {
  ...
  const binaryPath = resolve(
    rootDir,
    `dist/prod/claw-server/.tmp/binaries/mahoraga-claw-server-${target.id}${target.ext}`,
  )
  ...
  const zipPath = resolve(
    rootDir,
    `dist/prod/claw-server/mahoraga-claw-server-resources-${target.id}.zip`,
  )
