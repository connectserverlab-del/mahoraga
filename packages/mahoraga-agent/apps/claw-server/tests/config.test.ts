...
describe('loadClawConfig', () => {
  test('resolves default resources next to the packaged executable', () => {
    expect(
      resolveDefaultResourcesDir('/service/workdir', {
        execPath: '/opt/mahoraga-claw/resources/bin/mahoraga-claw-server',
        isStandalone: true,
      }),
    ).toBe('/opt/mahoraga-claw/resources')
  })
  ...
  test('accepts standalone binary argv without a script path', async () => {
    ...
    const result = loadClawConfig({
      argv: ['/usr/bin/mahoraga-claw-server', '--config', configPath],
      cwd: '/',
      env: {},
    })
  ...
  test('tolerates unknown future and Mahoraga-only sidecar fields', async () => {
    const configPath = await writeConfig({
  ...
  test('rejects old Claw config inputs', async () => {
    ...
    const envResult = loadClawConfig({
      ...
      env: {
        CLAW_SERVER_PORT: '1',
        MAHORAGA_CLAW_CDP_PORT: '2',
        MAHORAGA_CLAW_RESOURCES_DIR: 'env-resources',
      },
    })
