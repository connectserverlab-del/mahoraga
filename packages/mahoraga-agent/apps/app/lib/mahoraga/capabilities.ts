import { env } from '../env'
import { MahoragaAdapter } from './adapter'

const SERVER_VERSION_PREF = 'mahoraga.server.version'

type FeatureConfig = {
  minMahoragaVersion?: string
  maxMahoragaVersion?: string
...
/**
 * Features gated by Mahoraga version or explicit environment flags.
 ...
 * - minMahoragaVersion: feature enabled when Mahoraga >= this version
 * - maxMahoragaVersion: feature enabled when Mahoraga < this version (for deprecation)
...
const FEATURE_CONFIG: { [K in Feature]: FeatureConfig } = {
  ...
  [Feature.NEWTAB_CHAT_SUPPORT]: { minMahoragaVersion: '0.40.0.0' },
  [Feature.VERTICAL_TABS_SUPPORT]: { minMahoragaVersion: '0.42.0.0' },
  ...
  [Feature.AGENT_HARNESS_SUPPORT]: { minMahoragaVersion: '0.46.0.0' },
...
function hasVersionConstraints(config: FeatureConfig): boolean {
  return Boolean(
    config.minMahoragaVersion ||
      config.maxMahoragaVersion ||
...
async function doInitialize(): Promise<CapabilitiesState> {
  const adapter = MahoragaAdapter.getInstance()
  ...
  try {
    const versionStr = await adapter.getMahoragaVersion()
    if (versionStr) {
      state.mahoragaVersion = parseVersion(versionStr)
    }
  } catch {
    // Mahoraga version unknown - features requiring it will be disabled
  }
...
export function checkFeatureSupport(
  ...
  const hasMahoragaConstraints =
    config.minMahoragaVersion || config.maxMahoragaVersion
  if (
    hasMahoragaConstraints &&
    !checkVersionConstraints(
      state.mahoragaVersion,
      config.minMahoragaVersion,
      config.maxMahoragaVersion,
    )
  ) {
...
export const Capabilities = {
  ...
  async getMahoragaVersion(): Promise<string | null> {
    const state = await ensureInitialized()
    if (!state.mahoragaVersion) return null
    return state.mahoragaVersion.join('.')
  },
