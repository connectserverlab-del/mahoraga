...
let mahoragaToggleCalls: unknown[] = []
let mahoragaIsOpenCalls: unknown[] = []
let openCalls: unknown[] = []
let closeCalls: unknown[] = []
let setOptionsCalls: unknown[] = []
let mahoragaIsOpenResult = false
...
beforeEach(async () => {
  ...
  closeCalls = []
  setOptionsCalls = []
  mahoragaIsOpenResult = false
  getSidePanelPerWindowOverride = null

  globalThis.chrome = {
    sidePanel: {
      mahoragaToggle: async (options: unknown) => {
        mahoragaToggleCalls.push(options)
        return { opened: true }
      },
      mahoragaIsOpen: async (options: unknown) => {
        mahoragaIsOpenCalls.push(options)
        return mahoragaIsOpenResult
      },
...
describe('side panel scope routing', () => {
  ...
  it('keeps toolbar toggles on the Mahoraga tab-specific API when scope storage is absent', async () => {
    const result = await toggleSidePanel({ tabId: 7, windowId: 3 })

    expect(result).toEqual({ opened: true })
    expect(mahoragaToggleCalls).toEqual([{ tabId: 7 }])
    expect(openCalls).toEqual([])
    expect(closeCalls).toEqual([])
  })

  it('keeps toolbar toggles on the Mahoraga tab-specific API when scope storage is false', async () => {
  ...
  it('uses Chromium window APIs when the window-level preference is enabled', async () => {
    ...
    expect(mahoragaToggleCalls).toEqual([])
  ...
  it('keeps programmatic opens on the Mahoraga API in window mode', async () => {
    await setSidePanelPerWindowPreference(true)

    const result = await openSidePanel({ tabId: 7, windowId: 3 })

    expect(result).toEqual({ opened: true })
    expect(mahoragaIsOpenCalls).toEqual([{ tabId: 7 }])
    expect(mahoragaToggleCalls).toEqual([{ tabId: 7 }])
    expect(openCalls).toEqual([])
    expect(closeCalls).toEqual([])
  })
