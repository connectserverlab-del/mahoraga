...
describe('checkFeatureSupport — AGENT_HARNESS_SUPPORT', () => {
  const at = (mahoragaVersion: number[] | null) =>
    checkFeatureSupport(
      { mahoragaVersion, serverVersion: null },
      Feature.AGENT_HARNESS_SUPPORT,
    )

  it('hides harness agents below Mahoraga 0.46.0.0 or when version is unknown', () => {
    expect(at([0, 45, 9, 9])).toBe(false)
    expect(at([0, 45, 0, 0])).toBe(false)
    expect(at(null)).toBe(false)
  })

  it('shows harness agents at or above Mahoraga 0.46.0.0', () => {
...
describe('checkFeatureSupport — HERMES_AGENT_SUPPORT', () => {
  const at = (serverVersion: number[] | null) =>
    checkFeatureSupport(
      { mahoragaVersion: null, serverVersion },
