...
function checkFeatureSupport(
  ...
  if (feature === Feature.AGENT_HARNESS_SUPPORT) {
    return compareVersionAtLeast(state.mahoragaVersion, [0, 46, 0, 0])
  }
  if (feature === Feature.HERMES_AGENT_SUPPORT) {
...
const mahoragaAdapter = {
  ...
  getPref: async (name: string) =>
    new Promise<{ value: unknown } | null>((resolve) => {
      ...
      const getPref = globalThis.chrome?.mahoraga?.getPref
      if (getPref) {
        getPref(name, resolve)
        return
      }
      resolve(name === MAHORAGA_PREFS.MCP_PORT ? { value: 9105 } : null)
    }),
...
mock.module('@/lib/mahoraga/adapter', () => ({
  MahoragaAdapter: {
    getInstance: () => mahoragaAdapter,
  },
  getMahoragaAdapter: () => mahoragaAdapter,
}))

mock.module('@/lib/mahoraga/prefs', () => ({
  MAHORAGA_PREFS,
}))

mock.module('@/lib/mahoraga/sidePanelOpenStateStorage', () => ({
...
mock.module('@/lib/mahoraga/capabilities', () => ({
  Capabilities: {
