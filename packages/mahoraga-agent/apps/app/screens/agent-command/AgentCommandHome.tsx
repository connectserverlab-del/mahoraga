...
import { Feature } from '@/lib/mahoraga/capabilities'
import { createMahoragaAction } from '@/lib/chat-actions/types'
import { openSidePanelWithSearch } from '@/lib/messaging/sidepanel/openSidepanelWithSearch'
import {
  useAgentAdapters,
  useHarnessAgents,
} from '@/modules/agents/agents.hooks'
import { useCapabilities } from '@/modules/mahoraga/capabilities.hooks'
...
export const AgentCommandHome: FC = () => {
  ...
  // Default the picker to the user's default LLM provider (Mahoraga out of the
  ...
  const handleSend = async (input: ConversationInputSendInput) => {
    ...
    if (llmRoutingMode === 'sidepanel') {
      const action = createMahoragaAction({
        mode: 'chat',
        message: input.text,
        ...
        <div className="flex flex-col items-center gap-5 pt-[max(10vh,24px)] text-center">
          <div className="space-y-3">
            ...
            <p className="mx-auto max-w-2xl text-muted-foreground text-sm leading-6 [text-wrap:pretty]">
              Pick Mahoraga AI or any agent, then start a task — all without
              leaving this tab.
            </p>
          </div>
