...
import { getProxyPort } from '@/lib/mahoraga/helpers'
...
async function readCurrentPort(): Promise<number> {
  try {
    ...
    // Pref unset or Mahoraga API unavailable — fall back to the default port
    return PROXY_PORT_MIN
  }
}

/** Popover for editing the MCP proxy port and waiting for Mahoraga to rebind it. */
export const ServerPortEditor: FC<ServerPortEditorProps> = ({
  ...
  const handleSave = async () => {
    ...
    setError(null)
    try {
      const success = await getMahoragaAdapter().setPref(
        MAHORAGA_PREFS.PROXY_PORT,
        result.port,
      )
      if (!success) {
