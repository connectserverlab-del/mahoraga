...
function parseErrorMessage(
  ...
  const isMahoragaProvider = providerType === 'mahoraga'

  // All chat requests go through the local Mahoraga agent server, so any
  // fetch failure is always a local connection issue.
  if (message.includes('Failed to fetch') || message.includes('fetch failed')) {
    return {
      text: 'Unable to connect to Mahoraga agent. Follow below instructions.',
      url: 'https://docs.mahoraga.com/troubleshooting/connection-issues',
      isConnectionError: true,
    }
  }

  // Detect credit exhaustion from gateway (Mahoraga provider only)
  if (
    isMahoragaProvider &&
  ...
  // Detect Mahoraga rate limit (Mahoraga provider only)
  if (
    isMahoragaProvider &&
    message.includes('Mahoraga LLM daily limit reached')
  ) {
    return {
      text: 'Add your own API key for unlimited usage.',
      url: 'https://dub.sh/mahoraga-usage-limit',
      isRateLimit: true,
    }
  }
