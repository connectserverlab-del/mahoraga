import type { SnapshotDiff } from '@mahoraga/browser-core/core/snapshot/diff'
  ...
  if (tokenEstimate > MAX_INLINE_DIFF_TOKENS) {
    ...
    try {
      ...
      const text = diff.urlChanged
        ? `URL changed; full current snapshot is ${tokenEstimate} estimated tokens, over the ${MAX_INLINE_DIFF_TOKENS}-token inline limit, but saving it to a Mahoraga output file failed: ${saveError}`
        : `Diff is ${tokenEstimate} estimated tokens, over the ${MAX_INLINE_DIFF_TOKENS}-token inline limit, but saving it to a Mahoraga output file failed: ${saveError}`
      return {
        text: [
          text,
