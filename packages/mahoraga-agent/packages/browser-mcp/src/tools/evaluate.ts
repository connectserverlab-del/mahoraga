    ...
    if (text.length > TOOL_LIMITS.INLINE_PAGE_CONTENT_MAX_CHARS) {
      ...
      try {
        ...
        return textResult(
          [
            wrapUntrusted(excerpt, origin),
            `Evaluate result truncated at ${TOOL_LIMITS.INLINE_PAGE_CONTENT_MAX_CHARS} chars. Full result (${text.length} chars) could not be saved to a Mahoraga output file: ${saveError}`,
          ].join('\n\n'),
          {
            page: args.page,
            contentLength,
