...
import {
  diffSnapshots,
  type SnapshotDiff,
} from '@mahoraga/browser-core/core/snapshot/diff'
...
async function withMahoragaDir<T>(run: () => Promise<T>): Promise<T> {
  ...
  const mahoragaDir = mkdtempSync(join(tmpdir(), 'diff-format-test-'))
...
async function withOutputWriteFailure<T>(run: () => Promise<T>): Promise<T> {
  const previous = process.env.MAHORAGA_DIR
  const mahoragaDir = mkdtempSync(join(tmpdir(), 'diff-format-fail-'))
  ...
  it('keeps save-failure structured content metadata-only', async () => {
    await withOutputWriteFailure(async () => {
      ...
      expect(result.text).toContain(
        'saving it to a Mahoraga output file failed',
      )
      expect(result.text).not.toContain(lastMarker)
