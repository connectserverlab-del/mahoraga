...
describe('resource manifest and artifact staging', () => {
  ...
  it('stages the product binary with executable mode for non-Windows targets', async () => {
    ...
    const stagedPath = join(artifact.resourcesDir, 'bin/mahoraga-claw-server')
    expect(stagedBinaryName(product, target)).toBe('mahoraga-claw-server')
  ...
  it('archives with the descriptor archive base name and uploads latest plus version keys', async () => {
    ...
    expect(archiveResults[0]?.zipPath).toBe(
      join(distRoot, 'mahoraga-claw-server-resources-darwin-arm64.zip'),
    )
    ...
    expect(uploadedKeys).toEqual([
      'server/prod-resources/latest/mahoraga-claw-server-resources-darwin-arm64.zip',
      'server/prod-resources/0.0.0-test/mahoraga-claw-server-resources-darwin-arm64.zip',
    ])
