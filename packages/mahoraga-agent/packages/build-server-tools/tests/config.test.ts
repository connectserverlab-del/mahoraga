...
describe('build config', () => {
  ...
  it('does not pass demoted wrong-source process env to subprocesses', async () => {
    ...
    await writeFile(
      join(tempRoot, '.env.development'),
      formatEnv({ VITE_MAHORAGA_CLAW_API_URL: 'http://127.0.0.1:9200' }),
    )
    await writeFile(
      join(tempRoot, '.env.production'),
      formatEnv(REQUIRED_INLINE_ENV),
    )
    process.env.VITE_MAHORAGA_CLAW_API_URL = 'http://127.0.0.1:9200'
    ...
    try {
      ...
      expect(config.processEnv.VITE_MAHORAGA_CLAW_API_URL).toBeUndefined()
      expect(warnings).toEqual([
        'env: ignoring 1 process values that match root .env.development (Bun auto-load): VITE_MAHORAGA_CLAW_API_URL',
      ])
    } finally {
  ...
  it('passes process env through in CI when there are no root files to demote', async () => {
    ...
    expect(config.processEnv.VITE_MAHORAGA_CLAW_API_URL).toBe(
      'http://127.0.0.1:9200',
    )
  })
