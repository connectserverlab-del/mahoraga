...
func DefaultDevUserDataDir(root string) (string, error) {
	...
	return filepath.Join(os.TempDir(), fmt.Sprintf("mahoraga-dev-%s-%s", worktreeLabel(absRoot), key)), nil
}

func worktreeLabel(root string) string {
	worktree := root
	if filepath.Base(root) == "mahoraga-agent" && filepath.Base(filepath.Dir(root)) == "packages" {
