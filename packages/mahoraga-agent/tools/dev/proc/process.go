...
func DefaultWatchRunBaseDir() (string, error) {
	...
	return filepath.Join(home, ".mahoraga-dev", "runs"), nil
}
...
// KillBrowserProcessesForDevProfiles kills Mahoraga instances using temporary dev/test profiles.
func KillBrowserProcessesForDevProfiles(timeout time.Duration) (int, error) {
	return killBrowserProcesses([]string{"/tmp/mahoraga-dev"}, true, timeout)
}

// KillBrowserProcessesForUserDataDirs kills Mahoraga instances using the given user-data dirs.
...
func browserProfilePIDsFromPS(output string) []int {
	return browserProfilePIDsFromPSForUserDataDirs(output, []string{"/tmp/mahoraga-dev"}, true)
}
...
func isDevBrowserProcess(command string) bool {
	return isBrowserProcessForUserDataDir(command, []string{"/tmp/mahoraga-dev"}, true)
}
...
func isBrowserProcessForUserDataDir(command string, userDataDirs []string, includeDevTempProfiles bool) bool {
	...
	return includeDevTempProfiles &&
		(strings.Contains(command, "mahoraga-dev-") ||
			strings.Contains(command, "mahoraga-test-"))
}

func isBrowserAppCommand(command string) bool {
	return strings.Contains(command, "Mahoraga.app/Contents/MacOS/Mahoraga") ||
