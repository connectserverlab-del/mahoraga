...
const watchLockHelperEnv = "MAHORAGA_DEV_WATCH_LOCK_HELPER"
...
func TestWatchRunPathsStableAndSharedAcrossPortChanges(t *testing.T) {
	...
	withDifferentProfile.Profile = "/tmp/mahoraga-dev-other"
...
func TestBrowserProfilePIDsFromPSSelectsOnlyDevAndTestProfiles(t *testing.T) {
	output := `
  111  111 /Applications/Mahoraga.app/Contents/MacOS/Mahoraga --user-data-dir=/tmp/mahoraga-dev
  222  222 /Applications/BrowserClaw.app/Contents/MacOS/BrowserClaw --user-data-dir=/tmp/mahoraga-dev-abcd
  333  333 /Applications/BrowserClaw.app/Contents/MacOS/BrowserClaw --user-data-dir=/var/folders/x/mahoraga-test-abcd
  444  444 /Applications/BrowserClaw.app/Contents/MacOS/BrowserClaw --user-data-dir=/Users/me/Library/Application Support/Mahoraga
  555  555 rg mahoraga-test-
`
...
func TestDefaultDevUserDataDirIsWorktreeScoped(t *testing.T) {
	first := filepath.Join(t.TempDir(), "main-2", "packages", "mahoraga-agent")
	second := filepath.Join(t.TempDir(), "feat-new-mcp", "packages", "mahoraga-agent")
	...
	if !strings.Contains(firstProfile, "mahoraga-dev-main-2-") {
...
func TestAcquireWatchRunLockStopsExistingOwnerByStatePGID(t *testing.T) {
	...
	cmd.Env = append(os.Environ(),
		...
		"MAHORAGA_DEV_WATCH_LOCK_BASE="+baseDir,
		"MAHORAGA_DEV_WATCH_LOCK_READY="+readyPath,
		"MAHORAGA_DEV_WATCH_LOCK_IDENTITY="+string(identityJSON),
	)
...
func TestAcquireWatchRunLockStopsLegacyPortKeyedOwner(t *testing.T) {
	...
	cmd.Env = append(os.Environ(),
		watchLockHelperEnv+"=1",
		"MAHORAGA_DEV_WATCH_LOCK_BASE="+helperBaseDir,
		"MAHORAGA_DEV_WATCH_LOCK_READY="+readyPath,
		"MAHORAGA_DEV_WATCH_LOCK_IDENTITY="+string(identityJSON),
	)
	cmd.SysProcAttr = &syscall.SysProcAttr{Setpgid: true}
...
func runWatchLockHelper() {
	baseDir := os.Getenv("MAHORAGA_DEV_WATCH_LOCK_BASE")
	readyPath := os.Getenv("MAHORAGA_DEV_WATCH_LOCK_READY")
	var identity WatchRunIdentity
	if err := json.Unmarshal([]byte(os.Getenv("MAHORAGA_DEV_WATCH_LOCK_IDENTITY")), &identity); err != nil {
		os.Exit(2)
	}
