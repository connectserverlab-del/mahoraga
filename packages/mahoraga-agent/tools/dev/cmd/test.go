...
func init() {
	...
	testCobraCmd.Flags().BoolVar(&testHeadless, "headless", false, "Run Mahoraga headless")
...
func runTest(cmd *cobra.Command, args []string) error {
	...
	if n := proc.CleanupTempDirs("mahoraga-test-"); n > 0 {
	...
	tempDir, err = os.MkdirTemp("", "mahoraga-test-")
	if err != nil {
		cleanup()
	...
	proc.LogMsg(proc.TagBrowser, "Starting Mahoraga...")
	procs = append(procs, proc.StartManaged(ctx, &wg, proc.ProcConfig{
		Tag:     proc.TagBrowser,
		Dir:     root,
		Restart: false,
