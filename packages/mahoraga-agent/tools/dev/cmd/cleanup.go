...
var cleanupCmd = &cobra.Command{
	...
	Long:  "Stops target Mahoraga processes, clears target ports, and removes target temp directories.",
	RunE:  runCleanup,
}
...
func init() {
	...
	cleanupCmd.Flags().StringVar(&cleanupMahoragaDir, "mahoraga-dir", "", "Override target Mahoraga state directory")
	cleanupCmd.Flags().StringVar(&cleanupPortsValue, "ports", "", "Override ports as cdp,server,extension")
	cleanupCmd.Flags().StringVar(&cleanupBrowserUserDataDir, "browser-user-data-dir", "", "Override Mahoraga user-data dir to stop")
...
func runCleanup(cmd *cobra.Command, args []string) error {
	...
	target, err := resolveResetTarget(root, resetTargetOptions{
		...
		MahoragaDir:       cleanupMahoragaDir,
	...
	if !cleanupYes && !cleanupQuick {
		ok, err := confirmYesNo(out, bufio.NewReader(os.Stdin), resetPrompt{
			...
			Body:   fmt.Sprintf("Stops %s processes, clears target ports, and removes target temp profiles. This does not touch saved Mahoraga data, Lima, containers, or images.", target.Name),
...
func runSafeCleanup(out io.Writer, target resetTarget, opts safeCleanupOptions) error {
	if opts.ports {
		...
		if len(target.BrowserUserDataDirs) > 0 {
			...
			if killedBrowsers > 0 {
				fmt.Fprintf(out, "%s stopped %d Mahoraga %s profile process(es)\n", successStyle.Sprint("Stopped:"), killedBrowsers, target.Name)
			}
		}
		if target.Ports != nil {
			ports := *target.Ports
