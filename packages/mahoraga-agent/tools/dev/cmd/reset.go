...
const (
	limaVMName = "mahoraga-vm"
)

var resetCmd = &cobra.Command{
	Use:   "reset",
	Short: "Guide destructive Mahoraga profile and VM resets",
	Long:  "Walks through safe cleanup, VM shutdown/deletion, and target Mahoraga state reset.",
	RunE:  runReset,
}
...
func init() {
	...
	resetCmd.Flags().StringVar(&resetMahoragaDir, "mahoraga-dir", "", "Override target Mahoraga state directory")
	resetCmd.Flags().StringVar(&resetPortsValue, "ports", "", "Override ports as cdp,server,extension")
	resetCmd.Flags().StringVar(&resetBrowserUserDataDir, "browser-user-data-dir", "", "Override Mahoraga user-data dir to stop")
...
func runReset(cmd *cobra.Command, args []string) error {
	...
	if ok, err := confirmYesNo(out, reader, resetPrompt{
		...
		Body:   fmt.Sprintf("This stops %s processes, clears target ports, and removes target temp profiles. It does not touch saved Mahoraga data.", target.Name),
	...
	if strings.EqualFold(vm.Status, "Running") {
		if ok, err := confirmYesNo(out, reader, resetPrompt{
			Title:  "Stop VM?",
			Body:   "This shuts down mahoraga-vm. The VM, containers, images, and profile data stay on disk.",
			Action: "Stop mahoraga-vm",
		}); err != nil {
			return err
		} else if ok {
	...
	if ok, err := confirmYesNo(out, reader, resetPrompt{
		Title:  "Delete VM?",
		Body:   fmt.Sprintf("This deletes the Lima VM and its container store. %s remains.", target.MahoragaDir),
		Action: "Delete mahoraga-vm",
	}); err != nil {
		return err
	} else if ok {
...
func printResetOverview(out io.Writer, target resetTarget) {
	...
	fmt.Fprintln(out)
	fmt.Fprintf(out, "This can reset parts of %s. Pick the smallest reset that matches the problem.\n", pathStyle.Sprint(target.MahoragaDir))
	fmt.Fprintln(out)
	fmt.Fprintf(out, "  %s %s\n", labelStyle.Sprint("Stop VM:"), dimStyle.Sprint("Shuts down mahoraga-vm. Keeps data."))
	fmt.Fprintf(out, "  %s %s\n", labelStyle.Sprint("Delete VM:"), dimStyle.Sprint("Removes Lima/container state. Keeps the target state root."))
	fmt.Fprintf(out, "  %s %s\n", warnStyle.Sprint(target.DeleteRootLabel), dimStyle.Sprint("Deletes the target Mahoraga state root."))
	fmt.Fprintln(out)
}
...
func maybeDeleteTargetRoot(out io.Writer, reader *bufio.Reader, target resetTarget) error {
	ok, err := confirmTyped(
		...
		fmt.Sprintf("This deletes %s. %s", pathStyle.Sprint(target.MahoragaDir), target.DeleteRootBody),
		"DELETE",
	)
	if err != nil || !ok {
		return err
	}
	if err := validateDevProfileRootForDeletion(target.MahoragaDir); err != nil {
		return err
	}
	if err := os.RemoveAll(target.MahoragaDir); err != nil {
		return err
	}
	fmt.Fprintf(out, "%s %s\n", successStyle.Sprint("Deleted:"), pathStyle.Sprint(target.MahoragaDir))
	return nil
}
...
func maybeResetLegacyPodman(out io.Writer, reader *bufio.Reader) error {
	...
	fmt.Fprintln(out, "Mahoraga used Podman before the Lima VM runtime. These machines are legacy for this dev flow.")
