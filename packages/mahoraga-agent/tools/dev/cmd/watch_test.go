...
func TestResolveWatchDefaultPortsKeepsMahoragaServerPort(t *testing.T) {
	root := writeWatchEnvExample(t, "MAHORAGA_CDP_PORT=9001\nMAHORAGA_SERVER_PORT=9101\nMAHORAGA_EXTENSION_PORT=9301\n")
	...
	if ports != want {
		t.Fatalf("expected Mahoraga watch ports %+v, got %+v", want, ports)
	}
}
...
func TestBuildWatchEnvSelectsMahoragaProduct(t *testing.T) {
	env := buildWatchEnv(proc.Ports{
		CDP:       9012,
		Server:    9123,
		Extension: 9321,
	}, "/tmp/mahoraga-dev", false)

	for _, want := range []string{
		"MAHORAGA_PRODUCT=mahoraga",
		"MAHORAGA_USER_DATA_DIR=/tmp/mahoraga-dev",
	} {
...
func TestBuildWatchEnvSelectsBrowserClawProduct(t *testing.T) {
	env := buildWatchEnvWithBinaryResolution(proc.Ports{
		...
		Server:    9123,
		Extension: 9321,
	}, "/tmp/mahoraga-dev", true, browser.BinaryResolution{
	...
	for _, want := range []string{
		"MAHORAGA_PRODUCT=browserclaw",
		"MAHORAGA_BINARY=/Applications/BrowserClaw.app/Contents/MacOS/BrowserClaw",
		"MAHORAGA_CLAW_CDP_PORT=9012",
		"VITE_MAHORAGA_CLAW_API_URL=http://127.0.0.1:9123",
	} {
		if !hasEnvEntry(env, want) {
			t.Fatalf("expected env to contain %q, got %#v", want, env)
		}
...
func TestBuildWatchEnvUsesFallbackBrowserBinaryForClaw(t *testing.T) {
	env := buildWatchEnvWithBinaryResolution(proc.Ports{
		...
		Product:       browser.ProductBrowserClaw,
		Path:          browser.MahoragaBinaryPath,
		PreferredPath: browser.BrowserClawBinaryPath,
		Fallback:      true,
	})

	if !hasEnvEntry(env, "MAHORAGA_BINARY=/Applications/Mahoraga.app/Contents/MacOS/Mahoraga") {
...
func TestRustClawWatchInputsUseSourceAndManifestInputs(t *testing.T) {
	...
	for _, dir := range []string{
		...
		"crates/mahoraga-core/src",
		"crates/mahoraga-cdp/src",
		"crates/mahoraga-cdp/protocol",
		"target/debug",
	} {
	...
	for _, file := range []string{
		...
		"crates/mahoraga-core/Cargo.toml",
		"crates/mahoraga-cdp/Cargo.toml",
		"crates/mahoraga-cdp/build.rs",
	} {
	...
	for _, want := range []string{
		...
		filepath.Join(root, "crates/mahoraga-cdp/src"),
		filepath.Join(root, "crates/mahoraga-cdp/Cargo.toml"),
		filepath.Join(root, "crates/mahoraga-cdp/build.rs"),
		filepath.Join(root, "crates/mahoraga-cdp/protocol"),
		filepath.Join(root, "crates/mahoraga-core/src"),
		filepath.Join(root, "crates/mahoraga-core/Cargo.toml"),
