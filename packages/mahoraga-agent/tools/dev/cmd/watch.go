...
func runWatch(cmd *cobra.Command, args []string) error {
	...
	if watchNew {
		...
		dir, err := os.MkdirTemp("", "mahoraga-dev-")
		if err != nil {
		...
		if killedBrowsers > 0 {
			proc.LogMsgf(proc.TagInfo, "Stopped %d Mahoraga process(es) for profile %s", killedBrowsers, userDataDir)
		}
	...
	if watchClaw {
		...
		procs, err = startMahoragaWatch(ctx, &wg, root, env, p, reservations, userDataDir, watchManual)
		if err != nil {
			return err
		}
	}
...
func buildWatchEnvWithBinaryResolution(p proc.Ports, userDataDir string, claw bool, binaryResolution browser.BinaryResolution) []string {
	...
	env = append(env,
		fmt.Sprintf("MAHORAGA_USER_DATA_DIR=%s", userDataDir),
		fmt.Sprintf("MAHORAGA_PRODUCT=%s", watchProduct(claw)),
	)
	if claw {
		if binaryResolution.Path == "" {
			binaryResolution = browser.ResolveInstalledBinary(browser.ProductBrowserClaw)
		}
		env = append(env, fmt.Sprintf("MAHORAGA_BINARY=%s", binaryResolution.Path))
...
func watchProduct(claw bool) string {
	...
	return browser.ProductMahoraga
}
...
func buildClawWatchEnv(env []string, p proc.Ports) []string {
	...
	return append(env,
		fmt.Sprintf("MAHORAGA_CLAW_CDP_PORT=%d", p.CDP),
...
// startMahoragaWatch supervises the Mahoraga agent extension plus server dev pair.
func startMahoragaWatch(ctx context.Context, wg *sync.WaitGroup, root string, env []string, p proc.Ports, reservations *proc.PortReservations, userDataDir string, manual bool) ([]*proc.ManagedProc, error) {
	var procs []*proc.ManagedProc
	...
	if manual {
		...
		procs = append(procs, proc.StartManaged(ctx, wg, proc.ProcConfig{
			...
			Cmd: browser.BuildArgs(browser.ArgsConfig{
				...
				Product:           browser.ProductMahoraga,
			}),
		}))
	} else {
		reservations.ReleaseCDP()
	...
	sidecarPath := watchSidecarConfigPath(userDataDir, "mahoraga-server")
