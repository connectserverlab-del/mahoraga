...
func TestConfirmYesNoDefaultsNoAndExplainsAction(t *testing.T) {
	...
	prompt := resetPrompt{
		Title:  "Stop VM?",
		Body:   "This shuts down mahoraga-vm. Data stays on disk.",
	...
	for _, want := range []string{
		"Stop VM?",
		"This shuts down mahoraga-vm. Data stays on disk.",
		"Stop mahoraga-vm",
		"[y/N]",
	} {
...
func TestResetOverviewTellsUserToUseSmallestReset(t *testing.T) {
	...
	printResetOverview(&out, resetTarget{
		...
		MahoragaDir:    "/Users/me/.mahoraga-dev",
...
func TestParseLimaListOutputAcceptsSingleObject(t *testing.T) {
	entries, err := parseLimaListOutput([]byte(`{"name":"mahoraga-vm","status":"Running"}`))
	if err != nil {
		t.Fatal(err)
	}
	if len(entries) != 1 || entries[0].Name != "mahoraga-vm" || entries[0].Status != "Running" {
		t.Fatalf("unexpected entries: %#v", entries)
	}
}

func TestParseLimaListOutputAcceptsJSONLines(t *testing.T) {
	entries, err := parseLimaListOutput([]byte("{\"name\":\"one\",\"status\":\"Stopped\"}\n{\"name\":\"mahoraga-vm\",\"status\":\"Running\"}\n"))
	if err != nil {
		t.Fatal(err)
	}
	if len(entries) != 2 || entries[1].Name != "mahoraga-vm" || entries[1].Status != "Running" {
		t.Fatalf("unexpected entries: %#v", entries)
	}
}
...
func TestLimactlShellArgsUseGuestWorkdir(t *testing.T) {
	...
	want := []string{"shell", "--workdir", "/", "mahoraga-vm", "--", "sh", "-lc", "true"}
