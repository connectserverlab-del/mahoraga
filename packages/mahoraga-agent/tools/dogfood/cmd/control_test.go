...
func TestFormatStatusDataUsesHumanReadableSummary(t *testing.T) {
	got := formatStatus(map[string]any{
		...
		"pid":           float64(123),
		"uptime":        "5s",
		"log_path":      "/tmp/mahoraga-dogfood/daemon.jsonl",
		"mahoraga_dir": "/tmp/mahoraga-dogfood",
	...
	for _, want := range []string{
		...
		"Mahoraga dir: /tmp/mahoraga-dogfood",
		"Logs: /tmp/mahoraga-dogfood/daemon.jsonl",
	} {
...
func TestFormatStatusDataForClawOmitsExtensionPort(t *testing.T) {
	got := formatStatus(map[string]any{
		...
		"state":     "running",
		"state_dir": "/tmp/mahoraga-claw-dogfood",
		"log_path":  "/tmp/mahoraga-dogfood/claw/daemon.jsonl",
		"ports": map[string]any{
			"CDP":       float64(49337),
			"Server":    float64(9200),
			"Extension": float64(9315),
	...
	for _, want := range []string{
		...
		"Claw state: /tmp/mahoraga-claw-dogfood",
		"Logs: /tmp/mahoraga-dogfood/claw/daemon.jsonl",
	} {
		if !strings.Contains(got, want) {
			t.Fatalf("formatted status missing %q:\n%s", want, got)
		}
...
func TestDaemonUnavailableErrorIncludesTargetStartWhenNotRunning(t *testing.T) {
	...
	got := err.Error()
	if !strings.Contains(got, "mahoraga-dogfood --claw start-background") {
		t.Fatalf("missing target-specific start command: %v", err)
	}
	if strings.Contains(got, "mahoraga-dogfood start-background") {
...
func TestMonitorDaemonUntilRunningReturnsDaemonError(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "mahoraga-dogfood --claw logs tail") {
...
func TestMonitorDaemonUntilRunningDetachesOnInterrupt(t *testing.T) {
	...
	if !strings.Contains(got, "mahoraga-dogfood --claw logs tail") {
