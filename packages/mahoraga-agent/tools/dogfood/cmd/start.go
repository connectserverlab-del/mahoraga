...
import (
	...
	"mahoraga-dogfood/browser"
	"mahoraga-dogfood/config"
	"mahoraga-dogfood/pipeline"
	"mahoraga-dogfood/proc"
	"mahoraga-dogfood/profile"
	dogfoodruntime "mahoraga-dogfood/runtime"
...
func init() {
	startCmd.Flags().BoolVar(&startRefreshProfile, "refresh-profile", false, "Refresh copied Mahoraga profile before launch")
	startCmd.Flags().BoolVar(&startHeadless, "headless", false, "Run Mahoraga headless")
	startBackgroundCmd.Flags().BoolVar(&startBackgroundRefreshProfile, "refresh-profile", false, "Refresh copied Mahoraga profile before launch")
	startBackgroundCmd.Flags().BoolVar(&startBackgroundHeadless, "headless", false, "Run Mahoraga headless")
...
func buildAndStartEnvironment(ctx context.Context, cfg config.Config, opts environmentOptions) (*environment, error) {
	...
	case config.TargetMahoraga:
		return buildAndStartMahoragaEnvironment(ctx, cfg, agentRoot, opts)
...
func buildAndStartMahoragaEnvironment(ctx context.Context, cfg config.Config, agentRoot string, opts environmentOptions) (*environment, error) {
	reportProgress(opts, "preparing profile")
	if err := prepareMahoragaEnvironment(&cfg, agentRoot, opts); err != nil {
		return nil, err
	}
	...
	return startMahoragaEnvironment(ctx, cfg, agentRoot, opts)
}
...
func prepareStartCheckout(ctx context.Context, cfg config.Config, runner pipeline.Runner) (bool, error) {
	...
	if !dirty {
		if err := pipeline.EnsureBranch(ctx, cfg.RepoPath, cfg.Branch, runner, false); err != nil {
			return false, fmt.Errorf("switch to configured branch %s failed; run `mahoraga-dogfood %s pull` first if the branch is not available locally: %w", cfg.Branch, targetFlagOrDefault(cfg.Target), err)
		}
		return false, nil
	}
...
func prepareMahoragaEnvironment(cfg *config.Config, agentRoot string, opts environmentOptions) error {
...
func startMahoragaEnvironment(parent context.Context, cfg config.Config, agentRoot string, opts environmentOptions) (*environment, error) {
...
func serverRuntimeEnv(base []string, cfg config.Config) []string {
	...
	for _, entry := range base {
		if strings.HasPrefix(entry, "MAHORAGA_DIR=") {
			continue
		}
	...
	return append(env,
		...
		fmt.Sprintf("MAHORAGA_DIR=%s", cfg.MahoragaDir),
	)
}
...
func clawRuntimeEnv(base []string, cfg config.Config) []string {
	...
	for _, entry := range base {
		if strings.HasPrefix(entry, "MAHORAGA_DIR=") ||
			...
			strings.HasPrefix(entry, "MAHORAGA_BINARY=") ||
			strings.HasPrefix(entry, "MAHORAGA_USER_DATA_DIR=") ||
			strings.HasPrefix(entry, "MAHORAGA_CLAW_CDP_PORT=") ||
			strings.HasPrefix(entry, "MAHORAGA_SERVER_PORT=") ||
			strings.HasPrefix(entry, "VITE_MAHORAGA_CLAW_API_URL=") {
			continue
		}
		env = append(env, entry)
	}
	...
	return append(env,
		"NODE_ENV=development",
		fmt.Sprintf("BROWSERCLAW_DIR=%s", cfg.MahoragaDir),
		fmt.Sprintf("MAHORAGA_BINARY=%s", cfg.MahoragaAppPath),
		fmt.Sprintf("MAHORAGA_USER_DATA_DIR=%s", cfg.DevUserDataDir),
		fmt.Sprintf("MAHORAGA_CLAW_CDP_PORT=%d", cfg.Ports.CDP),
		fmt.Sprintf("MAHORAGA_SERVER_PORT=%d", cfg.Ports.Server),
		fmt.Sprintf("VITE_MAHORAGA_CLAW_API_URL=%s", apiURL),
	)
}
...
func resolveConfiguredBrowserApp(cfg *config.Config, exists func(string) bool) config.BrowserAppResolution {
	resolution := config.ResolveBrowserAppPath(cfg.Target, cfg.MahoragaAppPath, exists)
	cfg.MahoragaAppPath = resolution.Path
	return resolution
}
...
func validateBrowserAppExecutable(path string) error {
	...
	if info.IsDir() || info.Mode()&0111 == 0 {
		return fmt.Errorf("mahoraga_app_path is not an executable file: %s", path)
	}
	return nil
}
...
func printSummary(cfg config.Config, agentRoot string) {
	...
	proc.LogMsgf(proc.TagInfo, "App: %s", cfg.MahoragaAppPath)
	...
	proc.LogMsgf(proc.TagInfo, "%s: %s", stateDirLabel(cfg.Target), cfg.MahoragaDir)
