...
func TestRootUsageUsesCommandGroups(t *testing.T) {
	...
	for _, want := range []string{
		...
		"Use \"mahoraga-dogfood [command] --help\" for more information.",
	} {
...
func TestResolveTargetFlags(t *testing.T) {
	tests := []struct {
		...
		{name: "mahoraga", mahoraga: true, wantTarget: config.TargetMahoraga, wantPresent: true},
		{name: "claw", claw: true, wantTarget: config.TargetClaw, wantPresent: true},
		{name: "none"},
		{name: "both", mahoraga: true, claw: true, wantErr: true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, present, err := resolveTargetFlags(tt.mahoraga, tt.claw)
			if tt.wantErr {
				if err == nil {
...
func TestCommandRequiresTargetForLifecycleCommands(t *testing.T) {
	root := &cobra.Command{Use: "mahoraga-dogfood"}
