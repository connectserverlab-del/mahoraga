...
var daemonCmd = &cobra.Command{
	Use:    "daemon",
	Short:  "Run the mahoraga-dogfood background daemon",
	Hidden: true,
	RunE:   runDaemon,
}

func init() {
	daemonCmd.Flags().BoolVar(&daemonHeadless, "headless", false, "Run Mahoraga headless")
	daemonCmd.Flags().BoolVar(&daemonRefreshProfile, "refresh-profile", false, "Refresh copied Mahoraga profile before launch")
...
func runningError(paths runPaths) error {
	...
	if err == nil {
		if state.Mode == "background" {
			return fmt.Errorf("mahoraga-dogfood background daemon is already running (pid %d)", state.PID)
		}
		return fmt.Errorf("mahoraga-dogfood is already running in foreground mode (pid %d)", state.PID)
	}
	return fmt.Errorf("mahoraga-dogfood is already running")
}
	...
	for {
		...
		case <-tick.C:
			if resp, err := ipc.NewClient(paths.Socket).Send(ipc.Request{Command: ipc.CmdStatus}); err == nil && resp.OK {
				...
				fmt.Printf("  %s %s\n", labelStyle.Sprint("Status:"), commandStyle.Sprintf("mahoraga-dogfood %s status", targetFlag))
				fmt.Printf("  %s   %s\n", labelStyle.Sprint("Logs:"), commandStyle.Sprintf("mahoraga-dogfood %s logs tail", targetFlag))
				fmt.Printf("  %s   %s\n", labelStyle.Sprint("Stop:"), commandStyle.Sprintf("mahoraga-dogfood %s stop", targetFlag))
				return nil
			}
		}
	}
...
type daemonStatus struct {
	...
	MahoragaDir string       `json:"mahoraga_dir"`
...
func (d *dogfoodDaemon) status() daemonStatus {
	...
	return daemonStatus{
		...
		Ports:        d.ports,
		MahoragaDir: d.mahoragaDir,
		StateDir:     d.mahoragaDir,
		LogPath:      d.paths.Log,
	}
}
