...
func TestAcquireRefreshProfileLockReportsStopCommandWhenRunning(t *testing.T) {
	...
	for _, want := range []string{"cannot refresh profile", "mahoraga-dogfood --claw stop", "12345"} {
...
func TestEnsureDevProfileNotInUseReportsStopCommand(t *testing.T) {
	...
	for _, want := range []string{"cannot refresh profile", "mahoraga-dogfood --claw stop", devUserDataDir} {
		if !strings.Contains(err.Error(), want) {
			t.Fatalf("error missing %q: %v", want, err)
		}
	}
