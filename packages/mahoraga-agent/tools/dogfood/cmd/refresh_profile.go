...
var refreshProfileCmd = &cobra.Command{
	Use:     "refresh-profile",
	Short:   "Copy the configured Mahoraga profile into the selected dogfood profile",
	GroupID: groupRun,
...
func refreshProfileRunningError(paths runPaths, target config.Target) error {
	...
	if err == nil {
		if state.Mode == "background" {
			return fmt.Errorf("cannot refresh profile while mahoraga-dogfood background daemon is running (pid %d); run `mahoraga-dogfood %s stop` first", state.PID, targetFlag)
		}
		return fmt.Errorf("cannot refresh profile while mahoraga-dogfood is running in foreground mode (pid %d); stop it first", state.PID)
	}
	return fmt.Errorf("cannot refresh profile while mahoraga-dogfood is running; run `mahoraga-dogfood %s stop` first", targetFlag)
}
...
func ensureDevProfileNotInUse(cfg config.Config) error {
	...
	if inUse {
		...
		return fmt.Errorf("cannot refresh profile because the dogfood dev profile is in use at %s; run `mahoraga-dogfood %s stop` first", cfg.DevUserDataDir, targetFlag)
	}
	return nil
}
