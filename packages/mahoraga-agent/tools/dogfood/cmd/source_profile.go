...
func confirmSourceProfileImportWithChecker(out io.Writer, r *bufio.Reader, hasSingletons func() (bool, error)) error {
	for {
		...
		fmt.Fprintln(out, warnStyle.Sprint("Mahoraga appears to be using the source profile."))
		fmt.Fprintf(out, "%s ", labelStyle.Sprint("Quit Mahoraga, then press Enter to retry, or type \"continue\" to import anyway:"))
