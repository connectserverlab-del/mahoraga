...
func TestRunPathsLiveBesideConfig(t *testing.T) {
	paths := newTargetRunPaths(filepath.Join("/tmp", "mahoraga-dogfood", "config.yaml"), config.TargetClaw)
	if paths.Lock != filepath.Join("/tmp", "mahoraga-dogfood", "claw", "run.lock") {
		t.Fatalf("lock path got %q", paths.Lock)
	}
	if paths.Socket != filepath.Join("/tmp", "mahoraga-dogfood", "claw", "daemon.sock") {
		t.Fatalf("socket path got %q", paths.Socket)
	}
	if paths.Log != filepath.Join("/tmp", "mahoraga-dogfood", "claw", "daemon.jsonl") {
...
func TestDaemonStatusIncludesMahoragaDir(t *testing.T) {
	d := &dogfoodDaemon{
		state:        "running",
		startedAt:    time.Now(),
	...
	if got.MahoragaDir != "/tmp/mahoraga-dogfood" {
		t.Fatalf("Mahoraga dir got %q", got.MahoragaDir)
	}
}
