...
func daemonUnavailableError(paths runPaths, target config.Target, cause error) error {
	...
	if err == nil {
		...
		if errors.Is(cause, ipc.ErrDaemonNotRunning) {
			return fmt.Errorf("%w; start it with `mahoraga-dogfood %s start-background`", cause, targetFlagOrDefault(target))
		}
		return cause
	}
	if errors.Is(err, dogfoodruntime.ErrAlreadyRunning) {
		...
		if stateErr == nil && state.Mode == "foreground" {
			return fmt.Errorf("mahoraga-dogfood is running in foreground mode (pid %d); background daemon commands are unavailable", state.PID)
		}
		targetFlag := targetFlagOrDefault(target)
		return fmt.Errorf("mahoraga-dogfood background daemon is not responding; try `mahoraga-dogfood %s stop` if it is stuck, then `mahoraga-dogfood %s start-background`", targetFlag, targetFlag)
	}
	return err
}
...
func monitorDaemonUntilRunning(ctx context.Context, monitor daemonMonitor) error {
	...
	for {
		...
		case <-monitor.Detach:
			...
			fmt.Fprintf(out, "%s daemon still running. Run %s to reattach.\n", warnStyle.Sprint("Detached;"), commandStyle.Sprintf("mahoraga-dogfood %s logs tail", targetFlag))
			return nil
		case err := <-followErr:
...
func daemonReachedTerminalState(status func() (ipc.Response, error), target config.Target) (bool, error) {
	...
	case "error":
		...
		targetFlag := targetFlagOrDefault(target)
		return true, fmt.Errorf("%s; run `mahoraga-dogfood %s logs tail` for details", lastError, targetFlag)
	default:
		return false, nil
	}
}
...
func formatStatus(data any) string {
	...
	target, _ := stringValue(status["target"])
	if target == string(config.TargetClaw) {
		writeStringField(&out, "Claw state", firstStatusValue(status["state_dir"], status["mahoraga_dir"]))
	} else {
		writeStringField(&out, "Mahoraga dir", status["mahoraga_dir"])
	}
