...
func TestWriteProductionEnvFile(t *testing.T) {
	...
	assertContains(t, path, "MAHORAGA_CONFIG_URL=https://llm.mahoraga.com/api/mahoraga-server/config\n")
...
func TestWriteProductionEnvFileKeepsCLIDefaultWhenServerDefaultIsEmpty(t *testing.T) {
	...
	assertContains(t, filepath.Join(root, ".env.production"), "R2_BUCKET=mahoraga\n")
}
