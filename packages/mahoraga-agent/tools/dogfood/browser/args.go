...
func BuildArgs(cfg ArgsConfig) []string {
	args := []string{
		...
		// Keep all server aliases until installed Mahoraga apps converge on one switch.
		fmt.Sprintf("--mahoraga-mcp-port=%d", cfg.Ports.Server),
		fmt.Sprintf("--mahoraga-server-port=%d", cfg.Ports.Server),
		fmt.Sprintf("--mahoraga-proxy-port=%d", cfg.Ports.Server),
