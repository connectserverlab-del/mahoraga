...
const packagePath = 'packages/mahoraga-agent/apps/server/package.json'
const lockPath = 'packages/mahoraga-agent/bun.lock'
...
function writePackage(dir: string, version: string): void {
  ...
  writeFileSync(
    path,
    `${JSON.stringify({ name: '@mahoraga/server', version }, null, 2)}\n`,
  )
}
...
async function initFixture(version: string): Promise<{
  ...
  await mustRun(dir, ['git', 'config', 'user.name', 'Mahoraga Test'])
  await mustRun(dir, ['git', 'config', 'user.email', 'test@mahoraga.com'])
  writePackage(dir, version)
...
describe('prepare-server-release', () => {
  ...
  it('rejects a release version that already exists as a legacy tag', async () => {
    const { dir, bareDir } = await initFixture('0.0.123')
    try {
      await tag(dir, 'mahoraga-server-v0.0.123')
      await tag(dir, 'agent-server/v0.0.123')
      await mustRun(dir, ['git', 'push', 'origin', 'main', '--tags'])
      ...
      expect(outputText(result)).toContain(
        'Release version 0.0.123 already exists as tag mahoraga-server-v0.0.123',
      )
    } finally {
      rmSync(dir, { recursive: true, force: true })
      rmSync(bareDir, { recursive: true, force: true })
