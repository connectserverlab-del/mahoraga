...
describe('generateModelsData', () => {
  test('maps providers and omits deprecated models', () => {
    const output = generateModelsData(
      ...
      { 'source-provider': 'mahoraga-provider' },
    )

    expect(Object.keys(output)).toEqual(['mahoraga-provider'])
    expect(output['mahoraga-provider']).toEqual({
      name: 'Source Provider',
      doc: 'https://example.com/docs',
      models: [
        {
  ...
  test('omits non-chat models', () => {
    ...
    expect(output['mahoraga-provider']?.models.map((m) => m.id)).toEqual([
      'chat-model',
    ])
  })
  ...
  test('rejects duplicate transformed model ids', () => {
    ...
    ).toThrow('Duplicate model id for mahoraga-provider: duplicate-model')
  })

  test('rejects missing required providers', () => {
    expect(() =>
      generateModelsData({}, { 'source-provider': 'mahoraga-provider' }),
