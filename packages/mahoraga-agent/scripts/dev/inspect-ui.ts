...
// Matches DEV_PORTS.cdp from @mahoraga/shared/constants/ports
const DEFAULT_CDP_PORT = 9010
const REQUEST_TIMEOUT_MS = 30_000
const EXTENSION_ID =
  process.env.MAHORAGA_EXTENSION_ID || 'bflpfmnmnokmjhmgnolecpppdbdophmk'
...
class CDPClient {
  ...
  static async connect(port: number): Promise<CDPClient> {
    ...
    try {
      ...
      throw new Error(
        `Cannot reach CDP at ${versionUrl}. Is Mahoraga running with --cdp-port=${port}?`,
      )
    }
  ...
  if (!sw) {
    throw new Error(
      `No service worker found for extension ${EXTENSION_ID}. ` +
        'Is the Mahoraga agent extension installed and active?',
    )
  }
    ...
    const result = await cdp.send(
      'Runtime.evaluate',
      {
        expression: `(async () => {
          ...
          const result = await chrome.sidePanel.mahoragaToggle({ tabId });
          return { tabId, ...result };
        })()`,
        awaitPromise: true,
        returnByValue: true,
...
Commands:
  ...
  open-sidepanel                       Open the Mahoraga agent side panel

Target resolution:
  <target> can be a numeric index from 'targets' output, or a URL/title substring.

Environment:
  MAHORAGA_CDP_PORT   CDP port (default: ${DEFAULT_CDP_PORT})
...
async function main(): Promise<void> {
  ...
  const port = Number(process.env.MAHORAGA_CDP_PORT) || DEFAULT_CDP_PORT
