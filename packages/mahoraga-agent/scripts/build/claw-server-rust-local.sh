...
usage() {
  cat <<'USAGE'
Usage: claw-server-rust-local.sh [--target darwin-arm64] [--agent-root PATH] [--mahoraga-root PATH]

Build the local Rust BrowserClaw server and stage it in the Mahoraga resource
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --agent-root)
      ...
      ;;
    --mahoraga-root)
      mahoraga_root="${2:?--mahoraga-root requires a value}"
      shift 2
      ;;
    -h|--help)
      usage
...
if [ -z "$mahoraga_root" ]; then
  mahoraga_root="$(cd "$agent_root/../mahoraga" && pwd)"
else
  mahoraga_root="$(cd "$mahoraga_root" && pwd)"
fi

binary_name="mahoraga-claw-server-rs"
cargo_target_dir="${CARGO_TARGET_DIR:-$agent_root/target}"
binary_path="$cargo_target_dir/release/$binary_name"
destination="$mahoraga_root/resources/binaries/mahoraga_claw_server_rust/$target"
...
export MAHORAGA_AGENT_ROOT="$agent_root"
export MAHORAGA_ROOT="$mahoraga_root"
export MAHORAGA_TARGET="$target"
export MAHORAGA_RUST_BINARY="$binary_path"

uv run --project "$mahoraga_root" python <<'PY'
import hashlib
import json
import os
import shutil
...
mahoraga_root = Path(os.environ["MAHORAGA_ROOT"])
target = os.environ["MAHORAGA_TARGET"]
binary_path = Path(os.environ["MAHORAGA_RUST_BINARY"])
destination = (
    mahoraga_root
    / "resources/binaries/mahoraga_claw_server_rust"
    / target
)
stage_binary = destination / "resources/bin/mahoraga-claw-server-rs"
...
with tempfile.TemporaryDirectory() as tmp:
    archive_path = Path(tmp) / "mahoraga-claw-server-rust-resources.zip"
    ...
    expected = ["resources/bin/mahoraga-claw-server-rs"]
