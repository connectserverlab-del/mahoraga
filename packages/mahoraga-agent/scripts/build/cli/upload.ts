...
const CLI_ARCHIVE_PATTERN =
  /^mahoraga-cli_(?<version>[^_]+)_(?<os>darwin|linux|windows)_(?<arch>amd64|arm64)\.(?<ext>tar\.gz|zip)$/
...
async function uploadCliInstallers(rootDir: string): Promise<void> {
  ...
  log.header('Uploading Mahoraga CLI installer scripts')
...
async function uploadCliManifest(
  ...
  const manifestPath = join(tmpdir(), `mahoraga-cli-manifest-${version}.json`)
  await writeFile(
    manifestPath,
...
async function uploadCliRelease(
  ...
  log.header(`Uploading Mahoraga CLI v${version} release`)
  ...
  try {
    ...
    const versionTxtPath = join(tmpdir(), 'mahoraga-cli-version.txt')
