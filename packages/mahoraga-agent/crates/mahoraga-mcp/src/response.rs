...
use mahoraga_core::{ConsoleEntry, PageId, settle::SettleOutcome};
...
impl ToolResponse {
    ...
    async fn run_session_post_action(
        ...
        match action {
            PostAction::Snapshot { page } => {
                ...
                let observer = ctx.session.observe(mahoraga_core::PageId(page)).await;
            ...
            PostAction::Diff {
                ...
                let diff = ctx
                    .session
                    .observe(mahoraga_core::PageId(page))
                    .await
                    .diff()
                    .await?;
                ...
                let origin = match origin {
                    ...
                    None => ctx
                        .session
                        .pages
                        .get_info(mahoraga_core::PageId(page))
                        .await
                        .map(|info| info.url)
            ...
            PostAction::Screenshot { page } => {
                let page_session = ctx
                    .session
                    .pages
                    .get_session(mahoraga_core::PageId(page))
                    .await?;
    ...
    async fn settle_for_post_action(
        ...
        let outcome = tokio::select! {
            () = ctx.cancel.cancelled() => return Err(ToolError::Cancelled),
            outcome = mahoraga_core::settle::wait_for_action_settle(
                ctx.session.pages.as_ref(),
                PageId(page),
                mahoraga_core::timeouts::ACTION_SETTLE_DEFAULT_TIMEOUT,
            ) => outcome,
        };
        match outcome {
