...
(() => {
  ...
  function call(method, args) {
    return __mahoragaCall(method, JSON.stringify(args ?? []));
  }
  ...
  globalThis.__mahoragaBrowser = browser;
  globalThis.__mahoragaConsole = {
    log: sink(''),
    info: sink(''),
    warn: sink('warn: '),
    error: sink('error: '),
  ...
  globalThis.__mahoragaMakeRunFunction = (code) =>
    new AsyncFunction('browser', 'console', `"use strict";\n${code}`);
  globalThis.__mahoragaJsonSafeString = jsonSafeString;
  globalThis.__mahoragaSafeStringify = safeStringify;
})();
"#;
...
impl RunControl {
    ...
    where
        F: Future<Output = Result<T, mahoraga_core::CoreError>>,
    {
        tokio::select! {
...
struct BrowserBridge {
    session: Arc<mahoraga_core::BrowserSession>,
    ...
    let result = context
        .async_with(async |ctx| {
            ...
            let browser: Object<'_> = ctx
                .globals()
                .get("__mahoragaBrowser")
                .catch(&ctx)
                .map_err(|err| RunError::Engine(js_error_message(&ctx, err)))?;
            let console: Object<'_> = ctx
                .globals()
                .get("__mahoragaConsole")
                .catch(&ctx)
                .map_err(|err| RunError::Engine(js_error_message(&ctx, err)))?;
...
fn install_globals<'js>(
    ...
    globals
        .set("__mahoragaCall", Func::from(Async(call_bridge)))
        .catch(ctx)
        .map_err(|err| RunError::Engine(js_error_message(ctx, err)))?;
    globals
        .set("__mahoragaPushLog", Func::from(push_log))
        .catch(ctx)
        .map_err(|err| RunError::Engine(js_error_message(ctx, err)))?;
    Ok(())
}
...
fn refs_json(refs: &mahoraga_core::snapshot::RefMap) -> Value {
    Value::Array(
...
fn diff_json(diff: &mahoraga_core::snapshot::SnapshotDiff) -> Value {
    let mut value = json!({
        "text": diff.text,
        "added": diff.added,
...
mod tests {
    ...
    use mahoraga_core::{BrowserSession, BrowserSessionHooks, CdpConnection, WindowId};
    use futures_util::future::BoxFuture;
    use serde_json::json;
