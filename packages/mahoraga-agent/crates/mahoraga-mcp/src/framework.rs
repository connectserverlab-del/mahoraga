//! Tool registry and execution framework for Mahoraga MCP tools.

use crate::{response::ToolResponse, tools};
use mahoraga_core::{BrowserSession, CoreError, PageId, WindowId};
...
where
    T: JsonSchema + std::any::Any,
{
    let schema = mahoraga_schema_settings()
        .into_generator()
        .into_root_schema_for::<T>();
    let Value::Object(mut object) = serde_json::to_value(schema).unwrap_or_else(|err| {
        panic!("invalid Mahoraga MCP {purpose}: schema should serialize: {err}");
    }) else {
        panic!("invalid Mahoraga MCP {purpose}: schema root should be an object");
    };
    ...
    match object.get("type") {
        ...
        Some(Value::String(schema_type)) => {
            panic!(
                "invalid Mahoraga MCP {purpose}: root type should be object, got {schema_type}"
            );
        }
        Some(schema_type) => {
            panic!(
                "invalid Mahoraga MCP {purpose}: root type should be a string, got {schema_type}"
            );
        }
        None => {
            panic!("invalid Mahoraga MCP {purpose}: root type is missing");
        }
    }
...
fn mahoraga_schema_settings() -> SchemaSettings {
...
fn normalize_schema_object(schema: JsonObject) -> Arc<JsonObject> {
    ...
    if let Some(path) = first_boolean_path(&value) {
        panic!("unsupported boolean value in Mahoraga MCP schema at {path}");
    }
    match value {
        Value::Object(object) => Arc::new(object),
        _ => panic!("Mahoraga MCP schema root should remain an object"),
    }
}
...
#[must_use]
pub fn page_json(page: &mahoraga_core::pages::PageInfo) -> Value {
    let mut value = json!({
