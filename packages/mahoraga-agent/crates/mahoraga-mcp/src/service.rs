//! Thin rmcp service wrapper for the Mahoraga tool catalog.
...
use mahoraga_core::BrowserSession;
...
pub const BROWSER_MCP_INSTRUCTIONS: &str = r#"Mahoraga MCP - you are driving the user's real, live browser.
...
impl BrowserMcpService {
    /// Builds a thin rmcp ServerHandler over the Mahoraga tool catalog.
    #[must_use]
    ...
    async fn execute_registered_tool(
        ...
        let (mut result, error_message) =
            if let Some(browser_session) = self.browser_session().await {
                ...
                (
                    ToolResult::error(
                        "browser not connected (retrying); try again once Mahoraga reconnects",
                    ),
                    None,
                )
            };
