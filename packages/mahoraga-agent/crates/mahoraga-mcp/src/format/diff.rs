...
use mahoraga_core::snapshot::SnapshotDiff;
        ...
        match write_temp_tool_output_file(&ctx.output_files, "diff", "md", &wrapped_diff).await {
            ...
            Err(err) => {
                ...
                let text = if diff.url_changed {
                    format!(
                        "URL changed; full current snapshot is {token_estimate} estimated tokens, over the {MAX_INLINE_DIFF_TOKENS}-token inline limit, but saving it to a Mahoraga output file failed: {save_error}"
                    )
                } else {
                    format!(
                        "Diff is {token_estimate} estimated tokens, over the {MAX_INLINE_DIFF_TOKENS}-token inline limit, but saving it to a Mahoraga output file failed: {save_error}"
                    )
                };
                add_fields(
                    &mut structured,
