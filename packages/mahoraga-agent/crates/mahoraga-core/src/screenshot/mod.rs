...
const OVERLAY_ATTR: &str = "data-mahoraga-screenshot-annotation";
...
async fn inject_annotation_overlay(
    ...
    let expression = OVERLAY_SCRIPT
        ...
        .replace(
            "__MAHORAGA_FULL_PAGE__",
            if full_page { "true" } else { "false" },
        )
        .replace("__MAHORAGA_SCROLL__", &scroll_json.to_string());
    let _: Value = session
        .send(
            "Runtime.evaluate",
