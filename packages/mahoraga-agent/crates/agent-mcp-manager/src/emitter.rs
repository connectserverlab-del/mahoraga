...
mod tests {
    ...
    -> Result<(), Error> {
        let raw = "# model\nmodel = \"gpt-5\"\nmcp_servers = { mahoraga = { url = \"https://old.example/mcp\" } } # inline\n# tail\n";
        let emitter = emitter(AgentId::Codex)?;
        let added = emitter.add(raw, "BrowserClaw", &http())?;
        assert!(added.contains("mahoraga = { url = \"https://old.example/mcp\" }"));
        assert!(added.contains("# inline\n# tail"));
        assert_eq!(emitter.remove(&added, "BrowserClaw")?, raw);

        let removed_only = emitter.remove(raw, "mahoraga")?;
