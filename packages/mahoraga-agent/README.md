...
The agent platform powering [Mahoraga](https://github.com/connectserverlab-del/mahoraga) — contains the MCP server, agent UI, CLI, and shared packages.
...
apps/
  ...
  app/             # Mahoraga app UI (Chrome extension)
  cli/             # Go CLI for controlling Mahoraga from the terminal
packages/
...
| `apps/app` | Mahoraga app UI — Chrome extension for the chat interface |
| `apps/cli` | Go CLI — control Mahoraga from the terminal or AI coding agents |
...
- `apps/app`: Mahoraga app UI (Chrome extension).
...
└──────────────────────────────────────────────────────────────────────────┘
                                    ...
                                    ▼
┌──────────────────────────────────────────────────────────────────────────┐
│                 Mahoraga Server (serverPort: 9100)                      │
│                                                                          │
...
| 9000 | `ports.cdp` | Chromium CDP server (Mahoraga Server connects as client) |
...
Both are gitignored. Their tracked templates, `.env.development.example` and `.env.production.example`, are generated from `@mahoraga/shared/env/registry`; run `bun run env:examples` after changing the registry. CI drift-checks the generated examples.
...
|---------|------|---------|
| `dev-tools` | `.env.development` | Optional codegen and local tooling inputs such as `CDP_PROTOCOL_JSON` and `MAHORAGA_BINARY`. |
| `app` | `.env.development` | Browser extension and local Mahoraga launch settings, including dev ports, public Vite values, source-map upload settings, and optional GraphQL schema path. |
...
Production build and upload scripts read root `.env.production` plus exported process env through the shared loader in `@mahoraga/shared/env/*`; exported process env takes precedence. Missing required values fail with an error naming the key, section, and root file.
...
# App-specific test groups (from packages/mahoraga-agent)
