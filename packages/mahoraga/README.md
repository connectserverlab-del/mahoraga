# Mahoraga Browser (Chromium Fork)
...
This package contains the Mahoraga browser build system — everything needed to fetch Chromium source, apply Mahoraga patches, and produce signed binaries for macOS, Windows, and Linux. The build system is a Python CLI that orchestrates the entire pipeline from source to distributable.

Mahoraga patches add:
...
│   ├── mahoraga.py         # Main app definition
│   ├── modules/
...
│
├── chromium_patches/        # Mahoraga patches applied to Chromium source
...
The `mahoraga` CLI manages the full build lifecycle:
...
```bash
mahoraga setup          # Fetch and prepare Chromium source
mahoraga apply          # Apply all patches to Chromium source
mahoraga build          # Build Mahoraga binary
mahoraga package        # Package into distributable (DMG, installer, AppImage)
mahoraga sign           # Code sign the binary (macOS/Windows)
```

## Patch System

Mahoraga applies patches on top of vanilla Chromium. Patches are organized in two directories:
...
2. Use `mahoraga extract` to pull changes back into patch format
3. Place the patch in the appropriate directory mirroring Chromium's structure
4. Test with a full `mahoraga apply && mahoraga build` cycle
...
- [Mahoraga Agent Platform](../mahoraga-agent/) — the TypeScript/Go agent system that runs inside the browser
