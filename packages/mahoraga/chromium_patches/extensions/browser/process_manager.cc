...
+  if (mahoraga::IsActiveMahoragaExtension(worker_id.extension_id)) {
+    base::Uuid keepalive_uuid = IncrementServiceWorkerKeepaliveCount(
+        worker_id,
+        content::ServiceWorkerExternalRequestTimeoutType::kDoesNotTimeout,
+        Activity::PROCESS_MANAGER, "mahoraga_permanent_keepalive");
+    mahoraga_permanent_keepalives_[worker_id] = keepalive_uuid;
+    VLOG(1) << "mahoraga: Added permanent keepalive for extension "
...
+  // Mahoraga: Clean up permanent keepalive for Mahoraga extensions.
+  auto keepalive_iter = mahoraga_permanent_keepalives_.find(worker_id);
+  if (keepalive_iter != mahoraga_permanent_keepalives_.end()) {
+    DecrementServiceWorkerKeepaliveCount(worker_id, keepalive_iter->second,
+                                         Activity::PROCESS_MANAGER,
+                                         "mahoraga_permanent_keepalive");
+    mahoraga_permanent_keepalives_.erase(keepalive_iter);
+    VLOG(1) << "mahoraga: Removed permanent keepalive for extension "
+            << worker_id.extension_id;
+  }
+
