...
+#include "components/os_crypt/common/mahoraga_product_buildflags.h"
...
+#elif BUILDFLAG(MAHORAGA_PRODUCT_BROWSERCLAW)
+  EXPECT_EQ("BrowserClaw Safe Storage", KeychainPassword::GetServiceName());
+  EXPECT_EQ("BrowserClaw", KeychainPassword::GetAccountName());
+#else
+  EXPECT_EQ("Mahoraga Safe Storage", KeychainPassword::GetServiceName());
+  EXPECT_EQ("Mahoraga", KeychainPassword::GetAccountName());
+#endif
+}
+
 }  // namespace
