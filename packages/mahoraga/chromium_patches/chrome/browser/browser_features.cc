...
+BASE_FEATURE(kMahoragaAlphaFeatures, base::FEATURE_DISABLED_BY_DEFAULT);
+
+// Enables Mahoraga keyboard shortcuts (Option+K, Option+L, Option+A, etc.).
+// Disable this on keyboards where Option+letter produces special characters.
+BASE_FEATURE(kMahoragaKeyboardShortcuts, base::FEATURE_ENABLED_BY_DEFAULT);
+
