...
+}  // namespace
+
+ExtensionFunction::ResponseAction MahoragaGetPrefFunction::Run() {
...
+}
+
+ExtensionFunction::ResponseAction MahoragaSetPrefFunction::Run() {
+  std::optional<browser_os::SetPref::Params> params =
+      browser_os::SetPref::Params::Create(args());
+  EXTENSION_FUNCTION_VALIDATE(params);
+
+  // Security: only allow modifying mahoraga.* prefs
+  if (!params->name.starts_with("mahoraga.")) {
+    return RespondNow(Error("Only mahoraga.* preferences can be modified"));
+  }
+
+  Profile* profile = Profile::FromBrowserContext(browser_context());
+  PrefService* prefs = FindPrefService(params->name, profile);
...
+}
+
+ExtensionFunction::ResponseAction MahoragaLogMetricFunction::Run() {
...
+
+    mahoraga_metrics::MahoragaMetrics::Log(prefixed_event,
+                                             std::move(properties));
+  } else {
+    mahoraga_metrics::MahoragaMetrics::Log(
...
+}
+
+ExtensionFunction::ResponseAction MahoragaGetVersionNumberFunction::Run() {
...
+MahoragaGetMahoragaVersionNumberFunction::Run() {
+  std::string version = std::string(version_info::GetMahoragaVersionNumber());
+
+  return RespondNow(ArgumentList(
+      browser_os::GetMahoragaVersionNumber::Results::Create(version)));
+}
+
+namespace {
+
...
+}  // namespace
+
+MahoragaChoosePathFunction::MahoragaChoosePathFunction() = default;
+
+MahoragaChoosePathFunction::~MahoragaChoosePathFunction() {
...
+}
+
+ExtensionFunction::ResponseAction MahoragaChoosePathFunction::Run() {
...
+}
+
+void MahoragaChoosePathFunction::FileSelected(const ui::SelectedFileInfo& file,
+                                               int index) {
...
+}
+
+void MahoragaChoosePathFunction::FileSelectionCanceled() {
...
+}
+
+ExtensionFunction::ResponseAction MahoragaShowToastFunction::Run() {
...
+  }
+
+  ToastParams toast_params(ToastId::kMahoragaToast);
...
+}
+
+// Bridges chrome.mahoraga.showInfoBar to Chrome's tab-scoped alert infobar.
+ExtensionFunction::ResponseAction MahoragaShowInfoBarFunction::Run() {
...
+                      MAHORAGA_EXTENSION_INFOBAR_DELEGATE,
