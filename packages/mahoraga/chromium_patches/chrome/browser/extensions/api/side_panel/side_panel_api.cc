...
+SidePanelMahoragaToggleFunction::RunFunction() {
+  EXTENSION_FUNCTION_VALIDATE(extension());
+
+  std::optional<api::side_panel::MahoragaToggle::Params> params =
+      api::side_panel::MahoragaToggle::Params::Create(args());
...
+      service->MahoragaToggleSidePanelForTab(
...
+  }
+
+  api::side_panel::MahoragaToggleResult result;
+  result.opened = toggle_result.value();
+  return RespondNow(WithArguments(result.ToValue()));
+}
+
+ExtensionFunction::ResponseAction
+SidePanelMahoragaIsOpenFunction::RunFunction() {
+  EXTENSION_FUNCTION_VALIDATE(extension());
+
+  std::optional<api::side_panel::MahoragaIsOpen::Params> params =
+      api::side_panel::MahoragaIsOpen::Params::Create(args());
+  EXTENSION_FUNCTION_VALIDATE(params);
+
+  SidePanelService* service = GetService();
+  base::expected<bool, std::string> is_open_result =
+      service->MahoragaIsSidePanelOpenForTab(
+          *extension(), browser_context(), params->options.tab_id,
