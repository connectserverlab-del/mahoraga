...
+SidePanelService::MahoragaToggleSidePanelForTab(
+    const Extension& extension,
+    content::BrowserContext* context,
+    int tab_id,
+    bool include_incognito_information,
+    std::optional<bool> desired_state) {
+  LOG(INFO) << "mahoraga: MahoragaToggleSidePanelForTab called for tab_id="
...
+      !window || !web_contents) {
+    LOG(WARNING) << "mahoraga: Tab not found for tab_id=" << tab_id;
...
+  if (!browser_window) {
+    LOG(WARNING) << "mahoraga: No browser window for tab_id=" << tab_id;
...
+  }
+
+  LOG(INFO) << "mahoraga: has_contextual_options=" << has_contextual_options
+            << " for tab_id=" << tab_id;
+
...
+      LOG(WARNING) << "mahoraga: No side panel path configured for extension="
+                   << extension.id();
+      return base::unexpected(
...
+    }
+
+    LOG(INFO) << "mahoraga: Auto-registering contextual panel for tab_id="
...
+    LOG(WARNING) << "mahoraga: Side panel is disabled for tab_id=" << tab_id;
+    return base::unexpected(
+        base::StringPrintf("Side panel is disabled for tabId: %d", tab_id));
+  }
+
+  // Toggle the contextual panel.
+  LOG(INFO) << "mahoraga: Calling ToggleContextualExtensionSidePanel for tab_id="
+            << tab_id;
...
+
+  LOG(INFO) << "mahoraga: Toggle result: is_now_open=" << is_now_open
+            << " for tab_id=" << tab_id;
+
+  return is_now_open;
+}
+
+base::expected<bool, std::string>
+SidePanelService::MahoragaIsSidePanelOpenForTab(
+    const Extension& extension,
+    content::BrowserContext* context,
+    int tab_id,
+    bool include_incognito_information) {
+  LOG(INFO) << "mahoraga: MahoragaIsSidePanelOpenForTab called for tab_id="
+            << tab_id << ", extension=" << extension.id();
+
+  // Find the tab.
+  WindowController* window = nullptr;
...
+  api::side_panel::PanelOptions current_options = GetOptions(extension, tab_id);
+  if (!current_options.enabled.value_or(true)) {
+    LOG(INFO) << "mahoraga: Panel is disabled for tab_id=" << tab_id
+              << ", returning false";
+    return false;
+  }
+
...
+
+  LOG(INFO) << "mahoraga: IsOpen result: is_open=" << is_open
+            << " for tab_id=" << tab_id;
+
+  return is_open;
+}
