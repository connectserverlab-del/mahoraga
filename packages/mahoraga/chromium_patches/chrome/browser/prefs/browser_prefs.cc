...
+#include "chrome/browser/mahoraga/metrics/mahoraga_metrics_prefs.h"
...
+  mahoraga_server::RegisterLocalStatePrefs(registry);
+  mahoraga_metrics::RegisterLocalStatePrefs(registry);
...
+  mahoraga_metrics::RegisterProfilePrefs(registry);
...
+  RegisterMahoragaPrefs(registry);
...
+void RegisterMahoragaPrefs(user_prefs::PrefRegistrySyncable* registry) {
+  mahoraga::RegisterProfilePrefs(registry);
+}
+
 #if BUILDFLAG(IS_CHROMEOS)
