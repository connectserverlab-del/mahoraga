...
+  // the Mahoraga repo; keep the two in lockstep.
+  const char* base_url = mahoraga::IsBrowserClawProduct()
+                             ? "https://cdn.mahoraga.com/appcast-claw"
+                             : "https://cdn.mahoraga.com/appcast";
+
...
+
+// Forward declare internal SparkleGlue methods used by MahoragaUserDriver.
...
+@end
+
+@implementation MahoragaUserDriver
+
...
+@property(nonatomic, strong) MahoragaUserDriver* userDriver;
...
+  _userDriver = [[MahoragaUserDriver alloc] init];
...
+
+#if !defined(OFFICIAL_BUILD)
+  if (cmd->HasSwitch(mahoraga::kSparkleDryRun)) {
+    LOG(WARNING) << "Sparkle: DRY-RUN MODE enabled";
+    _dryRunMode = YES;
+  }
+
+  if (cmd->HasSwitch(mahoraga::kSparkleSpoofVersion)) {
+    std::string version = cmd->GetSwitchValueASCII(mahoraga::kSparkleSpoofVersion);
+    LOG(WARNING) << "Sparkle: Spoofing version as " << version;
+    _spoofedVersion = base::SysUTF8ToNSString(version);
+  }
+
+  if (cmd->HasSwitch(mahoraga::kSparkleVerbose)) {
...
+  auto* cmd = base::CommandLine::ForCurrentProcess();
+  if (cmd && cmd->HasSwitch(mahoraga::kSparkleForceCheck)) {
...
+  auto* cmd = base::CommandLine::ForCurrentProcess();
+  if (cmd && cmd->HasSwitch(mahoraga::kSparkleUrl)) {
+    std::string url = cmd->GetSwitchValueASCII(mahoraga::kSparkleUrl);
...
+  auto* cmd = base::CommandLine::ForCurrentProcess();
+  if (cmd && cmd->HasSwitch(mahoraga::kSparkleSkipSignature)) {
