...
@@ -0,0 +1,377 @@
+// Copyright 2024 Mahoraga Authors. All rights reserved.
...
+// chrome/app/app-Info.plist); the Mahoraga release pipeline signs all
...
+// product half follows mahoraga::GetProduct(). Feed keys are owned by
+// release/feeds/spec.py (_BROWSER_FEED_SLUGS) in the Mahoraga repo; keep
+// the two in lockstep.
+#if defined(ARCH_CPU_ARM64)
+constexpr char kAppcastURL[] =
+    "https://cdn.mahoraga.com/appcast-win-arm64.xml";
+constexpr char kClawAppcastURL[] =
+    "https://cdn.mahoraga.com/appcast-claw-win-arm64.xml";
+#else
+constexpr char kAppcastURL[] = "https://cdn.mahoraga.com/appcast-win.xml";
+constexpr char kClawAppcastURL[] =
+    "https://cdn.mahoraga.com/appcast-claw-win.xml";
+#endif
+
+const char* GetAppcastURL() {
+  return mahoraga::IsBrowserClawProduct() ? kClawAppcastURL : kAppcastURL;
+}
+
+// Matches SUScheduledCheckInterval on macOS; also WinSparkle's minimum.
+constexpr int kUpdateCheckIntervalSeconds = 3600;
+
+// Per-product registry state (skipped version, last check time): Mahoraga
+// and BrowserClaw install side by side, so sharing one path would
+// cross-contaminate their updaters.
+constexpr char kRegistryPath[] = "Software\\Mahoraga\\WinSparkle";
+constexpr char kClawRegistryPath[] = "Software\\BrowserClaw\\WinSparkle";
+
+// Version compared against the appcast's sparkle:version: the Mahoraga
...
+// BUILD.PATCH inflated by MAHORAGA_BUILD_OFFSET (frozen at ~7950.97 at
...
+// Context.get_sparkle_version() (Mahoraga repo, build/common/context.py);
+// all three must stay in lockstep.
+std::string GetUpdateFeedVersion() {
+  return base::StrCat({"10000.", version_info::GetMahoragaVersionNumber()});
+}
+
...
+  }
+
+  const bool is_claw = mahoraga::IsBrowserClawProduct();
+
+  // WinSparkle UI / User-Agent shows the Mahoraga release version;
+  // comparisons use the epoch-prefixed feed version below.
+  const std::wstring display_version =
+      base::UTF8ToWide(version_info::GetMahoragaVersionNumber());
+  win_sparkle_set_app_details(L"Mahoraga",
+                              is_claw ? L"BrowserClaw" : L"Mahoraga",
