...
+  // Mahoraga: Start AFTER CreateDevToolsProtocolHandler so that Mahoraga's
+  // CDP handler replaces Chromium's (StartRemoteDebuggingServer is a global
+  // singleton — the last caller wins).
+  LOG(INFO) << "mahoraga: Starting Mahoraga server process";
+  mahoraga::MahoragaServerManager::GetInstance()->Start();
+
...
+
+  // Mahoraga: Stop the Mahoraga server during shutdown
+  LOG(INFO) << "mahoraga: Stopping Mahoraga server process";
+  mahoraga::MahoragaServerManager::GetInstance()->Shutdown();
+
