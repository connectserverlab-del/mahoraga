diff --git a/chrome/browser/ui/views/side_panel/mahoraga_simple_page_extractor.h b/chrome/browser/ui/views/side_panel/mahoraga_simple_page_extractor.h
new file mode 100644
index 0000000000000..1fed9958dcfcb
--- /dev/null
+++ b/chrome/browser/ui/views/side_panel/mahoraga_simple_page_extractor.h
@@ -0,0 +1,71 @@
+// Copyright 2025 The Chromium Authors
+// Use of this source code is governed by a BSD-style license that can be
+// found in the LICENSE file.
+
+#ifndef CHROME_BROWSER_UI_VIEWS_SIDE_PANEL_MAHORAGA_SIMPLE_PAGE_EXTRACTOR_H_
+#define CHROME_BROWSER_UI_VIEWS_SIDE_PANEL_MAHORAGA_SIMPLE_PAGE_EXTRACTOR_H_
+
+#include <string>
+
+namespace ui {
...
+// Mahoraga LLM features (LLM Chat).
+//
...
+//         std::u16string text = side_panel::MahoragaSimplePageExtractor
+//             ::ExtractStructuredText(update);
+//         // Use extracted text...
+//       }), ...);
+//
+class MahoragaSimplePageExtractor {
+ public:
...
+  MahoragaSimplePageExtractor() = delete;
+  ~MahoragaSimplePageExtractor() = delete;
+  MahoragaSimplePageExtractor(const MahoragaSimplePageExtractor&) = delete;
+  MahoragaSimplePageExtractor& operator=(const MahoragaSimplePageExtractor&) = delete;
+};
+
+}  // namespace side_panel
+
+#endif  // CHROME_BROWSER_UI_VIEWS_SIDE_PANEL_MAHORAGA_SIMPLE_PAGE_EXTRACTOR_H_
