...
+
+// Preference names
+const char kThirdPartyLlmProvidersPref[] = "mahoraga.third_party_llm.providers";
+const char kThirdPartyLlmSelectedProviderPref[] = "mahoraga.third_party_llm.selected_provider";
+
...
+    LOG(INFO) << "[mahoraga] Provider list size changed from " << previous_size
...
+  
+  mahoraga_metrics::MahoragaMetrics::Log("llmchat.created");
+  
+  return container;
+}
+
...
+  if (!prefs) {
+    LOG(ERROR) << "[mahoraga] Failed to get PrefService";
...
+      if (!item.is_dict()) {
+        LOG(WARNING) << "[mahoraga] Invalid provider entry (not a dict), skipping";
+        continue;
+      }
+
...
+
+      if (!name || name->empty()) {
+        LOG(WARNING) << "[mahoraga] Provider missing name, skipping";
+        continue;
+      }
+
+      if (!url || url->empty()) {
+        LOG(WARNING) << "[mahoraga] Provider missing URL, skipping";
+        continue;
+      }
+
+      GURL provider_url(*url);
+      if (!provider_url.is_valid()) {
+        LOG(WARNING) << "[mahoraga] Invalid provider URL: " << *url;
+        continue;
+      }
+
...
+  if (providers_.empty()) {
+    LOG(INFO) << "[mahoraga] No providers in prefs, using defaults";
+    providers_ = GetDefaultProviders();
...
+    LOG(WARNING) << "[mahoraga] Invalid selected provider index: " << selected_index
+                 << ", resetting to 0";
+    current_provider_index_ = 0;
+    prefs->SetInteger(kThirdPartyLlmSelectedProviderPref, 0);
+  } else {
...
+  PrefService* prefs = GetProfile()->GetPrefs();
+  if (!prefs) {
+    LOG(ERROR) << "[mahoraga] Failed to get PrefService for saving";
+    return;
+  }
+
...
+    LOG(ERROR) << "[mahoraga] Invalid provider index: " << new_provider_index;
+    return;
+  }
+
+  provider_change_in_progress_ = true;
+
+  mahoraga_metrics::MahoragaMetrics::Log("llmchat.provider.changed");
+
+  if (owned_web_contents_) {
+    GURL current_url = owned_web_contents_->GetURL();
+    if (current_url.is_valid()) {
...
+  
+  mahoraga_metrics::MahoragaMetrics::Log("llmchat.screenshot.captured");
+
...
+    
+    mahoraga_metrics::MahoragaMetrics::Log("llmchat.content.copied");
+    
...
+  LOG(INFO) << "[mahoraga] Media access requested for origin: "
...
+
+    LOG(INFO) << "[mahoraga] Auto-granting media permission for trusted LLM: "
+              << origin_url.host();
+
...
+      VLOG(2) << "[mahoraga] Media permission check for trusted LLM origin: "
...
+
+  VLOG(2) << "[mahoraga] Media permission check for origin: "
+          << security_origin.Serialize()
+          << " type: " << static_cast<int>(type)
