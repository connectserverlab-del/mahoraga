...
+  LOG(INFO) << "mahoraga: IsContextualExtensionSidePanelOpen called for extension="
+            << extension_id;
+
+  if (!browser_window || !web_contents) {
+    LOG(WARNING) << "mahoraga: browser_window or web_contents is null";
+    return false;
+  }
+
...
+  LOG(INFO) << "mahoraga: is_active_tab=" << is_active_tab;
+
...
+    LOG(INFO) << "mahoraga: side_panel is_showing=" << is_showing;
+    if (!is_showing) {
+      return false;
+    }
...
+    LOG(INFO) << "mahoraga: contextual panel is_active=" << is_active;
+    return is_active;
+  }
+
...
+      SidePanelRegistry::From(tab);
+  bool is_active = IsKeyActiveInRegistry(contextual_registry, extension_key);
+  LOG(INFO) << "mahoraga: inactive tab contextual panel is_active=" << is_active;
+  return is_active;
+}
+
...
+  LOG(INFO) << "mahoraga: ToggleContextualExtensionSidePanel called for extension="
...
+  }
+
+  LOG(INFO) << "mahoraga: is_currently_open=" << is_currently_open
...
+  }
+
+  LOG(INFO) << "mahoraga: should_open=" << should_open;
+
+  // If already in desired state, return early.
+  if (should_open == is_currently_open) {
+    LOG(INFO) << "mahoraga: Already in desired state, no action needed";
+    return is_currently_open;
+  }
+
+  if (!should_open) {
+    LOG(INFO) << "mahoraga: Closing contextual panel";
...
+    return false;
+  } else {
+    LOG(INFO) << "mahoraga: Opening contextual panel";
+
+    SidePanelEntry* contextual_entry =
+        contextual_registry->GetEntryForKey(extension_key);
+    LOG(INFO) << "mahoraga: Got contextual_entry: "
+              << (contextual_entry ? "yes" : "no");
+
+    if (!contextual_entry) {
+      LOG(WARNING) << "mahoraga: No contextual entry found, cannot open";
+      return false;
+    }
+
...
+
+    if (is_active_tab) {
+      LOG(INFO) << "mahoraga: Calling side_panel_ui->Show() for active tab";
