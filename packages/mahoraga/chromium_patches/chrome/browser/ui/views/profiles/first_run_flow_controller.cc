...
+class MahoragaOnboardingStepController
+    : public ProfileManagementStepController {
+ public:
+  MahoragaOnboardingStepController(ProfilePickerWebContentsHost* host,
+                                    base::RepeatingClosure completion_callback)
+      : ProfileManagementStepController(host),
+        completion_callback_(std::move(completion_callback)) {}
+
+  ~MahoragaOnboardingStepController() override = default;
+
+  void Show(StepSwitchFinishedCallback step_shown_callback,
+            bool reset_state) override {
+    if (reset_state) {
+      host()->ShowScreenInPickerContents(
+          GURL(chrome::kChromeUIMahoragaOnboardingURL),
+          base::BindOnce(&MahoragaOnboardingStepController::OnLoaded,
+                         weak_ptr_factory_.GetWeakPtr(),
+                         std::move(step_shown_callback)));
+      return;
+    }
+
+    DCHECK_EQ(GURL(chrome::kChromeUIMahoragaOnboardingURL),
...
+
+  base::WeakPtrFactory<MahoragaOnboardingStepController> weak_ptr_factory_{
+      this};
+};
+
...
@@ -539,16 +595,12 @@ void FirstRunFlowController::ShowSigninError(Profile* profile,
 void FirstRunFlowController::Init() {
   RegisterStep(
       Step::kIntro,
-      CreateIntroStep(
+      std::make_unique<MahoragaOnboardingStepController>(
           host(),
-          base::BindRepeating(&FirstRunFlowController::HandleIntroSigninChoice,
-                              weak_ptr_factory_.GetWeakPtr()),
-          /*enable_animations=*/true));
+          base::BindRepeating(
+              &FirstRunFlowController::HandleMahoragaOnboardingComplete,
+              weak_ptr_factory_.GetWeakPtr())));
...
+void FirstRunFlowController::HandleMahoragaOnboardingComplete() {
+  mahoraga::onboarding::MarkCompleted(profile_);
