...
+  // Set text from action item if available for Mahoraga actions
+  if (auto* action_item = container_->GetActionItemFor(action_id)) {
+    if (mahoraga::IsMahoragaAction(action_id)) {
+      // Check if labels should be shown
+      bool show_labels = true;
+      if (browser_ && browser_->profile()) {
+        show_labels = mahoraga::ShouldShowToolbarLabels(
...
+  if (!mahoraga::IsMahoragaAction(action_id_)) {
+    return;
+  }
+
...
+  if (browser_ && browser_->profile()) {
+    show_labels =
+        mahoraga::ShouldShowToolbarLabels(browser_->profile()->GetPrefs());
+  }
+
+  if (show_labels) {
+    views::LabelButton::SetText(action_item->GetText());
...
+  // Update the text from the action item for Mahoraga actions
+  if (mahoraga::IsMahoragaAction(action_view_->GetActionId())) {
+    // Check if labels should be shown
+    bool show_labels = true;
