...
+                       MahoragaAssistantVisibilityPrefControlsPinnedState) {
+  PrefService* prefs = browser()->profile()->GetPrefs();
+  EXPECT_TRUE(prefs->GetBoolean(mahoraga::prefs::kShowAssistant));
+
+  model()->EnsureAlwaysPinnedActions();
+  EXPECT_TRUE(model()->Contains(kActionMahoragaAgent));
+
+  prefs->SetBoolean(mahoraga::prefs::kShowAssistant, false);
+  EXPECT_FALSE(model()->Contains(kActionMahoragaAgent));
+
+  prefs->SetBoolean(mahoraga::prefs::kShowAssistant, true);
+  EXPECT_TRUE(model()->Contains(kActionMahoragaAgent));
+}
+
