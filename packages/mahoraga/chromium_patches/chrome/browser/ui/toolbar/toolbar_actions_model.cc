...
+  // Check if it's a Mahoraga extension
+  if (mahoraga::IsMahoragaPinnedExtension(action_id)) {
+    return true;
+  }
+
   auto* management =
...
+       mahoraga::GetActiveMahoragaExtensionIds()) {
+    if (mahoraga::IsMahoragaPinnedExtension(ext_id) &&
