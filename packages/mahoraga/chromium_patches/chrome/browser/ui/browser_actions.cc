...
+  }
+
+  if (mahoraga::IsActiveMahoragaExtension(mahoraga::kAgentExtensionId)) {
+    root_action_item_->AddChild(
...
+                    LOG(WARNING) << "mahoraga: No active tab for Agent action";
+                    return;
+                  }
+
...
+                          ->enabled_extensions()
+                          .GetByID(mahoraga::kAgentExtensionId);
+                  if (!extension) {
+                    LOG(WARNING) << "mahoraga: Agent extension not found";
...
+                              MAHORAGA_AGENT_INSTALLING_INFOBAR_DELEGATE,
+                          nullptr,
+                          u"Mahoraga Agent is installing/updating. Please try "
+                          u"again shortly.",
+                          /*auto_expire=*/true,
+                          /*should_animate=*/true,
+                          /*closeable=*/true);
...
+                  LOG(INFO) << "mahoraga: Agent toolbar action for tab_id="
+                            << tab_id;
+
+                  extensions::SidePanelService* service =
+                      extensions::SidePanelService::Get(profile);
+                  if (!service) {
+                    LOG(WARNING) << "mahoraga: SidePanelService not found";
+                    return;
+                  }
+
+                  auto result = service->MahoragaToggleSidePanelForTab(
...
+                  if (!result.has_value()) {
+                    LOG(WARNING)
+                        << "mahoraga: Agent toggle failed: " << result.error();
+                  } else {
+                    LOG(INFO)
+                        << "mahoraga: Agent toggle result: " << result.value();
+                  }
+                },
+                bwi))
+            .SetActionId(kActionMahoragaAgent)
+            .SetText(u"Assistant")
+            .SetTooltipText(u"Ask Mahoraga")
