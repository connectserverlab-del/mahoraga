...
+  mahoraga::SyncShowTabGroupsInBookmarkBarPref(profile_->GetPrefs());
+
...
+  profile_pref_registrar_.Add(
+      mahoraga::prefs::kShowTabGroupsInBookmarkBar,
+      base::BindRepeating(&mahoraga::ApplyShowTabGroupsInBookmarkBarPref,
...
+
+  // Mahoraga: Check once so the per-URL gates below can use it.
+  const bool ntp_focus_content =
+      mahoraga::IsNtpFocusContentEnabled(profile_->GetPrefs());
+
   if (entry) {
