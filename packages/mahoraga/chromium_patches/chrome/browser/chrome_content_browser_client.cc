...
+// Handles chrome://mahoraga/* URLs by rewriting to extension URLs.
+// Forward handler: chrome://mahoraga/ai ->
+// chrome-extension://[id]/options.html
+static bool HandleMahoragaURL(GURL* url,
+                               content::BrowserContext* browser_context) {
+  if (!url->SchemeIs(content::kChromeUIScheme) ||
+      url->host() != mahoraga::kMahoragaHost) {
+    return false;
+  }
+
+  std::string extension_url = mahoraga::GetMahoragaExtensionURL(url->path());
...
+// chrome://mahoraga/ai This ensures the virtual URL is shown in the address
+// bar.
+static bool ReverseMahoragaURL(GURL* url,
+                                content::BrowserContext* browser_context) {
...
+      mahoraga::GetMahoragaVirtualURL(url->host(), url->path(), url->ref());
...
+  // Handler to rewrite chrome://mahoraga/* to extension URLs.
+  handler->AddHandlerPair(&HandleMahoragaURL, &ReverseMahoragaURL);
...
+#if BUILDFLAG(ENABLE_EXTENSIONS_CORE)
+  // Allow Mahoraga extensions to access private networks (e.g., localhost).
+  // This enables extension service workers to connect to local servers.
+  if (origin.scheme() == extensions::kExtensionScheme &&
+      mahoraga::IsActiveMahoragaExtension(origin.host())) {
