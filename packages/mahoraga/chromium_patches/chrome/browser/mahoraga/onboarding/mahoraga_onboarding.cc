diff --git a/chrome/browser/mahoraga/onboarding/mahoraga_onboarding.cc b/chrome/browser/mahoraga/onboarding/mahoraga_onboarding.cc
new file mode 100644
index 0000000000000..372f36bcb4864f440e38a461e9cf4ab982b191dd
--- /dev/null
+++ b/chrome/browser/mahoraga/onboarding/mahoraga_onboarding.cc
@@ -0,0 +1,605 @@
...
+#include "chrome/grit/mahoraga_onboarding_resources.h"
+#include "chrome/grit/mahoraga_onboarding_resources_map.h"
...
+namespace {
+
+constexpr int kMahoragaOnboardingApiVersion = 1;
+constexpr uint16_t kMahoragaImportableItems =
...
+}  // namespace
+
+class MahoragaOnboardingHandler : public content::WebUIMessageHandler,
+                                   public importer::ImporterProgressObserver {
+ public:
+  MahoragaOnboardingHandler() = default;
+  MahoragaOnboardingHandler(const MahoragaOnboardingHandler&) = delete;
+  MahoragaOnboardingHandler& operator=(const MahoragaOnboardingHandler&) =
+      delete;
+  ~MahoragaOnboardingHandler() override {
+    if (importer_host_) {
...
+        "mahoragaOnboardingPageReady",
+        base::BindRepeating(&MahoragaOnboardingHandler::HandlePageReady,
+                            base::Unretained(this)));
+    web_ui()->RegisterMessageCallback(
+        "mahoragaOnboardingRefreshSources",
+        base::BindRepeating(&MahoragaOnboardingHandler::HandleRefreshSources,
+                            base::Unretained(this)));
+    web_ui()->RegisterMessageCallback(
+        "mahoragaOnboardingStartImport",
+        base::BindRepeating(&MahoragaOnboardingHandler::HandleStartImport,
+                            base::Unretained(this)));
+    web_ui()->RegisterMessageCallback(
+        "mahoragaOnboardingComplete",
+        base::BindRepeating(&MahoragaOnboardingHandler::HandleComplete,
+                            base::Unretained(this)));
+  }
+
...
+        base::BindOnce(&MahoragaOnboardingHandler::HandleSourcesDetected,
+                       base::Unretained(this)));
+  }
+
...
+        source_profile.services_supported & kMahoragaImportableItems;
...
+      state.Set("apiVersion", kMahoragaOnboardingApiVersion);
...
+      }
+      CallJavascriptFunction("mahoragaOnboarding.receiveState", state);
+    }
+  }
+
...
+};
+
+MahoragaOnboardingUIConfig::MahoragaOnboardingUIConfig()
+    : DefaultWebUIConfig(content::kChromeUIScheme,
+                         chrome::kChromeUIMahoragaOnboardingHost) {}
+
+MahoragaOnboarding::MahoragaOnboarding(content::WebUI* web_ui)
+    : content::WebUIController(web_ui) {
+  content::WebUIDataSource* source = content::WebUIDataSource::CreateAndAdd(
+      Profile::FromWebUI(web_ui), chrome::kChromeUIMahoragaOnboardingHost);
+  webui::SetupWebUIDataSource(source, kMahoragaOnboardingResources,
+                              IDR_MAHORAGA_ONBOARDING_INDEX_HTML);
+
+  auto handler = std::make_unique<MahoragaOnboardingHandler>();
+  handler_ = handler.get();
+  web_ui->AddMessageHandler(std::move(handler));
+}
+
+MahoragaOnboarding::~MahoragaOnboarding() = default;
+
+void MahoragaOnboarding::SetCompletionCallback(
...
+}
+
+WEB_UI_CONTROLLER_TYPE_IMPL(MahoragaOnboarding)
