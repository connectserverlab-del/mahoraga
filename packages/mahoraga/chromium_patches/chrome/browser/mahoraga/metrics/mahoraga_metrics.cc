diff --git a/chrome/browser/mahoraga/metrics/mahoraga_metrics.cc b/chrome/browser/mahoraga/metrics/mahoraga_metrics.cc
new file mode 100644
index 0000000000000..a8e149094bdb5
--- /dev/null
+++ b/chrome/browser/mahoraga/metrics/mahoraga_metrics.cc
@@ -0,0 +1,100 @@
...
+MahoragaMetricsService* GetMetricsService() {
...
+  return MahoragaMetricsServiceFactory::GetForBrowserContext(profile);
+}
+
...
+  } else {
+    VLOG(1) << "mahoraga: Metrics service not available for event: " << event_name;
+  }
+}
+
+}  // namespace
+
+// static
+void MahoragaMetrics::Log(const std::string& event_name, double sample_rate) {
+  Log(event_name, base::DictValue(), sample_rate);
+}
+
+// static
+void MahoragaMetrics::Log(const std::string& event_name,
...
+
+// static
+void MahoragaMetrics::Log(const std::string& event_name, base::DictValue properties,
+                           double sample_rate) {
