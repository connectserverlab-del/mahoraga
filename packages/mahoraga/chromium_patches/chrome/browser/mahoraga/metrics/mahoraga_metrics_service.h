diff --git a/chrome/browser/mahoraga/metrics/mahoraga_metrics_service.h b/chrome/browser/mahoraga/metrics/mahoraga_metrics_service.h
new file mode 100644
index 0000000000000..028ec211e2f06
--- /dev/null
+++ b/chrome/browser/mahoraga/metrics/mahoraga_metrics_service.h
@@ -0,0 +1,96 @@
+// Copyright 2025 The Chromium Authors
+// Use of this source code is governed by a BSD-style license that can be
+// found in the LICENSE file.
+
+#ifndef CHROME_BROWSER_MAHORAGA_METRICS_MAHORAGA_METRICS_SERVICE_H_
+#define CHROME_BROWSER_MAHORAGA_METRICS_MAHORAGA_METRICS_SERVICE_H_
+
+#include <memory>
+#include <optional>
+#include <string>
...
+class MahoragaMetricsService : public KeyedService {
+ public:
+  explicit MahoragaMetricsService(
+      PrefService* pref_service,
+      PrefService* local_state_prefs,
+      scoped_refptr<network::SharedURLLoaderFactory> url_loader_factory);
+
+  MahoragaMetricsService(const MahoragaMetricsService&) = delete;
+  MahoragaMetricsService& operator=(const MahoragaMetricsService&) = delete;
+
+  ~MahoragaMetricsService() override;
+
...
+  base::WeakPtrFactory<MahoragaMetricsService> weak_factory_{this};
+};
+
+}  // namespace mahoraga_metrics
+
+#endif  // CHROME_BROWSER_MAHORAGA_METRICS_MAHORAGA_METRICS_SERVICE_H_
