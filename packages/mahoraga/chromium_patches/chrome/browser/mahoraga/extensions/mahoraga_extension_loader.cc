diff --git a/chrome/browser/mahoraga/extensions/mahoraga_extension_loader.cc b/chrome/browser/mahoraga/extensions/mahoraga_extension_loader.cc
new file mode 100644
index 0000000000000..f3665bd1ba8d9
--- /dev/null
+++ b/chrome/browser/mahoraga/extensions/mahoraga_extension_loader.cc
@@ -0,0 +1,279 @@
...
+namespace mahoraga {
+
+MahoragaExtensionLoader::MahoragaExtensionLoader(Profile* profile)
+    : profile_(profile) {
+  config_url_ =
+      GURL(base::FeatureList::IsEnabled(features::kMahoragaAlphaFeatures)
+               ? kMahoragaAlphaConfigUrl
+               : kMahoragaConfigUrl);
+
...
+}
+
+MahoragaExtensionLoader::~MahoragaExtensionLoader() = default;
+
+void MahoragaExtensionLoader::SetConfigUrl(const GURL& url) {
+  config_url_ = url;
+}
+
+void MahoragaExtensionLoader::StartLoading() {
+  LOG(INFO) << "mahoraga: Extension loader starting";
+
+  installer_ = std::make_unique<MahoragaExtensionInstaller>(profile_);
+  maintainer_ = std::make_unique<MahoragaExtensionMaintainer>(profile_);
+
+  installer_->StartInstallation(
+      config_url_, base::BindOnce(&MahoragaExtensionLoader::OnInstallComplete,
+                                  weak_ptr_factory_.GetWeakPtr()));
+}
+
+void MahoragaExtensionLoader::OnInstallComplete(InstallResult result) {
+  LOG(INFO) << "mahoraga: OnInstallComplete from_bundled="
...
+    LOG(WARNING) << "mahoraga: Install returned empty prefs, "
+                 << "reconstructing from installed extensions";
+    prefs_to_load = ReconstructPrefsFromInstalledExtensions();
+    LOG(INFO) << "mahoraga: Reconstructed prefs for " << prefs_to_load.size()
...
+
+base::DictValue
+MahoragaExtensionLoader::ReconstructPrefsFromInstalledExtensions() {
...
+
+    LOG(INFO) << "mahoraga: Reconstructed pref for installed extension " << id
...
+}
+
+const base::FilePath MahoragaExtensionLoader::GetBaseCrxFilePath() {
+  return bundled_crx_base_path_;
+}
+
+void MahoragaExtensionLoader::OnStartupComplete(bool from_bundled) {
+  LOG(INFO) << "mahoraga: Startup complete (from_bundled=" << from_bundled
+            << ")";
+
...
+        FROM_HERE,
+        base::BindOnce(&MahoragaExtensionLoader::InstallBundledExtensionsNow,
+                       weak_ptr_factory_.GetWeakPtr()));
+  } else {
+    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
+        FROM_HERE,
+        base::BindOnce(&MahoragaExtensionLoader::InstallRemoteExtensionsNow,
...
+}
+
+void MahoragaExtensionLoader::InstallRemoteExtensionsNow(
...
+  }
+
+  LOG(INFO) << "mahoraga: Installing " << extension_ids_.size()
...
+}
+
+void MahoragaExtensionLoader::InstallBundledExtensionsNow() {
...
+
+    if (installed) {
+      LOG(INFO) << "mahoraga: Upgrading " << id << " v"
+                << installed->version().GetString() << " -> v" << it->second
+                << " from bundled CRX";
+    } else {
+      LOG(INFO) << "mahoraga: Installing bundled " << id << " v" << it->second;
+    }
+
