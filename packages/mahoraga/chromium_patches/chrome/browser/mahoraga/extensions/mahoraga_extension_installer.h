diff --git a/chrome/browser/mahoraga/extensions/mahoraga_extension_installer.h b/chrome/browser/mahoraga/extensions/mahoraga_extension_installer.h
new file mode 100644
index 0000000000000..944dc8fa738b6
--- /dev/null
+++ b/chrome/browser/mahoraga/extensions/mahoraga_extension_installer.h
@@ -0,0 +1,100 @@
+// Copyright 2024 The Chromium Authors
+// Use of this source code is governed by a BSD-style license that can be
+// found in the LICENSE file.
+
+#ifndef CHROME_BROWSER_MAHORAGA_EXTENSIONS_MAHORAGA_EXTENSION_INSTALLER_H_
+#define CHROME_BROWSER_MAHORAGA_EXTENSIONS_MAHORAGA_EXTENSION_INSTALLER_H_
+
+#include <memory>
+#include <optional>
+#include <set>
...
+};
+
+// Handles one-time initial installation of Mahoraga extensions.
+// Tries bundled CRX files first, falls back to remote config.
+class MahoragaExtensionInstaller {
+ public:
+  using InstallCompleteCallback =
+      base::OnceCallback<void(InstallResult result)>;
+
+  explicit MahoragaExtensionInstaller(Profile* profile);
+  ~MahoragaExtensionInstaller();
+
+  MahoragaExtensionInstaller(const MahoragaExtensionInstaller&) = delete;
+  MahoragaExtensionInstaller& operator=(const MahoragaExtensionInstaller&) =
+      delete;
+
...
+
+  base::WeakPtrFactory<MahoragaExtensionInstaller> weak_ptr_factory_{this};
+};
+
+}  // namespace mahoraga
+
+#endif  // CHROME_BROWSER_MAHORAGA_EXTENSIONS_MAHORAGA_EXTENSION_INSTALLER_H_
