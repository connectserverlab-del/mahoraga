diff --git a/chrome/browser/mahoraga/extensions/mahoraga_extension_maintainer.cc b/chrome/browser/mahoraga/extensions/mahoraga_extension_maintainer.cc
new file mode 100644
index 0000000000000..568aba57b6197
--- /dev/null
+++ b/chrome/browser/mahoraga/extensions/mahoraga_extension_maintainer.cc
@@ -0,0 +1,440 @@
...
+    net::DefineNetworkTrafficAnnotation("mahoraga_extension_maintenance", R"(
+        semantics {
+          sender: "Mahoraga Extension Maintainer"
+          description:
+            "Fetches JSON configuration for Mahoraga extension maintenance."
...
+}  // namespace
+
+MahoragaExtensionMaintainer::MahoragaExtensionMaintainer(Profile* profile)
+    : profile_(profile) {}
+
+MahoragaExtensionMaintainer::~MahoragaExtensionMaintainer() = default;
+
+void MahoragaExtensionMaintainer::Start(const GURL& config_url,
+                                         std::set<std::string> extension_ids,
+                                         base::DictValue initial_config) {
+  config_url_ = config_url;
+  extension_ids_.clear();
+  for (const std::string& id : extension_ids) {
+    if (IsActiveMahoragaExtension(id)) {
+      extension_ids_.insert(id);
+    }
+  }
+  last_config_ = std::move(initial_config);
+
+  LOG(INFO) << "mahoraga: Maintainer started, " << extension_ids_.size()
+            << " extensions, scheduling in "
+            << kInitialMaintenanceDelay.InSeconds() << "s";
+
+  base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(
+      FROM_HERE,
+      base::BindOnce(&MahoragaExtensionMaintainer::RunMaintenanceCycle,
+                     weak_ptr_factory_.GetWeakPtr()),
+      kInitialMaintenanceDelay);
+}
+
+void MahoragaExtensionMaintainer::UpdateExtensionIds(
...
+}
+
+void MahoragaExtensionMaintainer::RunMaintenanceCycle() {
+  if (!profile_) {
...
+      base::BindOnce(&MahoragaExtensionMaintainer::OnConfigFetched,
+                     weak_ptr_factory_.GetWeakPtr(), std::move(loader)));
+}
+
+void MahoragaExtensionMaintainer::OnConfigFetched(
...
+      }
+
+      LOG(INFO) << "mahoraga: Updated config with " << last_config_.size()
+                << " extensions";
+    } else {
+      LOG(WARNING) << "mahoraga: Fetched config parsed as empty";
+    }
+  } else {
+    LOG(WARNING) << "mahoraga: Failed to fetch maintenance config";
+  }
+
+  ExecuteMaintenanceTasks();
+  ScheduleNextMaintenance();
+}
+
+base::DictValue MahoragaExtensionMaintainer::ParseConfigJson(
...
+}
+
+void MahoragaExtensionMaintainer::ExecuteMaintenanceTasks() {
+  LOG(INFO) << "mahoraga: Executing maintenance tasks";
+
...
+}
+
+void MahoragaExtensionMaintainer::ScheduleNextMaintenance() {
+  base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(
+      FROM_HERE,
+      base::BindOnce(&MahoragaExtensionMaintainer::RunMaintenanceCycle,
+                     weak_ptr_factory_.GetWeakPtr()),
...
+}
+
+void MahoragaExtensionMaintainer::UninstallInactiveProductExtensions() {
+  if (!profile_) {
+    return;
+  }
+
...
+  }
+
+  for (const std::string& id : GetAllMahoragaExtensionIds()) {
+    if (IsActiveMahoragaExtension(id)) {
+      continue;
+    }
+
...
+    }
+
+    LOG(INFO) << "mahoraga: Uninstalling inactive product extension " << id;
+
+    std::u16string error;
+    if (!registrar->UninstallExtension(
+            id, extensions::UNINSTALL_REASON_ORPHANED_EXTERNAL_EXTENSION,
+            &error)) {
+      LOG(WARNING) << "mahoraga: Failed to uninstall " << id << ": " << error;
+    }
+  }
+}
+
+void MahoragaExtensionMaintainer::UninstallDeprecatedExtensions() {
...
+    }
+
+    LOG(INFO) << "mahoraga: Uninstalling deprecated extension " << id;
+
+    std::u16string error;
+    if (!registrar->UninstallExtension(
+            id, extensions::UNINSTALL_REASON_ORPHANED_EXTERNAL_EXTENSION,
...
+}
+
+void MahoragaExtensionMaintainer::ReinstallMissingExtensions() {
+  if (!profile_ || last_config_.empty()) {
+    return;
+  }
+
...
+    }
+
+    LOG(INFO) << "mahoraga: Reinstalling missing extension " << id;
+
...
+}
+
+void MahoragaExtensionMaintainer::ReenableDisabledExtensions() {
+  if (!profile_) {
+    return;
+  }
+
...
+    }
+
+    LOG(INFO) << "mahoraga: Re-enabling disabled extension " << id;
+    registrar->EnableExtension(id);
+  }
+}
+
+void MahoragaExtensionMaintainer::ForceUpdateCheck() {
...
+  }
+
+  LOG(INFO) << "mahoraga: Force update check for " << extension_ids_.size()
+            << " extensions";
+
+  for (const std::string& id : extension_ids_) {
...
+    if (ext) {
+      LOG(INFO) << "mahoraga: ext=" << id << " v"
+                << ext->version().GetString();
+    } else {
+      LOG(INFO) << "mahoraga: ext=" << id << " not installed";
+    }
+  }
+
+  extensions::ExtensionUpdater::CheckParams params;
...
+}
+
+void MahoragaExtensionMaintainer::LogExtensionHealth(
...
+
+    mahoraga_metrics::MahoragaMetrics::Log("ota.extension.unexpected_state",
+                                             std::move(properties));
+
+    LOG(WARNING) << "mahoraga: Extension " << id << " in state: " << state
+                 << " (context: " << context << ")";
+  }
+}
+
