diff --git a/chrome/browser/mahoraga/server/mahoraga_server_manager.h b/chrome/browser/mahoraga/server/mahoraga_server_manager.h
new file mode 100644
index 0000000000000..36c2cc1a7f1e0
--- /dev/null
+++ b/chrome/browser/mahoraga/server/mahoraga_server_manager.h
@@ -0,0 +1,169 @@
+// Copyright 2024 The Chromium Authors
+// Use of this source code is governed by a BSD-style license that can be
+// found in the LICENSE file.
+
+#ifndef CHROME_BROWSER_MAHORAGA_SERVER_MAHORAGA_SERVER_MANAGER_H_
+#define CHROME_BROWSER_MAHORAGA_SERVER_MAHORAGA_SERVER_MANAGER_H_
+
+#include <memory>
+#include <set>
+
...
+class MahoragaServerUpdater;
+}
+
+namespace mahoraga {
+class MahoragaServerProxy;
+class HealthChecker;
...
+namespace mahoraga {
+
+// Manages the selected Mahoraga product server process.
+class MahoragaServerManager {
+ public:
+  // Production singleton (uses real implementations)
+  static MahoragaServerManager* GetInstance();
+
+  // Test constructor (dependency injection)
+  MahoragaServerManager(std::unique_ptr<ProcessController> process_controller,
...
+
+  MahoragaServerManager(const MahoragaServerManager&) = delete;
+  MahoragaServerManager& operator=(const MahoragaServerManager&) = delete;
+
+  void Start();
+  void Stop();
...
+
+  base::FilePath GetMahoragaServerExecutablePath() const;
+  base::FilePath GetMahoragaServerResourcesPath() const;
+
...
+
+ private:
+  friend base::NoDestructor<MahoragaServerManager>;
+
+  MahoragaServerManager();
+  ~MahoragaServerManager();
+
+  bool AcquireLock();
...
+
+  void LaunchMahoragaProcess();
+  void OnProcessLaunched(LaunchResult result);
+
+  void TerminateMahoragaProcess(base::OnceCallback<void()> callback);
+  void OnTerminateProcessComplete(base::OnceCallback<void()> callback,
+                                  bool killed);
+
+  void RestartMahoragaProcess();
...
+
+  base::FilePath GetMahoragaExecutionDir() const;
+
...
+  std::unique_ptr<MahoragaServerProxy> server_proxy_;
+
...
+  ServerPorts ports_;
+  int proxy_https_port_ = mahoraga_server::kDefaultProxyHttpsPort;
...
+
+  base::WeakPtrFactory<MahoragaServerManager> weak_factory_{this};
+};
+
+}  // namespace mahoraga
+
+#endif  // CHROME_BROWSER_MAHORAGA_SERVER_MAHORAGA_SERVER_MANAGER_H_
