diff --git a/chrome/browser/mahoraga/server/mahoraga_server_updater.cc b/chrome/browser/mahoraga/server/mahoraga_server_updater.cc
new file mode 100644
index 0000000000000..f154a9f21a6cb
--- /dev/null
+++ b/chrome/browser/mahoraga/server/mahoraga_server_updater.cc
@@ -0,0 +1,1096 @@
...
+#include "chrome/browser/mahoraga/server/mahoraga_server_constants.h"
...
+  return net::DefineNetworkTrafficAnnotation("mahoraga_server_appcast", R"(
+    semantics {
+      sender: "Mahoraga Server Updater"
+      description:
+        "Checks for updates to the Mahoraga server component by fetching "
+        "an appcast XML feed."
...
+      internal {
+        contacts {
+          email: "nikhil@mahoraga.com"
+        }
+      }
+    }
+    policy {
+      cookies_allowed: NO
+      setting: "This feature can be disabled via --disable-mahoraga-server or --disable-mahoraga-server-updater."
+      policy_exception_justification:
+        "Essential for keeping Mahoraga server component up to date."
+    })");
+}
+
+net::NetworkTrafficAnnotationTag GetDownloadTrafficAnnotation() {
+  return net::DefineNetworkTrafficAnnotation("mahoraga_server_download", R"(
+    semantics {
+      sender: "Mahoraga Server Updater"
+      description:
+        "Downloads a new version of the Mahoraga server component."
...
+  return net::DefineNetworkTrafficAnnotation("mahoraga_server_status", R"(
+    semantics {
+      sender: "Mahoraga Server Updater"
+      description:
+        "Checks if the local Mahoraga server is ready for hot-swap update."
...
+      setting: "This feature can be disabled via --disable-mahoraga-server or --disable-mahoraga-server-updater."
+      policy_exception_justification:
+        "Essential for coordinating Mahoraga server updates."
+    })");
+}
+
...
+    LOG(ERROR) << "mahoraga: Failed to decode public key from base64";
+    return false;
+  }
+  if (public_key_bytes.size() != ED25519_PUBLIC_KEY_LEN) {
+    LOG(ERROR) << "mahoraga: Invalid public key length: "
...
+    LOG(ERROR) << "mahoraga: Failed to decode signature from base64";
+    return false;
+  }
+  if (signature_bytes.size() != ED25519_SIGNATURE_LEN) {
+    LOG(ERROR) << "mahoraga: Invalid signature length: "
...
+    LOG(ERROR) << "mahoraga: Failed to read file for signature verification: "
+               << file_path;
+    return false;
+  }
+
...
+  if (result != 1) {
+    LOG(ERROR) << "mahoraga: Ed25519 signature verification failed";
+    return false;
+  }
+
+  LOG(INFO) << "mahoraga: Ed25519 signature verified successfully";
+  return true;
+}
+
...
+  }
+
+  LOG(INFO) << "mahoraga: Extracted ZIP to " << dest_dir;
+  return "";  // Success
+}
+
...
+    LOG(WARNING) << "mahoraga: Cleaning stale version directory: " << dest_dir;
...
+}  // namespace
+
+MahoragaServerUpdater::MahoragaServerUpdater(
+    mahoraga::MahoragaServerManager* manager)
+    : manager_(manager),
+      descriptor_(&mahoraga::GetManagedServerDescriptor()) {}
+
+MahoragaServerUpdater::~MahoragaServerUpdater() {
+  Stop();
+}
+
+void MahoragaServerUpdater::Start() {
+  LOG(INFO) << "mahoraga: Starting server updater";
+
...
+                            &MahoragaServerUpdater::OnUpdateTimer);
+}
+
+void MahoragaServerUpdater::LoadVersionCachesAsync() {
...
+          },
+          version_file),
+      base::BindOnce(&MahoragaServerUpdater::OnDownloadedVersionLoaded,
...
+          bundled_binary),
+      base::BindOnce(
+          [](base::WeakPtr<MahoragaServerUpdater> self,
...
+}
+
+void MahoragaServerUpdater::OnDownloadedVersionLoaded(
+    const std::string& version_str) {
+  if (!version_str.empty()) {
+    cached_downloaded_version_ = base::Version(version_str);
+    LOG(INFO) << "mahoraga: Cached downloaded version: "
...
+}
+
+void MahoragaServerUpdater::OnBundledVersionLoaded(int exit_code,
...
+      LOG(INFO) << "mahoraga: Cached bundled version: "
+                << cached_bundled_version_.GetString();
+    } else {
+      LOG(WARNING) << "mahoraga: Could not parse bundled version from: "
+                   << output;
+    }
+  } else {
+    LOG(WARNING) << "mahoraga: Failed to get bundled version (exit_code="
+                 << exit_code << ")";
+  }
...
+}
+
+void MahoragaServerUpdater::CheckVersionCachesAndStart() {
...
+}
+
+void MahoragaServerUpdater::Stop() {
+  LOG(INFO) << "mahoraga: Stopping server updater";
...
+}
+
+bool MahoragaServerUpdater::IsUpdateInProgress() const {
+  return update_in_progress_;
+}
+
+void MahoragaServerUpdater::CheckNow() {
+  if (!bundled_version_loaded_ || !downloaded_version_loaded_) {
+    LOG(INFO) << "mahoraga: Version caches not loaded yet, skipping check";
+    return;
+  }
+
+  if (update_in_progress_) {
+    LOG(INFO) << "mahoraga: Update check already in progress, skipping";
+    return;
+  }
+
+  FetchAppcast();
+}
+
+void MahoragaServerUpdater::OnUpdateTimer() {
+  CheckNow();
+}
+
+void MahoragaServerUpdater::FetchAppcast() {
...
+  if (cmd->HasSwitch(mahoraga::kServerAppcastUrl)) {
+    appcast_url = cmd->GetSwitchValueASCII(mahoraga::kServerAppcastUrl);
+    LOG(INFO) << "mahoraga: Using custom appcast URL: " << appcast_url;
+  } else if (base::FeatureList::IsEnabled(features::kMahoragaAlphaFeatures)) {
...
+  }
+
+  LOG(INFO) << "mahoraga: Fetching appcast from " << url;
+
...
+      url_loader_factory,
+      base::BindOnce(&MahoragaServerUpdater::OnAppcastFetched,
+                     weak_factory_.GetWeakPtr()),
+      kMaxAppcastSize);
+}
+
+void MahoragaServerUpdater::OnAppcastFetched(
...
+  }
+
+  LOG(INFO) << "mahoraga: Received appcast (" << response->size() << " bytes)";
+
+  // Parse the appcast
+  std::optional<AppcastItem> item =
+      MahoragaAppcastParser::ParseLatestItem(*response);
+  if (!item) {
+    OnError("check", "Failed to parse appcast XML");
+    return;
+  }
+
+  LOG(INFO) << "mahoraga: Latest version in appcast: "
...
+  }
+
+  LOG(INFO) << "mahoraga: Found enclosure for current platform: "
+            << enclosure->url;
+
+  // Compare with current version
+  base::Version current = GetCurrentVersion();
+  LOG(INFO) << "mahoraga: Current version: "
+            << (current.IsValid() ? current.GetString() : "(none)");
+
+  if (current.IsValid() && current >= item->version) {
+    LOG(INFO) << "mahoraga: Already up to date";
+    ResetState();
+    return;
+  }
+
+  LOG(INFO) << "mahoraga: New version available: "
+            << item->version.GetString();
+  pending_item_ = *item;
...
+}
+
+void MahoragaServerUpdater::CheckVersionAlreadyDownloaded(
...
+      base::BindOnce(&MahoragaServerUpdater::OnVersionExistsCheck,
+                     weak_factory_.GetWeakPtr(), enclosure, version));
+}
+
+void MahoragaServerUpdater::OnVersionExistsCheck(
+    const AppcastEnclosure& enclosure,
+    const base::Version& version,
+    bool exists) {
+  if (exists) {
+    LOG(INFO) << "mahoraga: Version " << version.GetString()
...
+}
+
+void MahoragaServerUpdater::StartDownload(const AppcastEnclosure& enclosure,
+                                           const base::Version& version) {
...
+            LOG(INFO) << "mahoraga: Downloading " << download_url;
+
+            auto request = std::make_unique<network::ResourceRequest>();
...
+
+            // Add progress logging (visible with --vmodule=*mahoraga*=1)
+            self->download_loader_->SetOnDownloadProgressCallback(
+                base::BindRepeating([](uint64_t current) {
+                  LOG(INFO) << "mahoraga: Download progress: "
...
+                url_loader_factory,
+                base::BindOnce(&MahoragaServerUpdater::OnDownloadComplete,
+                               self, ver),
+                download_path);
+          },
+          weak_factory_.GetWeakPtr(), enclosure, version));
+}
+
+void MahoragaServerUpdater::OnDownloadComplete(const base::Version& version,
...
+  }
+
+  LOG(INFO) << "mahoraga: Download complete: " << zip_path;
+
+  // Now verify and extract
+  VerifyAndExtract(zip_path, pending_signature_, version);
+}
+
+void MahoragaServerUpdater::VerifyAndExtract(const base::FilePath& zip_path,
+                                              const std::string& signature,
+                                              const base::Version& version) {
...
+
+  LOG(INFO) << "mahoraga: Verifying signature and extracting to " << dest_dir;
+
...
+      base::BindOnce(
+          [](base::WeakPtr<MahoragaServerUpdater> self, base::Version version,
+             VerifyExtractResult result) {
+            if (!self) {
+              return;
+            }
...
+}
+
+void MahoragaServerUpdater::OnVerifyAndExtractComplete(
+    const base::Version& version,
+    bool success,
...
+  }
+
+  LOG(INFO) << "mahoraga: Verification and extraction successful";
+
+  // Test the binary
+  TestBinary(version);
+}
+
+void MahoragaServerUpdater::TestBinary(const base::Version& version) {
+  state_ = State::kTesting;
+
+  base::FilePath binary_path = GetDownloadedBinaryPath(version);
+  LOG(INFO) << "mahoraga: Testing binary: " << binary_path;
+
...
+}
+
+void MahoragaServerUpdater::OnBinaryTestComplete(const base::Version& version,
+                                                  int exit_code,
+                                                  const std::string& output) {
+  if (exit_code != 0) {
+    LOG(ERROR) << "mahoraga: Binary test failed with exit code " << exit_code
+               << ": " << output;
+
...
+  }
+
+  LOG(INFO) << "mahoraga: Binary test passed: " << output;
+
+  // Check if server is ready for hot-swap
+  CheckServerStatus();
+}
+
+void MahoragaServerUpdater::CheckServerStatus() {
...
+
+  LOG(INFO) << "mahoraga: Checking server status at " << status_url;
+
+  auto request = std::make_unique<network::ResourceRequest>();
...
+      url_loader_factory,
+      base::BindOnce(&MahoragaServerUpdater::OnStatusFetched,
+                     weak_factory_.GetWeakPtr()),
+      4096);
+}
+
+void MahoragaServerUpdater::OnStatusFetched(
+    std::optional<std::string> response) {
+  if (!response.has_value()) {
+    int net_error = status_loader_->NetError();
+    LOG(WARNING) << "mahoraga: Failed to fetch server status: "
...
+    LOG(WARNING)
+        << "mahoraga: Invalid status response, proceeding with update";
+    OnServerStatusChecked(/*can_update=*/true);
+    return;
+  }
+
...
+    LOG(WARNING) << "mahoraga: Status response missing can_update field";
+    OnServerStatusChecked(/*can_update=*/true);
+    return;
+  }
+
...
+}
+
+void MahoragaServerUpdater::OnServerStatusChecked(bool can_update) {
+  if (!can_update) {
+    LOG(INFO) << "mahoraga: Server busy, will retry hot-swap at next check";
+
+    base::DictValue props;
+    props.Set("pending_version", pending_item_.version.GetString());
+    mahoraga_metrics::MahoragaMetrics::Log("server.ota.busy",
+                                             std::move(props));
+
+    ResetState();
+    return;
...
+}
+
+void MahoragaServerUpdater::PerformHotSwap(const base::Version& version) {
+  LOG(INFO) << "mahoraga: Performing hot-swap to version "
+            << version.GetString();
+
...
+      base::BindOnce(&MahoragaServerUpdater::OnHotSwapComplete,
+                     weak_factory_.GetWeakPtr(), old_version, version));
+}
+
+void MahoragaServerUpdater::OnHotSwapComplete(const base::Version& old_version,
+                                               const base::Version& new_version,
+                                               bool success) {
+  if (!success) {
+    LOG(ERROR) << "mahoraga: Hot-swap failed, reverting to bundled version";
+
...
+  }
+
+  LOG(INFO) << "mahoraga: Hot-swap successful! Now running version "
...
+  mahoraga_metrics::MahoragaMetrics::Log("server.ota.success",
+                                           std::move(props));
+
+  ResetState();
+}
+
+base::Version MahoragaServerUpdater::GetCurrentVersion() {
...
+}
+
+void MahoragaServerUpdater::InvalidateDownloadedVersion() {
+  LOG(WARNING) << "mahoraga: Invalidating downloaded version, "
...
+                LOG(ERROR) << "mahoraga: Failed to delete versions directory: "
+                           << versions_dir;
+              }
+            }
+          },
...
+}
+
+base::FilePath MahoragaServerUpdater::GetExecutionDir() const {
...
+  // .mahoraga/BrowserClawServer/); empty keeps the legacy Mahoraga layout.
...
+}
+
+base::FilePath MahoragaServerUpdater::GetVersionsDir() const {
+  return GetExecutionDir().AppendASCII(kVersionsDirectoryName);
+}
+
+base::FilePath MahoragaServerUpdater::GetVersionDir(
+    const base::Version& version) const {
+  return GetVersionsDir().AppendASCII(version.GetString());
+}
+
+base::FilePath MahoragaServerUpdater::GetPendingUpdateDir() const {
+  return GetExecutionDir().AppendASCII(kPendingUpdateDirectoryName);
+}
+
+base::FilePath MahoragaServerUpdater::GetBundledBinaryPath() const {
+  // Delegate to manager's existing logic
+  return manager_->GetMahoragaServerExecutablePath();
+}
+
+base::FilePath MahoragaServerUpdater::GetBundledResourcesPath() const {
+  return manager_->GetMahoragaServerResourcesPath();
+}
+
+base::FilePath MahoragaServerUpdater::GetDownloadedBinaryPath(
+    const base::Version& version) const {
...
+}
+
+base::FilePath MahoragaServerUpdater::GetDownloadedResourcesPath(
+    const base::Version& version) const {
+  return GetVersionDir(version).Append(FILE_PATH_LITERAL("resources"));
+}
+
+base::FilePath MahoragaServerUpdater::GetBestServerBinaryPath() {
...
+}
+
+base::FilePath MahoragaServerUpdater::GetBestServerResourcesPath() {
+  // Use cached versions to avoid blocking I/O
+  base::Version downloaded = cached_downloaded_version_;
+  base::Version bundled = cached_bundled_version_;
+
...
+}
+
+void MahoragaServerUpdater::CleanupPendingUpdate() {
+  base::FilePath pending_dir = GetPendingUpdateDir();
...
+}
+
+void MahoragaServerUpdater::CleanupOldVersions() {
+  base::FilePath versions_dir = GetVersionsDir();
+
+  base::ThreadPool::PostTask(
+      FROM_HERE, {base::MayBlock()},
...
+              LOG(INFO) << "mahoraga: Cleaning up old version: "
...
+              mahoraga_metrics::MahoragaMetrics::Log("server.ota.cleanup",
+                                                       std::move(props));
+            }
+          },
...
+}
+
+void MahoragaServerUpdater::OnError(const std::string& stage,
+                                     const std::string& error) {
+  LOG(ERROR) << "mahoraga: Update error at " << stage << ": " << error;
+
+  base::DictValue props;
...
+  }
+  mahoraga_metrics::MahoragaMetrics::Log("server.ota.error",
+                                           std::move(props));
+
...
+            [](base::FilePath dir) {
+              if (base::PathExists(dir)) {
+                LOG(INFO) << "mahoraga: Cleaning up failed version: " << dir;
+                base::DeletePathRecursively(dir);
+              }
+            },
+            version_dir));
...
+}
+
+void MahoragaServerUpdater::ResetState() {
+  state_ = State::kIdle;
