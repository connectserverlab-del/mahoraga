diff --git a/chrome/browser/mahoraga/server/process_controller_impl.cc b/chrome/browser/mahoraga/server/process_controller_impl.cc
new file mode 100644
index 0000000000000000000000000000000000000000..5e5fcdc251bfc3e352287150f9e43fc7c618b093
--- /dev/null
+++ b/chrome/browser/mahoraga/server/process_controller_impl.cc
@@ -0,0 +1,167 @@
...
+    LOG(ERROR) << "mahoraga: Failed to serialize " << config.log_name
+               << " config to JSON";
+    return base::FilePath();
+  }
+
+  if (!base::WriteFile(config_path, json_output.value())) {
+    LOG(ERROR) << "mahoraga: Failed to write " << config.log_name
+               << " config file: " << config_path;
+    return base::FilePath();
+  }
+
+  LOG(INFO) << "mahoraga: Wrote " << config.log_name << " config to "
+            << config_path;
+  return config_path;
+}
+
...
+    LOG(WARNING) << "mahoraga: Binary not found at " << actual_exe_path
...
+
+    if (!base::PathExists(actual_exe_path)) {
+      LOG(ERROR) << "mahoraga: Bundled binary also not found at: "
+                 << actual_exe_path;
+      return result;
+    }
+  }
...
+    LOG(ERROR) << "mahoraga: Execution directory path is empty";
+    return result;
+  }
+
+  // Ensure execution directory exists (blocking I/O)
+  if (!base::CreateDirectory(config.paths.execution)) {
+    LOG(ERROR) << "mahoraga: Failed to create execution directory at: "
...
+    LOG(ERROR) << "mahoraga: Failed to write config file, aborting launch";
+    return result;
+  }
+
...
+  }
+
+  LOG(INFO) << "mahoraga: Terminating process with SIGKILL (PID: "
...
+      LOG(INFO) << "mahoraga: Process killed successfully";
+    } else {
+      LOG(WARNING) << "mahoraga: WaitForExit failed";
+    }
+  } else {
+    LOG(INFO) << "mahoraga: SIGKILL sent (not waiting for exit)";
+  }
+#else
+  // Windows: Terminate with wait parameter
+  bool terminated = process->Terminate(0, wait);
+  if (terminated) {
+    LOG(INFO) << "mahoraga: Process terminated successfully";
+  } else {
+    LOG(ERROR) << "mahoraga: Failed to terminate process";
+  }
+#endif
+}
+
...
+  }
+
+  LOG(INFO) << "mahoraga: Waiting for process exit (PID: " << process->Pid()
+            << ", timeout: " << timeout.InSeconds() << "s)";
+
+  bool exited = process->WaitForExitWithTimeout(timeout, exit_code);
+  if (exited) {
+    LOG(INFO) << "mahoraga: Process exited with code " << *exit_code;
+  } else {
+    LOG(INFO) << "mahoraga: Process did not exit within timeout";
+  }
+  return exited;
+}
+
