diff --git a/chrome/browser/mahoraga/server/mahoraga_server_updater.h b/chrome/browser/mahoraga/server/mahoraga_server_updater.h
new file mode 100644
index 0000000000000..529d7fc706730
--- /dev/null
+++ b/chrome/browser/mahoraga/server/mahoraga_server_updater.h
@@ -0,0 +1,170 @@
+// Copyright 2024 The Chromium Authors
+// Use of this source code is governed by a BSD-style license that can be
+// found in the LICENSE file.
+
+#ifndef CHROME_BROWSER_MAHORAGA_SERVER_MAHORAGA_SERVER_UPDATER_H_
+#define CHROME_BROWSER_MAHORAGA_SERVER_MAHORAGA_SERVER_UPDATER_H_
+
+#include <memory>
+#include <optional>
+#include <string>
...
+
+// Manages automatic updates for the selected Mahoraga sidecar server.
+//
+// Update flow:
...
+class MahoragaServerUpdater : public mahoraga::ServerUpdater {
+ public:
+  explicit MahoragaServerUpdater(mahoraga::MahoragaServerManager* manager);
+  ~MahoragaServerUpdater() override;
+
+  MahoragaServerUpdater(const MahoragaServerUpdater&) = delete;
+  MahoragaServerUpdater& operator=(const MahoragaServerUpdater&) = delete;
+
...
+  void ResetState();
+
+  raw_ptr<mahoraga::MahoragaServerManager> manager_;
+
...
+  raw_ptr<const mahoraga::ManagedServerDescriptor> descriptor_;
+
...
+
+  base::WeakPtrFactory<MahoragaServerUpdater> weak_factory_{this};
+};
+
+}  // namespace mahoraga_server
+
+#endif  // CHROME_BROWSER_MAHORAGA_SERVER_MAHORAGA_SERVER_UPDATER_H_
