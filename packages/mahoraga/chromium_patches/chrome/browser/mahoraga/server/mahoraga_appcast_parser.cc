diff --git a/chrome/browser/mahoraga/server/mahoraga_appcast_parser.cc b/chrome/browser/mahoraga/server/mahoraga_appcast_parser.cc
new file mode 100644
index 0000000000000..dd30f75d414ec
--- /dev/null
+++ b/chrome/browser/mahoraga/server/mahoraga_appcast_parser.cc
@@ -0,0 +1,202 @@
...
+
+// static
+std::optional<AppcastItem> MahoragaAppcastParser::ParseLatestItem(
...
+
+// static
+std::vector<AppcastItem> MahoragaAppcastParser::ParseAllItems(
+    const std::string& xml) {
+  std::vector<AppcastItem> items;
+
+  XmlReader reader;
+  if (!reader.Load(xml)) {
+    LOG(WARNING) << "mahoraga: Failed to load appcast XML";
+    return items;
+  }
+
...
+        } else {
+          LOG(WARNING) << "mahoraga: Skipping invalid appcast item (no valid "
...
+  }
+
+  VLOG(1) << "mahoraga: Parsed " << items.size() << " appcast items";
+  return items;
+}
+
+}  // namespace mahoraga_server
