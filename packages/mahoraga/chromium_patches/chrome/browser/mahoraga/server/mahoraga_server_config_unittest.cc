diff --git a/chrome/browser/mahoraga/server/mahoraga_server_config_unittest.cc b/chrome/browser/mahoraga/server/mahoraga_server_config_unittest.cc
new file mode 100644
index 0000000000000..9a25f32a0da56
--- /dev/null
+++ b/chrome/browser/mahoraga/server/mahoraga_server_config_unittest.cc
@@ -0,0 +1,228 @@
...
+  config.identity.mahoraga_version = "1.2.3";
...
+}
+
+TEST(MahoragaProductTest, ReturnsBakedProductWithoutSwitch) {
+  EXPECT_EQ(BakedProduct(), GetProduct());
+}
+
+#if !BUILDFLAG(MAHORAGA_ALLOW_RUNTIME_PRODUCT_OVERRIDE)
+TEST(MahoragaProductTest, IgnoresProductSwitchWhenOverrideDisabled) {
+  base::test::ScopedCommandLine scoped_command_line;
+  scoped_command_line.GetProcessCommandLine()->AppendSwitchASCII(
+      kMahoragaProduct, ProductSwitchValue(OtherProduct()));
+
+  EXPECT_EQ(BakedProduct(), GetProduct());
+}
+#endif
+
+#if BUILDFLAG(MAHORAGA_ALLOW_RUNTIME_PRODUCT_OVERRIDE)
+TEST(MahoragaProductTest, HonorsProductSwitchWhenOverrideEnabled) {
+  base::test::ScopedCommandLine scoped_command_line;
+  scoped_command_line.GetProcessCommandLine()->AppendSwitchASCII(
+      kMahoragaProduct, ProductSwitchValue(OtherProduct()));
+
...
+}
+
+TEST(MahoragaProductTest, InvalidProductSwitchFallsBackToBakedProduct) {
+  base::test::ScopedCommandLine scoped_command_line;
+  scoped_command_line.GetProcessCommandLine()->AppendSwitchASCII(
+      kMahoragaProduct, "invalid");
+
+  EXPECT_EQ(BakedProduct(), GetProduct());
+}
+#endif
+
+TEST(MahoragaServerConfigTest, MahoragaDescriptorMatchesLegacyServer) {
+  const ManagedServerDescriptor& descriptor = GetMahoragaServerDescriptor();
+
+  EXPECT_EQ(Product::kMahoraga, descriptor.product);
+  EXPECT_EQ(std::string_view("Mahoraga server"), descriptor.log_name);
+  EXPECT_EQ(base::FilePath::StringType(FILE_PATH_LITERAL("MahoragaServer")),
+            ToPathString(descriptor.bundle_dir));
+  EXPECT_EQ(base::FilePath::StringType(FILE_PATH_LITERAL("mahoraga_server")),
...
+
+  // Empty state dir preserves the legacy .mahoraga/current_version layout.
+  EXPECT_TRUE(descriptor.updater.state_dir.empty());
+  EXPECT_EQ(std::string_view("https://cdn.mahoraga.com/appcast-server.xml"),
+            descriptor.updater.appcast_url);
+  EXPECT_EQ(
+      std::string_view("https://cdn.mahoraga.com/appcast-server.alpha.xml"),
+      descriptor.updater.alpha_appcast_url);
+  EXPECT_EQ(std::string_view("/status"), descriptor.updater.readiness_path);
+}
+
+TEST(MahoragaServerConfigTest, BrowserClawDescriptorMatchesClawServer) {
...
+            ToPathString(descriptor.bundle_dir));
+  EXPECT_EQ(
+      base::FilePath::StringType(FILE_PATH_LITERAL("mahoraga-claw-server")),
+      ToPathString(descriptor.binary_name));
+  EXPECT_EQ(base::FilePath::StringType(FILE_PATH_LITERAL("config.json")),
+            ToPathString(descriptor.config_file_name));
+  EXPECT_EQ(std::string_view("/system/health"), descriptor.health_path);
+  EXPECT_TRUE(descriptor.enable_updater);
+
+  // Claw isolates OTA state under .mahoraga/BrowserClawServer/ and fetches its
+  // own feed.
+  EXPECT_EQ(base::FilePath::StringType(FILE_PATH_LITERAL("BrowserClawServer")),
+            ToPathString(descriptor.updater.state_dir));
+  EXPECT_EQ(
+      std::string_view("https://cdn.mahoraga.com/appcast-claw-server.xml"),
+      descriptor.updater.appcast_url);
+  EXPECT_EQ(std::string_view(
+                "https://cdn.mahoraga.com/appcast-claw-server.alpha.xml"),
+            descriptor.updater.alpha_appcast_url);
+  // No readiness contract yet: empty path skips the status fetch.
+  EXPECT_TRUE(descriptor.updater.readiness_path.empty());
+}
+
+TEST(MahoragaServerConfigTest, ManagedDescriptorUsesSelectedProduct) {
...
+
+#if BUILDFLAG(MAHORAGA_ALLOW_RUNTIME_PRODUCT_OVERRIDE)
+TEST(MahoragaServerConfigTest, ManagedDescriptorFollowsRuntimeOverride) {
+  base::test::ScopedCommandLine scoped_command_line;
+  scoped_command_line.GetProcessCommandLine()->AppendSwitchASCII(
+      kMahoragaProduct, ProductSwitchValue(OtherProduct()));
+
...
+#endif
+
+TEST(MahoragaServerConfigTest, ServerConfigJsonHasUnifiedShape) {
...
+  ASSERT_NE(nullptr, instance->FindString("mahoraga_version"));
+  EXPECT_EQ("1.2.3", *instance->FindString("mahoraga_version"));
+  ASSERT_NE(nullptr, instance->FindString("chromium_version"));
+  EXPECT_EQ("140.0.0.0", *instance->FindString("chromium_version"));
+}
+
+// Both Mahoraga and BrowserClaw consume this same config.json. Guard against
...
+TEST(MahoragaServerConfigTest, ServerConfigJsonIncludesSharedRuntimeKeys) {
+  ServerLaunchConfig config = BuildLaunchConfig();
+  base::FilePath resources(FILE_PATH_LITERAL("resources"));
+
+  base::DictValue root = BuildServerConfigJson(config, resources);
