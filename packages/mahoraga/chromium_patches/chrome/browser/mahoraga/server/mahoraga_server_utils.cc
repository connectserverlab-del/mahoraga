diff --git a/chrome/browser/mahoraga/server/mahoraga_server_utils.cc b/chrome/browser/mahoraga/server/mahoraga_server_utils.cc
new file mode 100644
index 0000000000000..dbd7e166bb02c
--- /dev/null
+++ b/chrome/browser/mahoraga/server/mahoraga_server_utils.cc
@@ -0,0 +1,518 @@
...
+#endif
+
+namespace mahoraga::server_utils {
+
+namespace {
+
...
+                      bool allow_reuse) {
+  LOG(INFO) << "mahoraga: Finding port starting from " << starting_port;
+
...
+        LOG(INFO) << "mahoraga: Port " << starting_port
+                  << " was in use or excluded, using " << port_to_try
+                  << " instead";
+      } else {
+        LOG(INFO) << "mahoraga: Using port " << port_to_try;
+      }
+      return port_to_try;
+    }
+  }
+
+  LOG(WARNING) << "mahoraga: Could not find available port after "
...
+    LOG(WARNING) << "mahoraga: Invalid state file format";
+    return std::nullopt;
+  }
+
...
+    LOG(WARNING) << "mahoraga: State file missing required fields";
+    return std::nullopt;
+  }
+
+  ServerState state;
...
+
+  LOG(INFO) << "mahoraga: Read state file - PID: " << state.pid
...
+    LOG(ERROR) << "mahoraga: Failed to serialize state to JSON";
+    return false;
+  }
+
+  if (!base::WriteFile(state_path, json_output.value())) {
+    LOG(ERROR) << "mahoraga: Failed to write state file: " << state_path;
+    return false;
+  }
+
+  LOG(INFO) << "mahoraga: Wrote state file - PID: " << state.pid
+            << ", creation_time: " << state.creation_time;
+  return true;
+}
+
...
+    LOG(WARNING) << "mahoraga: Failed to delete state file: " << state_path;
+    return false;
+  }
+
+  LOG(INFO) << "mahoraga: Deleted state file";
+  return true;
+}
+
+// =============================================================================
...
+    }
+    PLOG(WARNING) << "mahoraga: Failed to send SIGTERM to PID " << pid;
+    return false;
+  }
+
...
+  LOG(WARNING) << "mahoraga: Process " << pid
+               << " did not terminate after SIGTERM, sending SIGKILL";
+  if (kill(pid, SIGKILL) != 0) {
+    if (errno == ESRCH) {
+      return true;  // Gone between checks
+    }
+    PLOG(ERROR) << "mahoraga: Failed to send SIGKILL to PID " << pid;
+    return false;
+  }
+
...
+    }
+    LOG(ERROR) << "mahoraga: Failed to open process " << pid
...
+    DWORD error = GetLastError();
+    LOG(ERROR) << "mahoraga: Failed to terminate process " << pid
+               << ", error: " << error;
+    return false;
+  }
+
