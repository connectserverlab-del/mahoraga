diff --git a/chrome/browser/mahoraga/server/mahoraga_server_proxy.cc b/chrome/browser/mahoraga/server/mahoraga_server_proxy.cc
new file mode 100644
index 0000000000000..557eedb4779c4
--- /dev/null
+++ b/chrome/browser/mahoraga/server/mahoraga_server_proxy.cc
@@ -0,0 +1,649 @@
...
+    "Service Unavailable";
+
+net::NetworkTrafficAnnotationTag kMahoragaProxyPumpTrafficAnnotation =
+    net::DefineNetworkTrafficAnnotation("mahoraga_proxy_pump", R"(
+      semantics {
+        sender: "Mahoraga Server Proxy"
+        description:
+          "Copies raw HTTP bytes between clients connected to the Mahoraga "
+          "proxy port and the Mahoraga sidecar server on loopback."
+        trigger: "A client connects to the Mahoraga proxy port."
+        data: "HTTP requests and responses for Mahoraga server functionality."
+        destination: LOCAL
+      }
+      policy {
+        cookies_allowed: NO
...
+          "Local proxy for Mahoraga server functionality."
+      })");
+
+}  // namespace
+
+struct MahoragaServerProxy::ListenerState {
...
+};
+
+class MahoragaServerProxy::Connection {
+ public:
+  Connection(int id,
...
+                                 drainable, from, to),
+                  kMahoragaProxyPumpTrafficAnnotation);
+    if (result != net::ERR_IO_PENDING) {
...
+};
+
+MahoragaServerProxy::MahoragaServerProxy() {
...
+}
+
+bool MahoragaServerProxy::Start(int http_port, int https_port) {
+  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
+
+  if (http_listener_) {
+    LOG(WARNING) << "mahoraga: Proxy already started on port "
+                 << bound_http_port_;
+    return false;
+  }
+
...
+}
+
+bool MahoragaServerProxy::StartHttpListener(int http_port) {
+  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
+
+  if (http_port < 0 || http_port > 65535) {
+    LOG(ERROR) << "mahoraga: Proxy invalid HTTP port " << http_port;
+    return false;
+  }
+
...
+  if (result != net::OK) {
+    LOG(ERROR) << "mahoraga: Proxy failed to bind 0.0.0.0:" << http_port
...
+  if (result != net::OK) {
+    LOG(ERROR) << "mahoraga: Proxy failed to read bound port - "
+               << net::ErrorToString(result);
+    return false;
+  }
+
...
+
+  LOG(INFO) << "mahoraga: MCP proxy listening on 0.0.0.0:" << bound_http_port_;
+  return true;
+}
+
+bool MahoragaServerProxy::StartHttpsListener(int https_port) {
+  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
+
+  if (https_port < 0 || https_port > 65535) {
+    LOG(ERROR) << "mahoraga: Proxy invalid HTTPS port " << https_port
...
+  if (result != net::OK) {
+    LOG(ERROR) << "mahoraga: HTTPS proxy failed to bind 0.0.0.0:" << https_port
+               << " - " << net::ErrorToString(result)
+               << ", continuing HTTP-only";
+    tls_context_.reset();
+    tls_cert_.reset();
...
+  result = listener->socket->GetLocalAddress(&local_address);
+  if (result != net::OK) {
+    LOG(ERROR) << "mahoraga: HTTPS proxy failed to read bound port - "
...
+
+  LOG(INFO) << "mahoraga: HTTPS MCP proxy listening on 0.0.0.0:"
+            << bound_https_port_;
+  return true;
+}
+
+bool MahoragaServerProxy::GenerateTlsCredentials() {
+  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
+
...
+          &der_cert)) {
+    LOG(ERROR) << "mahoraga: HTTPS proxy failed to create certificate, "
+                  "continuing HTTP-only";
+    tls_key_.reset();
+    return false;
+  }
...
+  if (!tls_cert_) {
+    LOG(ERROR) << "mahoraga: HTTPS proxy failed to parse certificate, "
+                  "continuing HTTP-only";
+    tls_key_.reset();
+    return false;
+  }
...
+                                             net::SSLServerConfig());
+  if (!tls_context_) {
+    LOG(ERROR) << "mahoraga: HTTPS proxy failed to create SSL context, "
+                  "continuing HTTP-only";
+    tls_cert_.reset();
+    tls_key_.reset();
+    return false;
...
+
+  if (http_listener_) {
+    LOG(INFO) << "mahoraga: Stopping MCP proxy on port " << bound_http_port_;
+    http_listener_.reset();
+  }
+
+  if (https_listener_) {
+    LOG(INFO) << "mahoraga: Stopping HTTPS MCP proxy on port "
+              << bound_https_port_;
...
+}
+
+void MahoragaServerProxy::SetBackendPort(int port) {
+  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
+
+  backend_port_ = port;
+  LOG(INFO) << "mahoraga: Proxy backend port set to " << port;
+}
+
+void MahoragaServerProxy::SetAllowRemote(bool allow) {
+  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
+
+  allow_remote_ = allow;
+  LOG(INFO) << "mahoraga: Proxy allow_remote set to "
+            << (allow ? "true" : "false");
+}
+
+void MahoragaServerProxy::StartAccept(ListenerState* listener) {
+  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
+
...
+      base::BindOnce(&MahoragaServerProxy::OnAccept,
...
+}
+
+void MahoragaServerProxy::OnAccept(ListenerState* listener, int result) {
+  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
+
+  if (!listener || !listener->socket) {
+    return;
...
+
+  if (result != net::OK) {
+    LOG(ERROR) << "mahoraga: " << (listener->is_https ? "HTTPS " : "")
+               << "Proxy accept failed - " << net::ErrorToString(result);
+    base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(
+        FROM_HERE,
+        base::BindOnce(&MahoragaServerProxy::StartAccept,
+                       weak_factory_.GetWeakPtr(), listener),
...
+}
+
+void MahoragaServerProxy::StartConnection(
+    std::unique_ptr<net::StreamSocket> client_socket) {
+  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
+
...
+      base::BindRepeating(&MahoragaServerProxy::OnConnectionFinished,
...
+}
+
+void MahoragaServerProxy::StartTlsHandshake(
+    std::unique_ptr<net::StreamSocket> client_socket) {
+  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
+
+  if (!tls_context_) {
...
+      base::BindOnce(&MahoragaServerProxy::OnTlsHandshakeDone,
...
+}
+
+void MahoragaServerProxy::OnTlsHandshakeDone(int handshake_id, int result) {
+  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
+
...
+
+  if (result != net::OK) {
+    VLOG(1) << "mahoraga: HTTPS proxy handshake failed - "
+            << net::ErrorToString(result);
...
+}
+
+void MahoragaServerProxy::OnConnectionFinished(int connection_id) {
+  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
+
