diff --git a/chrome/browser/mahoraga/server/mahoraga_appcast_parser_unittest.cc b/chrome/browser/mahoraga/server/mahoraga_appcast_parser_unittest.cc
new file mode 100644
index 0000000000000..ceb483dee35c6
--- /dev/null
+++ b/chrome/browser/mahoraga/server/mahoraga_appcast_parser_unittest.cc
@@ -0,0 +1,389 @@
+// Copyright 2024 The Chromium Authors
+// Use of this source code is governed by a BSD-style license that can be
+// found in the LICENSE file.
+
+#include "chrome/browser/mahoraga/server/mahoraga_appcast_parser.h"
+
...
+// =============================================================================
+
+TEST(MahoragaAppcastParserTest, ParsesValidAppcastWithSingleItem) {
...
+  )";
+
+  auto item = MahoragaAppcastParser::ParseLatestItem(kValidXml);
+
...
+}
+
+TEST(MahoragaAppcastParserTest, ParsesMultipleEnclosuresPerItem) {
...
+  )";
+
+  auto item = MahoragaAppcastParser::ParseLatestItem(kMultiPlatformXml);
+
+  ASSERT_TRUE(item.has_value());
...
+}
+
+TEST(MahoragaAppcastParserTest, ParseAllItems_ReturnsMultipleVersions) {
...
+  )";
+
+  auto items = MahoragaAppcastParser::ParseAllItems(kMultiVersionXml);
+
...
+// =============================================================================
+
+TEST(MahoragaAppcastParserTest, ReturnsNulloptForInvalidXml) {
+  auto item = MahoragaAppcastParser::ParseLatestItem("not valid xml at all");
+  EXPECT_FALSE(item.has_value());
+}
+
+TEST(MahoragaAppcastParserTest, ReturnsNulloptForEmptyString) {
+  auto item = MahoragaAppcastParser::ParseLatestItem("");
+  EXPECT_FALSE(item.has_value());
+}
+
+TEST(MahoragaAppcastParserTest, ReturnsNulloptForEmptyChannel) {
...
+  )";
+
+  auto item = MahoragaAppcastParser::ParseLatestItem(kEmptyXml);
+  EXPECT_FALSE(item.has_value());
+}
+
+TEST(MahoragaAppcastParserTest, ReturnsNulloptForMissingVersion) {
...
+  )";
+
+  auto item = MahoragaAppcastParser::ParseLatestItem(kNoVersionXml);
+  EXPECT_FALSE(item.has_value());
+}
+
+TEST(MahoragaAppcastParserTest, HandlesItemWithNoEnclosures) {
...
+  )";
+
+  auto item = MahoragaAppcastParser::ParseLatestItem(kNoEnclosureXml);
+
...
+}
+
+TEST(MahoragaAppcastParserTest, ParseAllItems_ReturnsEmptyForInvalidXml) {
+  auto items = MahoragaAppcastParser::ParseAllItems("invalid xml");
+  EXPECT_TRUE(items.empty());
+}
+
+TEST(MahoragaAppcastParserTest, ParseAllItems_ReturnsEmptyForEmptyChannel) {
+  const char kEmptyXml[] = R"(
+    <rss xmlns:sparkle="http://www.andymatuschak.org/xml-namespaces/sparkle">
+      <channel></channel>
+    </rss>
+  )";
+
+  auto items = MahoragaAppcastParser::ParseAllItems(kEmptyXml);
+  EXPECT_TRUE(items.empty());
+}
+
+// =============================================================================
...
+// =============================================================================
+
+TEST(MahoragaAppcastParserTest, HandlesWhitespaceInVersion) {
...
+  )";
+
+  auto item = MahoragaAppcastParser::ParseLatestItem(kWhitespaceXml);
+
...
+}
+
+TEST(MahoragaAppcastParserTest, HandlesZeroLengthEnclosure) {
...
+  )";
+
+  auto item = MahoragaAppcastParser::ParseLatestItem(kZeroLengthXml);
+
+  ASSERT_TRUE(item.has_value());
+  ASSERT_EQ(1u, item->enclosures.size());
...
+}
+
+TEST(MahoragaAppcastParserTest, HandlesMissingSignature) {
...
+  )";
+
+  auto item = MahoragaAppcastParser::ParseLatestItem(kNoSigXml);
+
+  ASSERT_TRUE(item.has_value());
+  ASSERT_EQ(1u, item->enclosures.size());
