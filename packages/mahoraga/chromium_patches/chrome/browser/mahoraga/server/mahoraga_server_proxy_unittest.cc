diff --git a/chrome/browser/mahoraga/server/mahoraga_server_proxy_unittest.cc b/chrome/browser/mahoraga/server/mahoraga_server_proxy_unittest.cc
new file mode 100644
index 0000000000000..522e6b7c49265
--- /dev/null
+++ b/chrome/browser/mahoraga/server/mahoraga_server_proxy_unittest.cc
@@ -0,0 +1,1257 @@
...
+};
+
+class MahoragaServerProxyTest : public testing::Test {
+ protected:
+  void SetUp() override {
...
+  MahoragaServerProxy proxy_;
+  net::EmbeddedTestServer backend_;
+  base::Lock lock_;
+};
+
+TEST(MahoragaServerProxyThreadingTest,
...
+                       *proxy = std::make_unique<MahoragaServerProxy>();
+                       std::move(done).Run();
+                     },
...
+}
+
+TEST_F(MahoragaServerProxyTest, GetForwardsRawResponseAndRequestHeaders) {
...
+}
+
+TEST_F(MahoragaServerProxyTest, PostBodyReachesBackend) {
...
+}
+
+TEST_F(MahoragaServerProxyTest, NoBackendConfiguredReturnsServiceUnavailable) {
+  TestTcpClient client;
+  ASSERT_THAT(client.Connect(proxy_.GetPort()), IsOk());
+  ASSERT_TRUE(
...
+}
+
+TEST_F(MahoragaServerProxyTest,
...
+}
+
+TEST_F(MahoragaServerProxyTest, StartWithZeroPortBindsEphemeralPort) {
+  EXPECT_GT(proxy_.GetPort(), 0);
+  EXPECT_EQ(0, proxy_.GetHttpsPort());
+}
+
+TEST_F(MahoragaServerProxyTest, HttpsForwardsBytesToBackend) {
...
+}
+
+TEST_F(MahoragaServerProxyTest, HttpsNoBackendReturnsServiceUnavailable) {
+  RestartProxyWithHttps();
+
+  TestTlsClient client;
+  ASSERT_THAT(client.Connect(proxy_.GetHttpsPort()), IsOk());
...
+}
+
+TEST_F(MahoragaServerProxyTest, HttpsEnabledLeavesHttpListenerLive) {
+  RestartProxyWithHttps();
+
+  ScriptedTcpBackend backend;
+  ASSERT_TRUE(backend.Start());
...
+}
+
+TEST_F(MahoragaServerProxyTest, KeepAliveHandlesTwoRequestsOnOneConnection) {
...
+}
+
+TEST_F(MahoragaServerProxyTest, StreamsPartialResponseBeforeBackendCloses) {
+  ScriptedTcpBackend backend;
+  ASSERT_TRUE(backend.Start());
+  proxy_.SetBackendPort(backend.port());
+
...
+}
+
+TEST_F(MahoragaServerProxyTest, LargeRequestAndResponseRoundTripByteIntact) {
...
+}
+
+TEST_F(MahoragaServerProxyTest, UpgradeStyleBidirectionalBytesAreTransparent) {
+  ScriptedTcpBackend backend;
+  ASSERT_TRUE(backend.Start());
+  proxy_.SetBackendPort(backend.port());
+
...
+}
+
+TEST_F(MahoragaServerProxyTest, ClientDisconnectMidStreamTearsDownConnection) {
+  ScriptedTcpBackend backend;
+  ASSERT_TRUE(backend.Start());
+  proxy_.SetBackendPort(backend.port());
+
