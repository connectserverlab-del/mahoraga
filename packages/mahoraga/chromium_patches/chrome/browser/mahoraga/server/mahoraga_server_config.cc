diff --git a/chrome/browser/mahoraga/server/mahoraga_server_config.cc b/chrome/browser/mahoraga/server/mahoraga_server_config.cc
new file mode 100644
index 0000000000000..6fe71062a9c96
--- /dev/null
+++ b/chrome/browser/mahoraga/server/mahoraga_server_config.cc
@@ -0,0 +1,178 @@
...
+namespace {
+
+constexpr ManagedServerDescriptor kMahoragaServerDescriptor = {
+    Product::kMahoraga,
+    "Mahoraga server",
+    FILE_PATH_LITERAL("MahoragaServer"),
+    FILE_PATH_LITERAL("mahoraga_server"),
+    FILE_PATH_LITERAL("config.json"),
+    "/system/health",
+    true,
+    {
+        // Empty state dir keeps the legacy .mahoraga/current_version layout.
+        FILE_PATH_LITERAL(""),
+        "https://cdn.mahoraga.com/appcast-server.xml",
+        "https://cdn.mahoraga.com/appcast-server.alpha.xml",
+        "/status",
+    },
+};
+
...
+    FILE_PATH_LITERAL("mahoraga-claw-server"),
+    FILE_PATH_LITERAL("config.json"),
+    "/system/health",
+    true,
+    {
+        // Isolate Claw OTA state under .mahoraga/BrowserClawServer/.
+        FILE_PATH_LITERAL("BrowserClawServer"),
+        "https://cdn.mahoraga.com/appcast-claw-server.xml",
...
+}  // namespace
+
+const ManagedServerDescriptor& GetMahoragaServerDescriptor() {
+  return kMahoragaServerDescriptor;
+}
+
...
+  }
+  return GetMahoragaServerDescriptor();
+}
+
...
+  instance.Set("mahoraga_version", config.identity.mahoraga_version);
...
+      "  chromium=%s\n"
+      "}",
+      install_id.c_str(), mahoraga_version.c_str(), chromium_version.c_str());
+}
+
