diff --git a/chrome/browser/mahoraga/server/mahoraga_server_proxy.h b/chrome/browser/mahoraga/server/mahoraga_server_proxy.h
new file mode 100644
index 0000000000000..0e62a9f7995e1
--- /dev/null
+++ b/chrome/browser/mahoraga/server/mahoraga_server_proxy.h
@@ -0,0 +1,93 @@
+// Copyright 2024 The Chromium Authors
+// Use of this source code is governed by a BSD-style license that can be
+// found in the LICENSE file.
+
+#ifndef CHROME_BROWSER_MAHORAGA_SERVER_MAHORAGA_SERVER_PROXY_H_
+#define CHROME_BROWSER_MAHORAGA_SERVER_MAHORAGA_SERVER_PROXY_H_
+
+#include <cstddef>
+#include <memory>
+
...
+namespace mahoraga {
+
+// HTTP and HTTPS proxy that fronts the Mahoraga sidecar without parsing or
+// rewriting traffic.
+//
...
+  ~MahoragaServerProxy();
+
+  MahoragaServerProxy(const MahoragaServerProxy&) = delete;
+  MahoragaServerProxy& operator=(const MahoragaServerProxy&) = delete;
+
...
+  base::WeakPtrFactory<MahoragaServerProxy> weak_factory_{this};
+};
+
+}  // namespace mahoraga
+
+#endif  // CHROME_BROWSER_MAHORAGA_SERVER_MAHORAGA_SERVER_PROXY_H_
