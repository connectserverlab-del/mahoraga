diff --git a/chrome/browser/mahoraga/server/mahoraga_server_config.h b/chrome/browser/mahoraga/server/mahoraga_server_config.h
new file mode 100644
index 0000000000000..a98e3b74de45e
--- /dev/null
+++ b/chrome/browser/mahoraga/server/mahoraga_server_config.h
@@ -0,0 +1,147 @@
+// Copyright 2024 The Chromium Authors
+// Use of this source code is governed by a BSD-style license that can be
+// found in the LICENSE file.
+
+#ifndef CHROME_BROWSER_MAHORAGA_SERVER_MAHORAGA_SERVER_CONFIG_H_
+#define CHROME_BROWSER_MAHORAGA_SERVER_MAHORAGA_SERVER_CONFIG_H_
+
+#include <string>
+#include <string_view>
+
...
+  // OTA contract consumed by MahoragaServerUpdater so a single updater can
...
+    // Subdirectory under .mahoraga holding this product's OTA state
+    // (current_version file + versions/). Empty keeps the legacy Mahoraga
+    // layout directly under .mahoraga for backward compatibility.
...
+
+// Returns the descriptor selected by mahoraga::GetProduct().
...
+
+  // Runtime data directory (~/.mahoraga or equivalent).
...
+}  // namespace mahoraga
+
+#endif  // CHROME_BROWSER_MAHORAGA_SERVER_MAHORAGA_SERVER_CONFIG_H_
