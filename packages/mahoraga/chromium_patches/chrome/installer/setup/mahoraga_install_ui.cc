diff --git a/chrome/installer/setup/mahoraga_install_ui.cc b/chrome/installer/setup/mahoraga_install_ui.cc
new file mode 100644
index 0000000000000000000000000000000000000000..93da7d596dbc87a85996de1b36cf702d94e813fe
--- /dev/null
+++ b/chrome/installer/setup/mahoraga_install_ui.cc
@@ -0,0 +1,262 @@
+// Copyright 2026 Mahoraga Authors. All rights reserved.
+// Use of this source code is governed by a BSD-style license that can be
+// found in the LICENSE file.
+
+#include "chrome/installer/setup/mahoraga_install_ui.h"
+
+#include <windows.h>
+
+#include <commctrl.h>
...
+namespace {
+
+constexpr wchar_t kInstallWindowClass[] = L"MahoragaInstallProgressWindow";
...
+}  // namespace
+
+class MahoragaInstallUI::UIThreadDelegate final
...
+};
+
+MahoragaInstallUI::MahoragaInstallUI(bool should_show)
+    : should_show_(should_show) {}
+
+MahoragaInstallUI::~MahoragaInstallUI() {
+  Close();
+}
+
+void MahoragaInstallUI::Show() {
...
+}
+
+void MahoragaInstallUI::CloseAndShowFailureMessage(int install_msg_base) {
+  if (!should_show_) {
+    return;
+  }
+
+  Close();
+  ShowMahoragaInstallFailureMessageBox(install_msg_base);
+}
+
