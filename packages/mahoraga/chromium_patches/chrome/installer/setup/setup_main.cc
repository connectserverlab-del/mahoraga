...
+namespace {
+
+bool ShouldShowMahoragaInstallUI(const InstallationState& original_state,
...
+
 InstallStatus InstallProductsHelper(InstallationState& original_state,
   ...
   const bool system_install = installer_state.system_install();
+  MahoragaInstallUI install_ui(
+      ShouldShowMahoragaInstallUI(original_state, cmd_line, installer_state));
+  install_ui.Show();
