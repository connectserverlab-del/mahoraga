diff --git a/chrome/utility/importer/mahoraga/chrome_password_importer.cc b/chrome/utility/importer/mahoraga/chrome_password_importer.cc
new file mode 100644
index 0000000000000..3aaeb073d7554
--- /dev/null
+++ b/chrome/utility/importer/mahoraga/chrome_password_importer.cc
@@ -0,0 +1,143 @@
...
+#include "chrome/utility/importer/mahoraga/chrome_importer_utils.h"
...
+    LOG(WARNING) << "mahoraga: Chrome encryption key unavailable; "
...
+    LOG(WARNING) << "mahoraga: Login Data not found at: "
...
+    LOG(WARNING) << "mahoraga: Failed to open Login Data database";
...
+      LOG(WARNING) << "mahoraga: Failed to prepare query";
+      DeleteDatabaseTemp(temp_db_path);
+      return passwords;
+    }
+
...
+          LOG(WARNING) << "mahoraga: Failed to decrypt password for: "
+                       << origin_url;
+          continue;
+        }
+      }
...
+  DeleteDatabaseTemp(temp_db_path);
+
+  LOG(INFO) << "mahoraga: Imported " << passwords.size() << " passwords";
+  if (skipped_app_bound > 0) {
+    LOG(WARNING) << "mahoraga: Skipped " << skipped_app_bound
