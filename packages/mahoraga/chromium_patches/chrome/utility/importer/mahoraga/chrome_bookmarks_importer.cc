diff --git a/chrome/utility/importer/mahoraga/chrome_bookmarks_importer.cc b/chrome/utility/importer/mahoraga/chrome_bookmarks_importer.cc
new file mode 100644
index 0000000000000..ff65fb5f3bc9e
--- /dev/null
+++ b/chrome/utility/importer/mahoraga/chrome_bookmarks_importer.cc
@@ -0,0 +1,248 @@
...
+    LOG(WARNING) << "mahoraga: Bookmarks file not found";
+    return result;
+  }
+
+  std::string bookmarks_content;
+  if (!base::ReadFileToString(bookmarks_path, &bookmarks_content)) {
+    LOG(WARNING) << "mahoraga: Failed to read Bookmarks file";
+    return result;
+  }
+
...
+    LOG(WARNING) << "mahoraga: Failed to parse Bookmarks JSON";
+    return result;
+  }
+
+  const base::DictValue* roots = bookmarks_value->GetDict().FindDict("roots");
+  if (!roots) {
+    LOG(WARNING) << "mahoraga: No roots in Bookmarks";
+    return result;
+  }
+
+  // Import bookmark bar
