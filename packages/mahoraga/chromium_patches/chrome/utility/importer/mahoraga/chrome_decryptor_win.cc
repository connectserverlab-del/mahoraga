diff --git a/chrome/utility/importer/mahoraga/chrome_decryptor_win.cc b/chrome/utility/importer/mahoraga/chrome_decryptor_win.cc
new file mode 100644
index 0000000000000..97d92c4309ffe
--- /dev/null
+++ b/chrome/utility/importer/mahoraga/chrome_decryptor_win.cc
@@ -0,0 +1,279 @@
...
+    LOG(WARNING) << "mahoraga: Failed to read Local State file";
+    return false;
+  }
+
+  auto parsed = base::JSONReader::Read(json_content, base::JSON_PARSE_RFC);
+  if (!parsed || !parsed->is_dict()) {
+    LOG(WARNING) << "mahoraga: Failed to parse Local State JSON";
+    return false;
+  }
+
...
+  if (!os_crypt) {
+    LOG(WARNING) << "mahoraga: No os_crypt section in Local State";
+    return false;
+  }
+
+  const std::string* encoded_key = os_crypt->FindString("encrypted_key");
+  if (!encoded_key || encoded_key->empty()) {
+    LOG(WARNING) << "mahoraga: No encrypted_key in os_crypt";
+    return false;
+  }
+
...
+  if (!decoded) {
+    LOG(WARNING) << "mahoraga: Failed to base64 decode encrypted_key";
+    return false;
+  }
+
...
+    LOG(WARNING) << "mahoraga: Encrypted key too short";
+    return false;
+  }
+
+  // Check for "DPAPI" prefix
+  if (!base::StartsWith(encrypted_data, kDpapiPrefix)) {
+    LOG(WARNING) << "mahoraga: Missing DPAPI prefix on key";
+    return false;
+  }
+
+  // Strip "DPAPI" prefix
...
+    LOG(WARNING) << "mahoraga: CryptUnprotectData failed for key with error: "
+                 << GetLastError();
+    return false;
+  }
+  return true;
...
+    LOG(WARNING) << "mahoraga: Invalid AES key size: " << key.size();
+    return false;
+  }
+
+  // Minimum: nonce (12) + auth tag (16) + at least 1 byte of data
+  if (ciphertext_length < kNonceLength + kAuthTagLength) {
+    LOG(WARNING) << "mahoraga: Ciphertext too short for AES-GCM";
+    return false;
+  }
+
...
+
+  if (!decrypted) {
+    LOG(WARNING) << "mahoraga: AES-GCM decryption failed";
+    return false;
+  }
+
...
+    LOG(WARNING) << "mahoraga: Local State file not found at: "
...
+    LOG(WARNING) << "mahoraga: Unexpected key length: "
...
+      LOG(WARNING) << "mahoraga: Empty ciphertext after v10 prefix";
