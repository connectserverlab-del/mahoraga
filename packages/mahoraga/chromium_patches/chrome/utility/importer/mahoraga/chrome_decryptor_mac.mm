diff --git a/chrome/utility/importer/mahoraga/chrome_decryptor_mac.mm b/chrome/utility/importer/mahoraga/chrome_decryptor_mac.mm
new file mode 100644
index 0000000000000..762582371aee4
--- /dev/null
+++ b/chrome/utility/importer/mahoraga/chrome_decryptor_mac.mm
@@ -0,0 +1,194 @@
...
+      LOG(WARNING) << "mahoraga: Chrome Safe Storage not found in Keychain";
+    } else if (error == errSecAuthFailed) {
+      LOG(WARNING) << "mahoraga: Keychain access denied";
+    } else {
+      LOG(WARNING) << "mahoraga: Keychain error: " << error;
+    }
+    return false;
+  }
+
...
+    LOG(WARNING) << "mahoraga: Invalid key size";
+    return false;
+  }
+
...
+                          key.data(), kIv)) {
+    LOG(WARNING) << "mahoraga: EVP_DecryptInit_ex failed";
+    return false;
+  }
+
...
+    LOG(WARNING) << "mahoraga: EVP_DecryptUpdate failed";
+    return false;
+  }
+
+  int final_length = 0;
...
+                           &final_length)) {
+    LOG(WARNING) << "mahoraga: EVP_DecryptFinal_ex failed - possible padding error";
+    return false;
+  }
+
...
+    LOG(WARNING) << "mahoraga: PBKDF2 key derivation failed";
+    if (result) {
