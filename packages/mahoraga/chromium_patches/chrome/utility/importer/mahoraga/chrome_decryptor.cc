diff --git a/chrome/utility/importer/mahoraga/chrome_decryptor.cc b/chrome/utility/importer/mahoraga/chrome_decryptor.cc
new file mode 100644
index 0000000000000..36088220e95b9
--- /dev/null
+++ b/chrome/utility/importer/mahoraga/chrome_decryptor.cc
@@ -0,0 +1,31 @@
+// Copyright 2024 AKW Technology Inc
+// Chrome decryption - Linux stub (deferred implementation)
+
+#include "chrome/utility/importer/mahoraga/chrome_decryptor.h"
+
+#include "base/logging.h"
+#include "build/build_config.h"
+
+namespace mahoraga_importer {
+
+#if BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS)
+
+std::string ExtractChromeKey(const base::FilePath& profile_path,
+                             KeyExtractionResult* result) {
+  LOG(INFO) << "mahoraga: Linux key extraction not yet implemented";
+  if (result) {
+    *result = KeyExtractionResult::kPlatformNotSupported;
+  }
+  return std::string();
+}
+
+DecryptResult DecryptChromeValue(const std::string& ciphertext,
+                                 const std::string& key,
+                                 std::string* plaintext) {
+  LOG(INFO) << "mahoraga: Linux decryption not yet implemented";
+  return DecryptResult::kError;
+}
+
+#endif  // BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS)
+
+}  // namespace mahoraga_importer
