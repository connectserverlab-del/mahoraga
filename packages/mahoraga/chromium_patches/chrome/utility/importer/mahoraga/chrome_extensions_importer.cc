diff --git a/chrome/utility/importer/mahoraga/chrome_extensions_importer.cc b/chrome/utility/importer/mahoraga/chrome_extensions_importer.cc
new file mode 100644
index 0000000000000..bd13aa92a2373
--- /dev/null
+++ b/chrome/utility/importer/mahoraga/chrome_extensions_importer.cc
@@ -0,0 +1,98 @@
...
+    LOG(WARNING) << "mahoraga: Failed to read "
...
+  if (!preferences) {
+    LOG(WARNING) << "mahoraga: Failed to parse JSON from "
+                 << preferences_path.BaseName().value();
+    return extension_ids;
+  }
+
...
+    LOG(WARNING) << "mahoraga: No preferences files found";
+    return extension_ids;
+  }
+
