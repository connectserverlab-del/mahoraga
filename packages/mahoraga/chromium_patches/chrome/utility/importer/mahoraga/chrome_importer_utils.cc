diff --git a/chrome/utility/importer/mahoraga/chrome_importer_utils.cc b/chrome/utility/importer/mahoraga/chrome_importer_utils.cc
new file mode 100644
index 0000000000000..3000a42871631
--- /dev/null
+++ b/chrome/utility/importer/mahoraga/chrome_importer_utils.cc
@@ -0,0 +1,65 @@
...
+    LOG(WARNING) << "mahoraga: Failed to create temp file for "
+                 << source_path.BaseName().value();
+    return base::FilePath();
+  }
+
+  if (!base::CopyFile(source_path, temp_path)) {
+    LOG(WARNING) << "mahoraga: Failed to copy "
