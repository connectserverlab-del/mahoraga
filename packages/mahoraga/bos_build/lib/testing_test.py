...
from .testing import MockMahoragaRoot, MockChromium, make_context
...
class MockChromiumTest(unittest.TestCase):
    ...
    def test_with_out_dir_creates_arch_dir_and_args_gn(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(out, m.src / "out" / "Default_mahoraga_arm64")
...
class MockMahoragaRootTest(unittest.TestCase):
    def test_version_files_written(self):
        with tempfile.TemporaryDirectory() as tmp:
            r = MockMahoragaRoot(Path(tmp))
            self.assertIn("MAJOR=137", (r.root / "CHROMIUM_VERSION").read_text())
            self.assertEqual(
                (r.root / "bos_build" / "config" / "MAHORAGA_BUILD_OFFSET")
                .read_text()
                .strip(),
                "80",
            )
            mahoraga_version = (r.root / "resources" / "MAHORAGA_VERSION").read_text()
            self.assertIn("MAHORAGA_MAJOR=0", mahoraga_version)
            self.assertIn("MAHORAGA_MINOR=31", mahoraga_version)
...
class MakeContextTest(unittest.TestCase):
    def test_context_loads_versions_from_mocks(self):
        with (
            ...
            r = MockMahoragaRoot(Path(root_tmp))
            ...
            self.assertEqual(ctx.mahoraga_chromium_version, "137.0.7231.69")
