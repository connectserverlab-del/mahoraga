...
MockMahoragaRoot builds a minimal packages/mahoraga root. Tests compose
...
class MockChromium:
    ...
    def with_out_dir(
        self, arch: str, args_gn: Optional[str] = None, product: str = "mahoraga"
    ) -> Path:
    ...
    def with_git(self) -> "MockChromium":
        ...
        self._git("config", "user.email", "tests@mahoraga.invalid")
        self._git("config", "user.name", "Mahoraga Build Tests")
...
class MockMahoragaRoot:
    """Minimal fake of the packages/mahoraga root (ctx.root_dir).
    ...
    def __init__(
        ...
        offset_file = root / "bos_build" / "config" / "MAHORAGA_BUILD_OFFSET"
        offset_file.parent.mkdir(parents=True, exist_ok=True)
        offset_file.write_text(f"{build_offset}\n")

        _write_version_file(
            root / "resources" / "MAHORAGA_VERSION",
            mahoraga_version,
            prefix="MAHORAGA_",
        )
    ...
    def add_replacement_file(
        self, relative_path: str, content: str, product: str = "mahoraga"
    ) -> Path:
