...
- bos_build/config/MAHORAGA_BUILD_OFFSET: added to chromium BUILD to
  produce the mahoraga chromium version (keeps our build numbers above
  upstream's for update ordering)
- resources/MAHORAGA_VERSION: the semantic product version
"""
...
def load_build_offset(root_dir: Path) -> str:
    """Read bos_build/config/MAHORAGA_BUILD_OFFSET."""
    version_file = join_paths(root_dir, "bos_build", "config", "MAHORAGA_BUILD_OFFSET")
    if version_file.exists():
...
def load_semantic_version(root_dir: Path) -> str:
    """Read resources/MAHORAGA_VERSION into e.g. "0.31.0".

    PATCH is only included when non-zero; a zero BUILD renders as ".0".
    """
    version_file = join_paths(root_dir, "resources", "MAHORAGA_VERSION")
    ...
    major = version_dict.get("MAHORAGA_MAJOR", "0")
    minor = version_dict.get("MAHORAGA_MINOR", "0")
    build = version_dict.get("MAHORAGA_BUILD", "0")
    patch = version_dict.get("MAHORAGA_PATCH", "0")
...
def derive_mahoraga_chromium_version(
    version_dict: Dict[str, str], build_offset: str
) -> str:
    """chromium version with BUILD shifted by the mahoraga offset."""
...
# resources/MAHORAGA_VERSION is the single source of update identity. The
...
# (chromium BUILD.PATCH inflated by MAHORAGA_BUILD_OFFSET, ~7950.97 at
...
# invisible to updaters; every release must bump the Mahoraga version.
UPDATE_FEED_EPOCH = 10000


def load_mahoraga_version_parts(root_dir: Path) -> tuple:
    """Load (major, minor, build, patch) ints from resources/MAHORAGA_VERSION."""
    version_file = join_paths(root_dir, "resources", "MAHORAGA_VERSION")
    if not version_file.exists():
        return ()
    ...
    return tuple(
        int(version_dict.get(f"MAHORAGA_{key}", "0"))
        for key in ("MAJOR", "MINOR", "BUILD", "PATCH")
    )


def update_feed_version(mahoraga_version_parts: tuple) -> str:
    """Epoch-prefixed feed version compared by Sparkle/WinSparkle."""
    if not mahoraga_version_parts:
        raise ValueError("resources/MAHORAGA_VERSION was not loaded")

    major, minor, build, patch = mahoraga_version_parts
