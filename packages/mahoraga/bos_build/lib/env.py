#!/usr/bin/env python3
"""
Environment variable configuration for Mahoraga build system
...
def _load_dotenv_file():
    ...
    mahoraga_root = get_package_root()
    project_root = mahoraga_root.parent.parent  # repo root

    # Try loading .env from multiple locations (most specific first)
    env_locations = [
        mahoraga_root / ".env",  # packages/mahoraga/.env
...
class EnvConfig:
    ...
    def r2_bucket(self) -> str:
        """R2 bucket name (default: mahoraga)"""
        return os.environ.get("R2_BUCKET", "mahoraga")

    @property
    def r2_cdn_base_url(self) -> str:
        """CDN base URL for R2 artifacts (default: http://cdn.mahoraga.com)"""
        return os.environ.get("R2_CDN_BASE_URL", "http://cdn.mahoraga.com")
