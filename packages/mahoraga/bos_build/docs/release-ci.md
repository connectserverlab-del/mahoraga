...
They are dispatch-only and keep Mahoraga and BrowserClaw releases independent:

```text
release-mahoraga.yml
  preflight
    - read packages/mahoraga/resources/MAHORAGA_VERSION for browser artifacts
  ...
  server resources, when include_servers=true
    - release-server.yml
  browser builds
    - release-linux.yml -> build-mahoraga.yml with products=mahoraga
    - release-windows.yml -> build-mahoraga.yml with products=mahoraga
    - release-macos.yml with products=mahoraga
  ...
  stage_updates
    ...
    - upload staged XML/JSON as staged-update-feeds-mahoraga-<version>
...
release-browserclaw.yml
  ...
  browser builds
    - release-linux.yml -> build-mahoraga.yml with products=browserclaw
    - release-windows.yml -> build-mahoraga.yml with products=browserclaw
...
| --- | --- | --- | --- |
| `.github/workflows/release-server.yml` | Builds Mahoraga server resource zips for every browser target, uploads versioned R2 resource keys, attaches server release assets, and reflects the server package version. | Manual and `agent-server/v*` tags | Yes, when `include_servers=true` and `products` includes `mahoraga` |
...
| `.github/workflows/release-cli.yml` | Builds mahoraga-cli release binaries, uploads them to CDN, publishes npm package metadata, and creates the CLI GitHub release. | `cli/v*` tags | No orchestrator use |
...
| `.github/workflows/release-mahoraga.yml` | Orchestrates one Mahoraga release, including server resources, selected browser platforms, optional agent CRX upload, staged feed artifacts, and draft GitHub release assets. | Manual only | No reusable entry point |
...
Browser artifacts use the Mahoraga browser version from
`packages/mahoraga/resources/MAHORAGA_VERSION` (for example `0.47.2.2`).
Server resources do not use that version. `release-server.yml` resolves
`packages/mahoraga-agent/apps/server/package.json` and tags
`agent-server/vX.Y.Z`; `release-claw-server.yml` resolves
`packages/mahoraga-agent/apps/claw-server/package.json` and tags
`claw-server/vX.Y.Z`; `release-claw-server-rust.yml` resolves
`packages/mahoraga-agent/apps/claw-server-rust/Cargo.toml` and tags
...
`packages/mahoraga/bos_build/config/download_resources.yaml` and the Rust copy
entries in `packages/mahoraga/bos_build/config/copy_resources.yaml` are kept as
...
`mahoraga-claw-server-rs` to the runtime name `mahoraga-claw-server`.
...
resources from the checked-out `packages/mahoraga-agent` tree. Its browser
build command leaves downloads enabled, so `download_resources` fetches the
published Mahoraga server bundle, the active Bun BrowserClaw server bundle, and
the onboarding bundle from R2 using the runner-local `packages/mahoraga/.env`
R2 credentials. Flip the commented Rust blocks only when intentionally shipping
the Rust server.

The reusable nesting depth is `release-mahoraga.yml` or
`release-browserclaw.yml` -> `release-linux.yml` or `release-windows.yml` ->
`build-mahoraga.yml`, which stays below GitHub's limit of four workflow
levels.
...
`build-mahoraga.yml` callers enabling such a profile must also pass
...
```bash
gh workflow run release-mahoraga.yml \
  -f platforms=all \
...
```bash
# Mahoraga Linux only, still stages feed previews where applicable.
gh workflow run release-mahoraga.yml \
  -f platforms=linux \
  -f extensions=skip
...
gh workflow run release-linux.yml -f products=mahoraga -f upload_to_r2=true
...
| Mahoraga server resources | R2 names plus `MAHORAGA_CONFIG_URL`, `POSTHOG_API_KEY`, `SENTRY_DSN` | Present | `AGENT_RUNNER_JWT_SECRET` is optional and inlined only when present. Per-product release preflight fails before starting paid builds if selected required names are absent. |
...
`packages/mahoraga/bos_build/docs/warpbuild-ci.md`; keep that document
...
uv run mahoraga release github create --version <version> --draft --product mahoraga
uv run mahoraga release github create --version <version> --draft --product browserclaw
```
...
```bash
cd packages/mahoraga
uv run mahoraga release appcast --version <version> --product <mahoraga|browserclaw>
uv run mahoraga release extensions --channel <alpha|prod> --set <agent|browserclaw>=<extension-version>
```

The job uploads the staged XML/JSON files as one artifact:

- `staged-update-feeds-mahoraga-<version>`
...
uv run mahoraga release list --version <version> --product mahoraga
uv run mahoraga release list --version <version> --product browserclaw

# Copy versioned R2 objects to live download/ aliases.
uv run mahoraga release publish --version <version> --product mahoraga
uv run mahoraga release publish --version <version> --product browserclaw

# Publish appcast changes after inspecting the staged artifact.
uv run mahoraga release appcast --version <version> --product mahoraga --publish
uv run mahoraga release appcast --version <version> --product browserclaw --publish

# Publish extension update manifests only if the per-product run built a CRX.
uv run mahoraga release extensions --channel alpha --set agent=<agent-extension-version> --publish
uv run mahoraga release extensions --channel alpha --set browserclaw=<browserclaw-extension-version> --publish
```
