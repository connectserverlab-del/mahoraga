...
| --- | --- | --- | --- | --- |
| `.github/workflows/nightly-mahoraga.yml` | Mahoraga | `0 4 * * *` | Scheduled runs bump `offset+build`, commit through the bot PR flow, and merge with `[skip ci]`. | `nightly-mahoraga` |
...
```bash
uv run mahoraga build --profile nightly-macos --product <product> --arch arm64 \
  --chromium-src "$CHROMIUM_SRC"
```

The profile is `packages/mahoraga/bos_build/profiles/nightly-macos.yaml`:
...
Mahoraga stages only the resources used by Mahoraga:
...
```text
packages/mahoraga/resources/binaries/mahoraga_server/darwin-arm64
packages/mahoraga/resources/binaries/mahoraga_claw_onboard
```

BrowserClaw stages the shared Mahoraga server bundle, the product-independent
...
`resources/binaries/mahoraga_claw_server/darwin-arm64`. The normal resources
...
```bash
packages/mahoraga-agent/scripts/build/claw-server-rust-local.sh \
  --target darwin-arm64 \
  --agent-root packages/mahoraga-agent \
  --mahoraga-root packages/mahoraga
```

That helper builds the Rust server natively with Cargo and stages
`resources/binaries/mahoraga_claw_server_rust/darwin-arm64`; the commented copy
block renames `mahoraga-claw-server-rs` to the runtime name
`mahoraga-claw-server`.
...
`packages/mahoraga/.env` on import, so the runner-local PEM values such as
`MAHORAGA_AGENT_V2_KEY` and `BROWSERCLAW_KEY` do not need to be exported in the
workflow.
...
`packages/mahoraga/.env`, and Chromium checkout. Unlike nightlies, releases do
...
```bash
uv run mahoraga build --preset release --product <product> --arch <arch> \
  --chromium-src "$CHROMIUM_SRC"
```
...
build Mahoraga first and BrowserClaw second in the same job. The workflow also
...
./config.sh --url https://github.com/<owner>/<repo> --token <REGISTRATION_TOKEN> \
  --labels mahoraga-builder --name mac-mini-builder --work _work
```

The workflows target:

```yaml
runs-on: [self-hosted, macOS, ARM64, mahoraga-builder]
```
...
- Build repo clone, for example `/Users/<user>/code/mahoraga-release`
...
- `packages/mahoraga/.env` with signing, notarization, R2, Slack, and extension PEM values
...
| --- | --- | --- |
| `MAHORAGA_REPO_PATH` | `/Users/<user>/code/mahoraga-release` | Persistent build repo clone. Use an absolute path. |
| `MAHORAGA_CHROMIUM_SRC` | `/Users/<user>/code/chromium-release/src` | Chromium `src` checkout. Use an absolute path. |
| `MAHORAGA_NIGHTLY_REF` | `main` | Optional; falls back to the repo default branch. |

## Version Policy

Only the Mahoraga nightly calls `bos_build/scripts/bump_version.py` with a
mutable bump mode.

- Mahoraga schedule: 04:00 UTC, `offset+build`, commit and push enabled, R2 upload enabled
- Mahoraga manual default: `offset+build`, commit disabled, R2 upload enabled
- Mahoraga manual hotfix option: choose `offset+patch`
- Mahoraga manual dry run option: choose `none`
...
`MAHORAGA_BUILD_OFFSET` is the internal Chromium-build monotonic counter.
`MAHORAGA_BUILD` advances the public nightly semantic version. `MAHORAGA_PATCH`
is reserved for manual hotfix-style builds because setting both build and patch
nonzero produces a four-part version.

Mahoraga nightly version commits use:
...
- Mahoraga: `bump`, `commit_version`, and `upload_to_r2`
...
```text
packages/mahoraga/releases/<version>/Mahoraga_v<version>_arm64.dmg
packages/mahoraga/releases/<version>/BrowserClaw_v<version>_arm64.dmg
```
...
confirm `MACOS_KEYCHAIN_PASSWORD` is present in `packages/mahoraga/.env`.
...
No Mahoraga version commit: check `commit_version`, the selected bump mode,
