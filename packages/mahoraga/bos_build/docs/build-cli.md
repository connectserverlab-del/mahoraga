...
Everything `mahoraga build` can do beyond the common invocations in
[`../README.md`](../README.md). Run these from `packages/mahoraga`.
...
mahoraga build --preset debug --gn-arg symbol_level=2 --gn-arg dcheck_always_on=true
```
...
```bash
mahoraga build --modules clean,compile,sign_macos --product mahoraga
mahoraga build --list                                # every available step
```
...
`mahoraga build -m series_patches` separately if you need them.
...
uv run mahoraga build --modules clean --chromium-src "$CHROMIUM_ROOT/src" -t release
uv run mahoraga source ensure --root "$CHROMIUM_ROOT" --step sync
uv run mahoraga build --profile nightly-ci --chromium-src "$CHROMIUM_ROOT/src"
```
...
WarpCache, `mahoraga source cache restore|save` handles the R2 checkout cache.
