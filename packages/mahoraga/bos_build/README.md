...
The build and release system for Mahoraga and BrowserClaw. One Python CLI
(`mahoraga`) turns a Chromium checkout into signed, packaged browsers, then
ships them.

Run everything from `packages/mahoraga`:

```bash
cd packages/mahoraga
uv sync                 # once
cp .env.example .env    # once, then fill in what you need
uv run mahoraga --help
```

Every `mahoraga …` command below is really `uv run mahoraga …`. Drop the
prefix if you have the venv activated.

## Read this first

**`mahoraga build` builds a binary. It does not release a product.**
...
| I want to… | Do this |
| --- | --- |
| Build on my machine | `mahoraga build --preset debug` |
| See exactly what a build will run | `mahoraga build --preset release --show-plan` |
| Release Mahoraga or BrowserClaw | `gh workflow run release-mahoraga.yml` |
| Make a staged release live | `mahoraga release publish`, then `mahoraga release appcast --publish` |
...
| Grab today's signed mac build | Download the `nightly-mahoraga` / `nightly-browserclaw` prerelease |
| Check the patch stack | `mahoraga dev doctor` |
...
- **product** — `mahoraga` or `browserclaw`. One file each,
...
```
bos_build/
  mahoraga.py  entry — the `mahoraga` Typer app (also `python -m bos_build`)
...
```bash
mahoraga build --preset release --show-plan
```
...
```bash
# Fast iteration.
mahoraga build --preset debug --chromium-src ~/chromium/src

# Signed local release build, macOS arm64.
mahoraga build --preset release --product browserclaw --arch arm64
...
mahoraga build --preset release --provision none --clean --product browserclaw --arch x64 --sign --upload --chromium-src C:\src\chromium-3\src

# Resume after a failure, without recompiling.
mahoraga build --preset release --from sign_macos

# Subtract steps from the composed plan.
mahoraga build --preset release --skip upload,series_patches
```
...
| --- | --- | --- |
| `release-ci` | `build-mahoraga.yml`, the reusable Linux/Windows release lane | `preset: release`, `clean: false`, `provision: none` — the workflow provisions and caches Chromium itself |
...
`packages/mahoraga/resources/MAHORAGA_VERSION`. You do not pass it.
...
Every input has a default, so a bare `gh workflow run release-mahoraga.yml`
...
```bash
gh workflow run release-mahoraga.yml -f platforms=linux -f extensions=skip
...
mahoraga release list --version <version> --product mahoraga
mahoraga release feeds status

# 2. Copy versioned R2 objects to the live download/ aliases.
mahoraga release publish --version <version> --product mahoraga

# 3. Diff the appcast, then publish it.
mahoraga release appcast --version <version> --product mahoraga
mahoraga release appcast --version <version> --product mahoraga --publish
```

Swap in `--product browserclaw` for the other product. If you need to recreate
the draft GitHub release by hand, that is
`mahoraga release github create --version <version> --draft --product <id>`.
...
mahoraga ext release --version 0.0.118 --name agent

# Feeds only, no CRX build. Pin versions; anything unset carries over from live.
mahoraga release extensions --channel alpha --set agent=0.0.118
mahoraga release extensions --channel alpha --set browserclaw=0.1.4 --publish
```
...
| --- | --- | --- | --- |
| Mahoraga agent server | `packages/mahoraga-agent/apps/server/package.json` | `release-server.yml` | `agent-server/v*` |
...
prereleases anyone can download: `nightly-mahoraga` (04:00 UTC) and
...
```bash
mahoraga dev doctor                            # .features.yaml <-> patches on disk
mahoraga dev doctor --against ~/chromium/src   # + which patches fail, by feature
mahoraga dev doctor --feature llm-chat --json  # filtered / machine-readable

mahoraga product list                          # registered products
mahoraga product doctor                        # identity uniqueness + branding assets
```
...
| Thing | Source |
| --- | --- |
| Browser version | `packages/mahoraga/resources/MAHORAGA_VERSION` |
| Chromium pin | `packages/mahoraga/CHROMIUM_VERSION`, `BASE_COMMIT` |
...
| Patch stack map | `packages/mahoraga/chromium_patches/.features.yaml` |
| Which server bundle ships | `config/download_resources.yaml`, `config/copy_resources.yaml` |
| Local secrets | `packages/mahoraga/.env` (copy `.env.example`) |
...
there is needed to build Mahoraga):
