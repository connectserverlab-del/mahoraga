...
def _alpha_content(bundle_id="mahoraga-server"):
...
class BundleDerivationTest(unittest.TestCase):
    def test_mahoraga_defaults_keep_todays_literals(self):
        ...
        self.assertEqual(
            ...
            "mahoraga_server_0.0.9_darwin_arm64.zip",
        )
...
class PromoteContentTest(unittest.TestCase):
    def test_promote_rerenders_with_prod_metadata(self):
        prod_spec = server_feed("mahoraga-server", "prod")

        promoted = promote_appcast_content(_alpha_content(), prod_spec)

        self.assertIn("<title>Mahoraga Server</title>", promoted)
        self.assertIn(
            "<link>https://cdn.mahoraga.com/appcast-server.xml</link>", promoted
        )
    ...
    def test_promote_refuses_source_without_enclosures(self):
        ...
        with self.assertRaisesRegex(ValueError, "enclosure"):
            promote_appcast_content(empty, server_feed("mahoraga-server", "prod"))
