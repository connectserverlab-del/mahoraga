#!/usr/bin/env python3
"""Server OTA module for Mahoraga Server binary updates"""
...
class ServerOTAModule(Step):
    """OTA update module for Mahoraga Server binaries
    ...
    requires = []
    description = "Create and upload Mahoraga Server OTA update"
    ...
    def execute(self, context: Context) -> None:
        ctx = context
        log_info(f"\n🚀 Mahoraga Server OTA v{self.version} ({self.channel})")
        log_info("=" * 70)
    ...
    def _finalize_release(
        ...
        log_info(
            "\n📋 Next step: Run 'mahoraga ota server release-appcast "
