...
from ...products.mahoraga.product import MAHORAGA_SERVER_BUNDLE
...
class SignServerBundleWindowsTest(unittest.TestCase):
    ...
    def test_signs_mahoraga_descriptor_binary(self):
        self._assert_signs_bundle_binary(
            MAHORAGA_SERVER_BUNDLE, "mahoraga_server.exe"
        )

    def test_signs_browserclaw_descriptor_binary(self):
        self._assert_signs_bundle_binary(
            BROWSERCLAW_SERVER_BUNDLE, "mahoraga-claw-server.exe"
        )

    def test_fails_when_browserclaw_descriptor_binary_is_missing(self):
        with tempfile.TemporaryDirectory() as tmp:
            resources = Path(tmp) / "resources"
            _write_exe(resources / "bin" / "mahoraga_server.exe")
    ...
    def test_preflights_all_descriptor_binaries_before_signing(self):
        bundle = replace(
            ...
            windows_binaries=("mahoraga-claw-server.exe", "missing.exe"),
        )
        with tempfile.TemporaryDirectory() as tmp:
            resources = Path(tmp) / "resources"
            _write_exe(resources / "bin" / "mahoraga-claw-server.exe")
...
class SignWindowsBinaryLoggingTest(unittest.TestCase):
    def test_redacts_credentials_echoed_in_codesigntool_error(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            binary = root / "mahoraga_server.exe"
            _write_exe(tool)
            _write_exe(binary)
            env = cast(
                EnvConfig,
