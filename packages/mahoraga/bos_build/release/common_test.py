...
ARTIFACT = {
    "url": "https://cdn.mahoraga.com/releases/0.31.0/win/Mahoraga_v0.31.0_x64_installer.exe",
...
# get_download_path_mapping(mahoraga) must stay byte-identical to it.
MAHORAGA_DOWNLOAD_GOLDEN = {
    "macos": {
        "arm64": "download/Mahoraga-arm64.dmg",
        "x64": "download/Mahoraga-x86_64.dmg",
        "universal": "download/Mahoraga.dmg",
    },
    "win": {
        "x64_installer": "download/Mahoraga_installer.exe",
    },
    "linux": {
        "x64_appimage": "download/Mahoraga.AppImage",
        "x64_deb": "download/Mahoraga.deb",
        "arm64_appimage": "download/Mahoraga-arm64.AppImage",
        "arm64_deb": "download/Mahoraga-arm64.deb",
    },
}
...
class DownloadPathMappingTest(unittest.TestCase):
    def test_mahoraga_mapping_matches_golden_constant(self):
        self.assertEqual(
            get_download_path_mapping(get_product_descriptor("mahoraga")),
            MAHORAGA_DOWNLOAD_GOLDEN,
        )

    def test_default_product_mapping_is_mahoraga(self):
        self.assertEqual(get_download_path_mapping(), MAHORAGA_DOWNLOAD_GOLDEN)
...
class ListAllVersionsTest(unittest.TestCase):
    ...
    def test_returns_empty_without_r2_config(self):
        self.assertEqual(list_all_versions("mahoraga", _env(configured=False)), [])
...
class ListLegacyVersionsTest(unittest.TestCase):
    def test_keeps_only_version_shaped_prefixes(self):
        client = _FakeR2Client(
            {
                "releases/": [
                    _page("releases/", ["mahoraga", "browserclaw", "0.28.4", "0.29.0"])
                ]
            }
        )
