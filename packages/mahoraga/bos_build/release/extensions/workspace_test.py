...
class ResolveSourceTest(unittest.TestCase):
    def setUp(self):
        ...
        (self.monorepo / "packages/mahoraga-agent").mkdir(parents=True)
    ...
    def test_in_repo_resolves_to_working_tree_without_git(self):
        ...
        self.assertEqual(path, self.monorepo / "packages/mahoraga-agent")
    ...
    def test_external_fresh_clone_command(self):
        ...
        dest = self.work_root / "repos" / "Mahoraga-feedback-extension"
            ...
            [
                (
                    (
                        ...
                        "--branch",
                        "main",
                        "https://github.com/connectserverlab-del/mahoraga-feedback-extension.git",
                        str(dest),
                    ),
                    None,
                )
    ...
    def test_external_existing_updates_and_branch_override(self):
        git = RecordingGit()
        dest = self.work_root / "repos" / "Mahoraga-agent"
    ...
    def test_clone_url_embeds_gh_token_when_set(self):
        with patch.dict("os.environ", {"GH_TOKEN": "secret123"}):
            self.assertEqual(
                clone_url("connectserverlab-del/mahoraga-agent"),
                "https://x-access-token:secret123@github.com/connectserverlab-del/mahoraga-agent.git",
            )
        with patch.dict("os.environ", {}, clear=False):
            ...
            self.assertEqual(
                clone_url("connectserverlab-del/mahoraga-agent"),
                "https://github.com/connectserverlab-del/mahoraga-agent.git",
            )
...
class GitErrorRedactionTest(unittest.TestCase):
    def test_token_in_clone_args_and_stderr_is_masked(self):
        args = [
            ...
            "--branch",
            "main",
            "https://x-access-token:tok3n@github.com/connectserverlab-del/x.git",
            "/work/repos/x",
        ]
        stderr = (
            "fatal: repository "
...
class UpdateManifestVersionTest(unittest.TestCase):
    def test_bumps_version_and_preserves_other_keys(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            manifest.write_text(
                json.dumps({"name": "@mahoraga/app", "version": "0.0.1", "private": True})
            )
            update_manifest_version(manifest, "9.8.7")
            content = manifest.read_text()
            data = json.loads(content)
            self.assertEqual(data["version"], "9.8.7")
            self.assertEqual(data["name"], "@mahoraga/app")
...
class SpecSourceTypesTest(unittest.TestCase):
    def test_external_specs_expose_repo_names_for_work_dirs(self):
        ...
        self.assertEqual(source.repo.split("/")[-1], "Mahoraga-agent")
