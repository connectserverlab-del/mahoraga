...
ALL_KEYS = {
    "MAHORAGA_AGENT_V2_KEY": "agent-pem",
    "MAHORAGA_CONTROLLER_KEY": "controller-pem",
...
def _ctx(has_r2=True, bucket="mahoraga"):
    return cast(
        Context,
        SimpleNamespace(
...
class ValidateTest(unittest.TestCase):
    ...
    def test_missing_signing_key_env_names_the_variable(self):
        with patch.dict("os.environ", {}, clear=False):
            ...
            os.environ.pop("MAHORAGA_AGENT_V2_KEY", None)
            with patch(f"{MODULE}.find_chrome_binary", return_value="chrome"):
                with self.assertRaisesRegex(
                    ValidationError, "MAHORAGA_AGENT_V2_KEY"
                ):
...
class ExecuteTest(unittest.TestCase):
    ...
    def test_agent_flow_order_and_arguments(self):
        ...
        self.assertIn("VITE_PUBLIC_MAHORAGA_API", env_args[1])
