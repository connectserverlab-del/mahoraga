...
from ...core.products import (
    ...
    MAHORAGA_CONTROLLER_EXTENSION_ID,
...
EXTENSION_SPECS: Tuple[ExtensionSpec, ...] = (
    ExtensionSpec(
        name="agent",
        source=InRepoSource(path="packages/mahoraga-agent"),
        ...
        extension_id=MAHORAGA_AGENT_EXTENSION_ID,
        signing_key_env="MAHORAGA_AGENT_V2_KEY",
        env=(
    ...
    ExtensionSpec(
        name="controller",
        source=ExternalRepoSource(repo="connectserverlab-del/mahoraga-agent", branch="main"),
        ...
        extension_id=MAHORAGA_CONTROLLER_EXTENSION_ID,
        signing_key_env="MAHORAGA_CONTROLLER_KEY",
        env=("NODE_ENV", "POSTHOG_API_KEY"),
    ),
    ExtensionSpec(
        name="bugreporter",
        source=ExternalRepoSource(
            repo="connectserverlab-del/mahoraga-feedback-extension", branch="main"
        ),
        ...
        extension_id=MAHORAGA_BUG_REPORTER_EXTENSION_ID,
    ...
    ExtensionSpec(
        ...
        source=InRepoSource(path="packages/mahoraga-agent"),
        pre_build="bun ci",
        build="bun run --filter @mahoraga/claw-app build",
