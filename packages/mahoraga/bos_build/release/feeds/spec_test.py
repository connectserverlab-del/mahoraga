...
class FeedTableTest(unittest.TestCase):
    ...
    def test_mahoraga_browser_feeds_are_publishable(self):
        feeds = browser_feeds_for_product("mahoraga")
        self.assertTrue(all(feed.publishable for feed in feeds))

    def test_mahoraga_browser_feed_derivation(self):
        by_key = {feed.key: feed for feed in browser_feeds_for_product("mahoraga")}

        mac = by_key["appcast.xml"]
        self.assertEqual(mac.platform, "macos")
        self.assertEqual(mac.artifact_keys, ("universal", "arm64"))
        self.assertEqual(mac.title, "Mahoraga")
        ...
        self.assertEqual(win.title, "Mahoraga Windows Updates")
    ...
    def test_server_feed_lookup_and_titles(self):
        prod = server_feed("mahoraga-server", "prod")
        self.assertEqual(prod.key, "appcast-server.xml")
        self.assertEqual(prod.title, "Mahoraga Server")
        self.assertEqual(prod.product, "mahoraga")

        alpha = server_feed("browserclaw-server", "alpha")
        self.assertEqual(alpha.key, "appcast-claw-server.alpha.xml")
        self.assertEqual(alpha.title, "Mahoraga Claw Server (Alpha)")
...
class ExtensionRegistryTest(unittest.TestCase):
    def test_registry_matches_product_constants(self):
        ...
        self.assertEqual(
            by_name["agent"].extension_id, MAHORAGA_AGENT_EXTENSION_ID
        )
        self.assertEqual(
