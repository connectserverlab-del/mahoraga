...
CDN_BASE_URL = "https://cdn.mahoraga.com"
...
EXTENSIONS: Tuple[ExtensionSpec, ...] = (
    ExtensionSpec("agent", MAHORAGA_AGENT_EXTENSION_ID, True),
    ExtensionSpec("bugreporter", MAHORAGA_BUG_REPORTER_EXTENSION_ID, True),
...
# Browser feed key infix per product ("" keeps today's mahoraga keys).
_BROWSER_FEED_SLUGS = {"mahoraga": "", "browserclaw": "claw"}

# Products whose shipping updater selects this feed by mahoraga::GetProduct()
...
# the mahoraga feed, so its feeds stay unpublishable until that client lands.
_BROWSER_FEED_CLIENTS = frozenset({"mahoraga", "browserclaw"})

# Server feed key slug per bundle (appcast-<slug>[.alpha].xml).
_SERVER_FEED_SLUGS = {
    "mahoraga-server": "server",
...
def _browser_feeds(product_id: str) -> Tuple[FeedSpec, ...]:
    ...
    mahoraga::GetProduct() — see _BROWSER_FEED_CLIENTS.
    """
