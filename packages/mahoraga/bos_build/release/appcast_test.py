...
def _artifact(filename: str) -> dict:
    return {
        "filename": filename,
        "url": f"https://cdn.mahoraga.com/releases/mahoraga/0.47.0.2/{filename}",
...
def _macos_release(*artifact_keys: str) -> dict:
    names = {
        "arm64": "Mahoraga_v0.47.0.2_arm64.dmg",
        "x64": "Mahoraga_v0.47.0.2_x86_64.dmg",
        "universal": "Mahoraga_v0.47.0.2_universal.dmg",
    }
    return {
...
def _win_release(*artifact_keys: str) -> dict:
    names = {
        "x64_installer": "Mahoraga_v0.47.0.2_x64_installer.exe",
        "arm64_installer": "Mahoraga_v0.47.0.2_arm64_installer.exe",
    }
    return {
        "sparkle_version": "10000.0.47.0.2",
        "build_date": "2026-06-19T06:41:33Z",
...
class AppcastModuleTest(unittest.TestCase):
    ...
    def test_mac_arm64_only_feeds_appcast_xml_and_skips_x86_feed(self):
        ...
        self.assertIn("Mahoraga_v0.47.0.2_arm64.dmg", self.publisher.calls[0].content)
    ...
    def test_validate_refuses_publishing_unpublishable_feed(self):
        ...
        unpublishable = tuple(
            ...
            for feed in browser_feeds_for_product("mahoraga")
        )
        module = AppcastModule(product_id="mahoraga", publish=True)
