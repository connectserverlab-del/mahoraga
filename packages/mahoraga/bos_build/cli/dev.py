...
def main(
    ...
    Extract patches from commits:
      mahoraga dev extract commit HEAD
      mahoraga dev extract range HEAD~5 HEAD
      mahoraga dev extract patch chrome/common/foo.h

    Check patch-stack health (read-only):
      mahoraga dev doctor
      mahoraga dev doctor --against ~/chromium/src --json

    Applying and syncing patches is handled by bpatch
    (packages/mahoraga/tools/bpatch).
    """
