#!/usr/bin/env python3
"""Release CLI - Modular release automation for Mahoraga"""
...
def main(
    ...
    """Release automation for Mahoraga

    \b
    Commands:
      mahoraga release list                           # Newest releases per product
      mahoraga release list 0.31.0                    # Artifacts for a version
      mahoraga release appcast --version 0.31.0       # Generate appcast XML
      mahoraga release publish --version 0.31.0       # Publish to download/ paths
      mahoraga release download --version 0.31.0      # Download all artifacts
      mahoraga release github create --version 0.31.0

    Use --product to target a specific product (default: mahoraga).
    """
    if show_modules:
...
def list_releases(
    ...
    \b
    Examples:
      mahoraga release list                       # Newest 5 per product
      mahoraga release list --all                 # Every version
      mahoraga release list -n 10                 # Newest 10 per product
      mahoraga release list --product browserclaw # One product only
      mahoraga release list 0.31.0                # Artifact details
    """
...
def download(
    ...
    \b
    Examples:
      mahoraga release download --version 0.31.0
      mahoraga release download --version 0.31.0 --os macos
      mahoraga release download --version 0.31.0 --output ./downloads
    """
    release_ctx = create_release_context(version, product=product)
...
def github_create(
    ...
    Examples:
      mahoraga release github create --version 0.31.0
      mahoraga release github create --version 0.31.0 --publish  # Also publish to download/
      mahoraga release github create --version 0.31.0 --no-draft # Create published release
    """
