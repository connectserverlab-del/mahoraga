#!/usr/bin/env python3
"""Build CLI - Modular build system for Mahoraga"""
...
def main(
    ...
    product: Optional[str] = typer.Option(
        ...
        "--product",
        "-p",
        help="Product to build (mahoraga, browserclaw)",
    ),
    ...
    lock_wait: bool = typer.Option(
        False,
        "--lock-wait",
        help="Wait for another Mahoraga build using the same Chromium checkout "
    ...
    """Mahoraga Build System - Modular pipeline executor

    Build Mahoraga with a preset (planner-composed), phase flags, or
    explicit modules.

    \b
    Presets (Recommended - one pipeline definition, switches select):
      mahoraga build --preset release --product mahoraga --arch arm64
      mahoraga build --preset release --product browserclaw --no-upload
      mahoraga build --profile nightly-ci --arch x64
      mahoraga build --preset debug
    ...
    Phase Flags (Auto-Ordered):
      mahoraga build --setup --build --sign --package
      mahoraga build --build --sign           # Skip setup

    \b
    Explicit Modules (Power Users):
      mahoraga build --modules clean,compile,sign_macos

    \b
    List Available:
      mahoraga build --list                   # Show all modules and phases
    """
    ...
    if options_provided > 1:
        ...
        log_error("Examples:")
        log_error("  mahoraga build --preset release --product browserclaw")
        log_error("  mahoraga build --setup --build --sign")
        log_error("  mahoraga build --modules clean,compile")
        raise typer.Exit(1)
    ...
    log_info("🚀 Mahoraga Build System")
    log_info("=" * 70)
...
def _execute_runs(
    ...
    *,
    ...
    if has_flags:
        ...
        if prep:
            log_warning(
                "⚠️  --prep does NOT apply series_patches. Run 'mahoraga build -m series_patches' separately if needed."
            )
    ...
    log_info(f"📍 Build offset: {summary_ctx.mahoraga_build_offset}")
    if len(runs) > 1:
