...
from bos_build.mahoraga import app
...
class ReleaseAppcastCliTest(unittest.TestCase):
    def setUp(self):
        ...
        self.paths = {
            "mahoraga-server": self.root / "appcast-server.xml",
    ...
    def _publisher(self) -> FeedPublisher:
        return FeedPublisher(
            env=SimpleNamespace(r2_bucket="mahoraga"),
    ...
    def test_empty_default_feed_names_selection_and_hints_browserclaw(self):
        self.paths["mahoraga-server"].write_text(
            _appcast("mahoraga-server", "prod")
        )
        ...
        self.assertIn(
            "Resolved appcast: product=mahoraga bundle=mahoraga-server "
            "channel=prod spec=appcast-server.xml "
            f"source={self.paths['mahoraga-server'].resolve()}",
            output,
        )
        self.assertIn(
            f"appcast-server.xml has no <item> entries: "
            f"{self.paths['mahoraga-server'].resolve()}",
            output,
        )
        self.assertIn(
    ...
    def test_custom_empty_file_does_not_suggest_another_product(self):
        ...
        custom.write_text(_appcast("mahoraga-server", "prod"))
        self.paths["browserclaw-server"].write_text(
            _appcast("browserclaw-server", "prod", "0.0.12")
        )
