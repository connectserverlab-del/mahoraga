#!/usr/bin/env python3
"""OTA CLI - Over-The-Air update automation for Mahoraga"""
...
server_app = typer.Typer(
    help="Mahoraga Server OTA commands",
    pretty_exceptions_enable=False,
    pretty_exceptions_show_locals=False,
)
...
def server_release(
    ...
    product: str = typer.Option(
        "mahoraga", "--product", help="Product whose server bundle to release"
    ),
):
    """Publish Mahoraga server OTA update
    ...
    Full Release (all platforms):
      mahoraga ota server release --version 0.0.69 --channel alpha

    \b
    Single Platform:
      mahoraga ota server release --version 0.0.69 --platform darwin_arm64

    \b
    Multiple Platforms:
      mahoraga ota server release --version 0.0.69 --platform darwin_arm64,darwin_x64
    """
    log_info(f"🚀 Mahoraga Server OTA v{version}")
    log_info("=" * 70)
...
def server_release_appcast(
    ...
    product: str = typer.Option(
        "mahoraga", "--product", help="Product whose server appcast to publish"
    ),
    ...
    Preview the alpha appcast (dry run):
      mahoraga ota server release-appcast --channel alpha

    \b
    Publish it:
      mahoraga ota server release-appcast --channel alpha --publish
    """
    ...
    if not source_path.exists():
        ...
        if not appcast_file:
            log_error(
                "Run 'mahoraga ota server release' first to generate the appcast"
            )
        raise typer.Exit(1)
...
def server_promote(
    product: str = typer.Option(
        "mahoraga", "--product", help="Product whose server appcast to promote"
    ),
    publish: bool = typer.Option(
        False,
        "--publish",
    ...
    \b
    Preview (dry run):
      mahoraga ota server promote

    \b
    Promote for real:
      mahoraga ota server promote --publish
    """
    bundle_id = _server_bundle_id(product)
...
def test_signing(
    ...
    \b
    Example:
      mahoraga ota test-signing /path/to/file.zip
    """
...
def server_main(ctx: typer.Context):
    """Mahoraga Server OTA commands

    \b
    Release (upload artifacts):
      mahoraga ota server release --version 0.0.36
    ...
    \b
    List Platforms:
      mahoraga ota server list-platforms
    """
...
def main(ctx: typer.Context):
    """OTA update automation for Mahoraga
