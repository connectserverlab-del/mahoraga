#!/usr/bin/env python3
"""Bump Mahoraga version files for CI and print the resulting version."""
...
def _semantic_version(version_text: str) -> str:
    major = _read_int_setting(version_text, "MAHORAGA_MAJOR")
    minor = _read_int_setting(version_text, "MAHORAGA_MINOR")
    build = _read_int_setting(version_text, "MAHORAGA_BUILD")
    patch = _read_int_setting(version_text, "MAHORAGA_PATCH")
    if patch != 0:
...
def bump_version(package_root: Path, mode: str) -> str:
    """Apply the requested CI version bump and return Mahoraga semantic version."""
    if mode not in BUMP_MODES:
        raise ValueError(f"Unsupported bump mode: {mode}")

    offset_file = package_root / "bos_build" / "config" / "MAHORAGA_BUILD_OFFSET"
    version_file = package_root / "resources" / "MAHORAGA_VERSION"
    offset = int(offset_file.read_text().strip()) if mode != "none" else None
    version_text = version_file.read_text()
    next_version_text = version_text
    if mode == "offset+build":
        next_version_text = _bump_version_key(version_text, "MAHORAGA_BUILD")
        ...
        next_version_text = _bump_version_key(version_text, "MAHORAGA_PATCH")
