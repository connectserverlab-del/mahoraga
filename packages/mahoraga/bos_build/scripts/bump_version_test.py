#!/usr/bin/env python3
"""Tests for the Mahoraga CI version bump script."""
...
class BumpVersionTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.offset_file = self.root / "bos_build" / "config" / "MAHORAGA_BUILD_OFFSET"
        self.version_file = self.root / "resources" / "MAHORAGA_VERSION"
    ...
    def test_offset_and_build_bumps_internal_and_semantic_versions(self) -> None:
        ...
        self.assertIn("MAHORAGA_BUILD=1\n", self.version_file.read_text())
        self.assertIn("MAHORAGA_PATCH=0\n", self.version_file.read_text())
    ...
    def test_offset_and_build_resets_stale_patch(self) -> None:
        ...
        self.assertEqual(self.offset_file.read_text(), "147\n")
        self.assertIn("MAHORAGA_BUILD=3\n", self.version_file.read_text())
        self.assertIn("MAHORAGA_PATCH=0\n", self.version_file.read_text())
    ...
    def test_offset_only_preserves_semantic_version(self) -> None:
        ...
        self.assertEqual(self.offset_file.read_text(), "147\n")
        self.assertIn("MAHORAGA_BUILD=0\n", self.version_file.read_text())
    ...
    def test_offset_and_patch_outputs_four_part_version(self) -> None:
        ...
        self.assertEqual(self.offset_file.read_text(), "147\n")
        self.assertIn("MAHORAGA_PATCH=1\n", self.version_file.read_text())
    ...
    def test_malformed_version_does_not_write_offset(self) -> None:
        self.version_file.write_text("MAHORAGA_MAJOR=0\n")

        with self.assertRaisesRegex(ValueError, "Missing MAHORAGA_BUILD"):
