...
class GetAppPathTest(unittest.TestCase):
    def test_extensions_manifest_url_uses_dedicated_bundled_manifest(self):
        ...
        self.assertEqual(
            ...
            "https://cdn.mahoraga.com/extensions/bundled-manifest.xml",
        )
    ...
    def test_arch_build_ignores_stale_universal_app(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            stale_universal = (
                chromium_src
                / "out"
                / "Default_mahoraga_universal"
                / ctx.MAHORAGA_APP_NAME
            )
            stale_universal.mkdir(parents=True)

            fresh_arm64 = chromium_src / ctx.out_dir / ctx.MAHORAGA_APP_NAME
    ...
    def test_universal_architecture_resolves_universal_out_dir(self):
        ...
        expected = Path("/nonexistent-src") / ctx.out_dir / ctx.MAHORAGA_APP_NAME
        self.assertTrue(str(ctx.out_dir).endswith("Default_mahoraga_universal"))
    ...
    def test_browserclaw_context_derives_names_and_paths(self):
        with (
            ...
            self.assertEqual(ctx.MAHORAGA_APP_BASE_NAME, "BrowserClaw")
            self.assertEqual(ctx.MAHORAGA_APP_NAME, "BrowserClaw.app")
    ...
    def test_debug_gn_args_allow_override_and_package_all(self):
        ...
        self.assertEqual(
            ...
            [
                ...
                "mahoraga_allow_runtime_product_override = true",
                "mahoraga_package_all_server_resources = true",
            ],
        )
        ...
        for product in ("mahoraga", "browserclaw"):
            with self.subTest(product=product):
                ...
                self.assertEqual(
                    ctx.get_product_gn_args(),
                    [
                        f'mahoraga_product = "{product}"',
                        "mahoraga_allow_runtime_product_override = false",
                        "mahoraga_package_all_server_resources = false",
                    ],
                )
