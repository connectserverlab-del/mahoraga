...
class Context:
    ...
    mahoraga_build_offset: str = ""
    mahoraga_version_parts: tuple = ()  # (major, minor, build, patch) ints
    mahoraga_chromium_version: str = ""
    semantic_version: str = ""  # e.g. "0.31.0" from resources/MAHORAGA_VERSION
    release_version: str = (
    ...
    def __post_init__(self):
        ...
        if not self.mahoraga_build_offset:
            self.mahoraga_build_offset = versions_mod.load_build_offset(self.root_dir)

        if not self.semantic_version:
            self.semantic_version = versions_mod.load_semantic_version(self.root_dir)

        if not self.mahoraga_version_parts:
            self.mahoraga_version_parts = versions_mod.load_mahoraga_version_parts(
                self.root_dir
            )

        if not self.mahoraga_chromium_version:
            self.mahoraga_chromium_version = (
                versions_mod.derive_mahoraga_chromium_version(
                    version_dict, self.mahoraga_build_offset
                )
            )
    ...
    @property
    def MAHORAGA_APP_BASE_NAME(self) -> str:
        return self.product.app_base_name

    @property
    def MAHORAGA_APP_NAME(self) -> str:
        if IS_WINDOWS():
    ...
    def get_extensions_manifest_url(self) -> str:
        ...
        return "https://cdn.mahoraga.com/extensions/bundled-manifest.xml"
    ...
    def get_app_path(self) -> Path:
        ...
        return join_paths(self.chromium_src, self.out_dir, self.MAHORAGA_APP_NAME)
    ...
    def get_mahoraga_chromium_version(self) -> str:
        """Get mahoraga chromium version string"""
        return self.mahoraga_chromium_version

    def get_mahoraga_version(self) -> str:
        """Get mahoraga version string (build offset)"""
        return self.mahoraga_build_offset

    def get_semantic_version(self) -> str:
        """Get semantic version from resources/MAHORAGA_VERSION"""
        return self.semantic_version

    def get_sparkle_version(self) -> str:
        """Update feed version compared by Sparkle/WinSparkle.

        Epoch-prefixed Mahoraga version (see the derivation notes in
        ...
        """
        return versions_mod.update_feed_version(self.mahoraga_version_parts)

    def get_release_path(self, platform: str) -> str:
        """Get R2 path for release artifacts, e.g. "releases/mahoraga/0.31.0/macos/"."""
        return (
    ...
    def get_product_gn_args(self) -> list[str]:
        ...
        return [
            f'mahoraga_product = "{self.product.gn_product}"',
            f"mahoraga_allow_runtime_product_override = {dev_build}",
            f"mahoraga_package_all_server_resources = {dev_build}",
        ]
