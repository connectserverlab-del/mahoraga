#!/usr/bin/env python3
"""Exclusive lock for Mahoraga builds that mutate a Chromium checkout."""
...
class ChromiumCheckoutLock:
    ...
    Mahoraga release builds run destructive setup (`git reset`, `git clean`,
...
def lock_path_for(chromium_src: Path) -> Path:
    ...
    return resolved.parent / ".mahoraga-build-locks" / f"{resolved.name}-{digest}.lock"
...
def _format_contention(
    *, chromium_src: Path, lock_path: Path, holder: Optional[dict[str, object]]
    ...
    lines = [
        ...
        "Another Mahoraga build is using this checkout; refusing to run because "
