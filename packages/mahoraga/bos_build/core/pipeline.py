#!/usr/bin/env python3
"""Pipeline validation for Mahoraga build system"""
...
def show_available_modules(available_modules: Dict[str, Type[Step]]) -> None:
    ...
    log_info("=" * 70)
    log_info("  mahoraga build --modules clean,git_setup,configure,compile")
    log_info("  mahoraga build --modules compile,sign_macos,package_macos")
    log_info("  mahoraga build --preset release --product mahoraga --arch arm64")
    log_info("=" * 70 + "\n")
