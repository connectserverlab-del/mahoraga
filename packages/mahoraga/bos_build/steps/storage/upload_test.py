...
def _upload_ctx(
    dist_dir: Path,
    *,
    product_id: str = "mahoraga",
    display_name: str = "Mahoraga",
    artifact_prefix: str = "Mahoraga",
) -> SimpleNamespace:
    return SimpleNamespace(
        env=SimpleNamespace(
            r2_bucket="mahoraga",
            r2_cdn_base_url="https://cdn.mahoraga.com",
        ...
        mahoraga_chromium_version="136.0.0.0.1",
...
class UploadMetadataTest(unittest.TestCase):
    def test_linux_x64_artifacts_use_x64_keys(self) -> None:
        self.assertEqual(
            _get_artifact_key("Mahoraga_v1.2.3_x64.AppImage", "linux"),
            "x64_appimage",
        )
        self.assertEqual(
            _get_artifact_key("Mahoraga_v1.2.3_amd64.deb", "linux"),
            "x64_deb",
        )

    def test_linux_arm64_artifacts_use_arm64_keys(self) -> None:
        self.assertEqual(
            _get_artifact_key("Mahoraga_v1.2.3_arm64.AppImage", "linux"),
            "arm64_appimage",
        )
        self.assertEqual(
            _get_artifact_key("Mahoraga_v1.2.3_arm64.deb", "linux"),
            "arm64_deb",
        )
        self.assertEqual(
            _get_artifact_key("Mahoraga_v1.2.3_aarch64.deb", "linux"),
            "arm64_deb",
        )

    def test_windows_installer_artifacts_use_arch_keys(self) -> None:
        self.assertEqual(
            _get_artifact_key("Mahoraga_v1.2.3_x64_installer.exe", "win"),
            "x64_installer",
        )
        self.assertEqual(
            _get_artifact_key("Mahoraga_v1.2.3_x64_installer.zip", "win"),
            "x64_zip",
        )
        self.assertEqual(
            _get_artifact_key("Mahoraga_v1.2.3_arm64_installer.exe", "win"),
            "arm64_installer",
        )
        self.assertEqual(
            _get_artifact_key("Mahoraga_v1.2.3_arm64_installer.zip", "win"),
            "arm64_zip",
        )
    ...
    def test_upload_attaches_macos_dmg_signature_metadata_by_filename(self) -> None:
        with (
            ...
            dist_dir = Path(tmp)
            dmg_name = "Mahoraga_v1.2.3_arm64.dmg"
    ...
    def test_upload_keeps_same_product_macos_arch_artifacts(self) -> None:
        with (
            ...
            (dist_dir / "Mahoraga_v1.2.3_arm64.dmg").write_bytes(b"stale")
            ctx = _upload_ctx(
                dist_dir,
    ...
    def test_merge_release_metadata_preserves_existing_artifacts(self) -> None:
        existing = {
            ...
            "build_date": "old",
            "artifacts": {
                "x64_appimage": {"filename": "Mahoraga_v1.2.3_x64.AppImage"},
                "x64_deb": {"filename": "Mahoraga_v1.2.3_amd64.deb"},
            },
        }
        new = {
            "platform": "linux",
            ...
            "build_date": "new",
            "artifacts": {
                "arm64_appimage": {"filename": "Mahoraga_v1.2.3_arm64.AppImage"},
                "arm64_deb": {"filename": "Mahoraga_v1.2.3_arm64.deb"},
            },
        }
