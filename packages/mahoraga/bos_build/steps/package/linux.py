#!/usr/bin/env python3
"""Linux packaging module for Mahoraga (AppImage and .deb)"""
...
class LinuxPackageModule(Step):
    ...
    def validate(self, ctx: Context) -> None:
        ...
        chrome_binary = join_paths(out_dir, ctx.MAHORAGA_APP_NAME)
    ...
    def execute(self, ctx: Context) -> None:
        log_info(
            f"\n📦 Packaging {ctx.MAHORAGA_APP_BASE_NAME} {ctx.get_mahoraga_chromium_version()} for Linux ({ctx.architecture})"
        )
...
def copy_browser_files(
    ...
    """
    out_dir = join_paths(ctx.chromium_src, ctx.out_dir)
    extensions_dir_name = "mahoraga_extensions"
    ...
    mahoraga_path = Path(join_paths(target_dir, ctx.MAHORAGA_APP_NAME))
    if mahoraga_path.exists():
        mahoraga_path.chmod(0o755)
...
def prepare_appdir(ctx: Context, appdir: Path) -> bool:
    ...
    desktop_file = create_desktop_file(
        ctx, apps_dir, f"{appimage_dir}/{ctx.MAHORAGA_APP_NAME}"
    )
    ...
    desktop_content = desktop_content.replace(
        f"Exec={appimage_dir}/{ctx.MAHORAGA_APP_NAME} %U", "Exec=AppRun %U"
    )
...
"${{HERE}}"{appimage_dir}/{ctx.MAHORAGA_APP_NAME} "$@"
"""
...
exec {ctx.product.linux.lib_dir}/{ctx.MAHORAGA_APP_NAME} "$@"
"""
...
def create_control_file(ctx: Context, debian_dir: Path) -> None:
    ...
    version = ctx.get_mahoraga_chromium_version()
...
Recommends: apparmor
Maintainer: Mahoraga Team <support@mahoraga.com>
...
profile {profile_name} {ctx.product.linux.lib_dir}/{ctx.MAHORAGA_APP_NAME} flags=(unconfined) {{
  userns,
...
def create_metainfo_file(ctx: Context, metainfo_dir: Path) -> None:
    ...
    and other AppStream-aware tools can display Mahoraga in their catalogs.
    """
...
<component type="desktop-application">
  ...
  <developer id="com.mahoraga">
    <name>Mahoraga Team</name>
  </developer>
...
def package_appimage(ctx: Context, package_dir: Path) -> Optional[Path]:
    ...
    appdir = Path(
        join_paths(package_dir, f"{ctx.MAHORAGA_APP_BASE_NAME}-{ctx.architecture}.AppDir")
    )
    if appdir.exists():
        safe_rmtree(appdir)
...
def package_deb(ctx: Context, package_dir: Path) -> Optional[Path]:
    ...
    debdir = Path(
        join_paths(package_dir, f"{ctx.MAHORAGA_APP_BASE_NAME}_{ctx.architecture}_deb")
    )
    if debdir.exists():
        safe_rmtree(debdir)
