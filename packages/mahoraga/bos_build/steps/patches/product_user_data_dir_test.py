...
def _product_identity_branches(source: str) -> tuple[str, str]:
    """Return the BrowserClaw and Mahoraga compile-time identity branches."""
    match = re.search(
        r"#if BUILDFLAG\(MAHORAGA_PRODUCT_BROWSERCLAW\)\n"
        r"(?P<browserclaw>.*?)\n#else\n(?P<mahoraga>.*?)\n#endif",
        source,
        re.DOTALL,
    )
    ...
    return match.group("browserclaw"), match.group("mahoraga")
...
class ProductUserDataDirPatchTest(unittest.TestCase):
    def test_mac_profile_root_comes_from_mahoraga_product_gn_arg(self) -> None:
        build = _patch("chrome/BUILD.gn")
        plist = _patch("chrome/app/app-Info.plist")
        buildflags = _patch("chrome/browser/mahoraga/buildflags.gni")

        self.assertIn(
            '"MAHORAGA_PRODUCT_DIR_NAME=$mahoraga_product_dir_name"', build
        )
        self.assertRegex(
            plist,
            re.compile(
                r"\+\t<key>CrProductDirName</key>\n"
                r"\+\t<string>\$\{MAHORAGA_PRODUCT_DIR_NAME\}</string>"
            ),
        )
        self.assertRegex(
            buildflags,
            re.compile(
                r'\+if \(mahoraga_product_browserclaw\) \{\n'
                r'\+  mahoraga_product_dir_name = "BrowserClaw"\n'
                r'\+\} else \{\n'
                r'\+  mahoraga_product_dir_name = "Mahoraga"\n'
                r'\+\}'
            ),
        )
    ...
    def test_linux_profile_roots_are_product_specific(self) -> None:
        ...
        self.assertRegex(
            linux_paths,
            re.compile(
                r"\+#elif BUILDFLAG\(MAHORAGA_PRODUCT_BROWSERCLAW\)\n"
    ...
    def test_windows_profile_roots_are_product_specific(self) -> None:
        ...
        browserclaw, mahoraga = _product_identity_branches(install_modes)

        self.assertEqual(
            install_modes.count("#if BUILDFLAG(MAHORAGA_PRODUCT_BROWSERCLAW)"),
            1,
        )
        self.assertIn(
        ...
        self.assertIn(
            'inline constexpr wchar_t kProductPathName[] = L"Mahoraga";', mahoraga
        )
        self.assertNotIn("GetProduct", install_modes)
        self.assertNotIn("mahoraga_product.h", install_modes)
    ...
    def test_windows_install_identity_branches_are_complete(self) -> None:
        ...
        expected_fields = {
            "base_app_name": ('L"BrowserClaw"', 'L"Mahoraga"'),
            "base_app_id": ('L"BrowserClaw"', 'L"Mahoraga"'),
            "browser_prog_id_prefix": ('L"BClawHTML"', 'L"BOSHTML"'),
            "browser_prog_id_description": (
                'L"BrowserClaw HTML Document"',
                'L"Mahoraga HTML Document"',
            ),
            "direct_launch_url_scheme": ('"browserclaw"', '"mahoraga"'),
            "pdf_prog_id_prefix": ('L"BClawPDF"', 'L"BOSPDF"'),
            "pdf_prog_id_description": (
                'L"BrowserClaw PDF Document"',
                'L"Mahoraga PDF Document"',
            ),
            "active_setup_guid": (
        ...
        for field, (browserclaw_value, mahoraga_value) in expected_fields.items():
            with self.subTest(field=field, product="browserclaw"):
                self.assertEqual(
                    _field_initializer(browserclaw, field), browserclaw_value
                )
            with self.subTest(field=field, product="mahoraga"):
                self.assertEqual(_field_initializer(mahoraga, field), mahoraga_value)

        self.assertIn('L"924012147-"', browserclaw)
        self.assertIn('L"924012148-"', mahoraga)
    ...
    def test_windows_install_clsids_are_product_specific(self) -> None:
        ...
        mahoraga_guids = {
            _guid(_field_initializer(mahoraga, field)) for field in guid_fields
        }

        self.assertTrue(browserclaw_guids.isdisjoint(mahoraga_guids))
        expected_clsids = {
            "toast_activator_clsid": (
        ...
        for field, (
            ...
            with self.subTest(field=field, product="mahoraga"):
                self.assertEqual(
                    _guid(_field_initializer(mahoraga, field)), mahoraga_clsid
                )
    ...
    def test_windows_install_shared_identity_matches_implemented_interfaces(
        ...
        self.assertNotIn("elevator_iid", mahoraga)
        self.assertNotIn("tracing_service_iid", browserclaw)
        self.assertNotIn("tracing_service_iid", mahoraga)
