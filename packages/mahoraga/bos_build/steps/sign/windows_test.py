...
from .windows import (
    ...
    get_mahoraga_server_binary_paths,
    get_existing_mahoraga_server_binary_paths,
    get_missing_required_mahoraga_server_binary_paths,
    sign_with_codesigntool,
)


class WindowsSignPathsTest(unittest.TestCase):
    def test_mahoraga_and_claw_server_binaries_are_expected_for_signing(self):
        build_output_dir = Path("/tmp/out/Default")

        self.assertEqual(
            get_mahoraga_server_binary_paths(build_output_dir),
            [
                build_output_dir
                / "MahoragaServer"
                / "default"
    ...
    def test_missing_optional_claw_binary_is_not_required_before_packaging(self):
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                get_existing_mahoraga_server_binary_paths(build_output_dir),
                [
                    build_output_dir
                    / "MahoragaServer"
                    / "default"
            ...
            self.assertEqual(
                get_missing_required_mahoraga_server_binary_paths(build_output_dir),
                [],
            )
    ...
    def test_sign_executables_fails_when_required_server_binary_missing(self):
        with TemporaryDirectory() as tmp:
            ...
            with self.assertRaisesRegex(RuntimeError, "mahoraga_server.exe"):
                WindowsSignModule()._sign_executables(
                    build_output_dir, self._ctx("mahoraga")
                )
    ...
    def test_missing_chrome_is_fatal_for_each_product(self):
        for product_id in ("mahoraga", "browserclaw"):
            with self.subTest(product=product_id), TemporaryDirectory() as tmp:
                build_output_dir = Path(tmp)
                for binary in get_mahoraga_server_binary_paths(
    ...
    def test_browserclaw_requires_claw_binary(self):
        with TemporaryDirectory() as tmp:
            ...
            with self.assertRaisesRegex(RuntimeError, "mahoraga-claw-server.exe"):
                WindowsSignModule()._sign_executables(
