#!/usr/bin/env python3
"""Windows signing module for Mahoraga"""
...
class WindowsSignModule(Step):
    ...
    def _sign_executables(self, build_output_dir: Path, ctx: Context) -> None:
        ...
        missing = get_missing_required_mahoraga_server_binary_paths(
        ...
        binaries_to_sign.extend(
            get_existing_mahoraga_server_binary_paths(build_output_dir, ctx.product.id)
        )
...
def get_mahoraga_server_binary_paths(
...
def get_existing_mahoraga_server_binary_paths(
    build_output_dir: Path,
    product_id: str | None = None,
) -> List[Path]:
...
def get_missing_required_mahoraga_server_binary_paths(
    build_output_dir: Path,
    product_id: str | None = None,
) -> List[Path]:
