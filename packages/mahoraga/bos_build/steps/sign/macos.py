#!/usr/bin/env python3
"""Application signing and notarization module for Mahoraga (macOS)"""
...
def get_mahoraga_server_binary_info(component_path: Path) -> Optional[Dict[str, str]]:
    """Return metadata for known Mahoraga Server binaries, if applicable."""
...
class MacOSSignModule(Step):
    ...
    def _stamp_update_versions(self, app_path: Path, ctx: Context) -> None:
        ...
        CFBundleVersion, so it must carry the epoch-prefixed Mahoraga
...
def find_components_to_sign(
    ...
    # Check both versioned and non-versioned paths for Mahoraga Framework
    # Handle both release and debug framework names
    if ctx:
        framework_names = [ctx.product.mac_framework_name(ctx.build_type)]
    else:
        framework_names = [
            "Mahoraga Framework.framework",
            "Mahoraga Dev Framework.framework",
    ...
    for fw_name in framework_names:
        ...
        if fw_path.exists():
            ...
            if ctx and ctx.mahoraga_chromium_version:
                versioned_path = join_paths(
                    fw_path, "Versions", ctx.mahoraga_chromium_version
                )
    ...
    # Find all dylibs (check versioned path for Mahoraga Framework libraries)
    for nxtscape_fw_path in nxtscape_framework_paths:
    ...
    for bundle in bundles:
        ...
        for item in bundle_root.rglob("*"):
            if (
                ...
                and get_mahoraga_server_binary_info(item) is not None
            ):
                components["executables"].append(item)
...
def get_identifier_for_component(
    component_path: Path, base_identifier: str = "com.mahoraga"
) -> str:
    ...
    # Mahoraga Server binaries share the same entitlements/options but need unique identifiers.
    mahoraga_server_info = get_mahoraga_server_binary_info(component_path)
    if mahoraga_server_info:
        suffix = mahoraga_server_info.get("identifier_suffix", component_path.stem)
...
def get_signing_options(component_path: Path) -> str:
    ...
    # Known Mahoraga Server binaries share the same relaxed options.
    mahoraga_server_info = get_mahoraga_server_binary_info(component_path)
    if mahoraga_server_info:
        return mahoraga_server_info.get("options", "runtime")
...
def sign_all_components(
    ...
    base_identifier = (
        ctx.product.mac_signing_identifier(ctx.build_type) if ctx else "com.mahoraga"
    )
    main_identifier = (
        ctx.product.mac_signing_identifier(ctx.build_type)
        if ctx
        else "com.mahoraga.Mahoraga"
    )
    ...
    if components["executables"]:
        ...
        for exe in components["executables"]:
            ...
            entitlements = None
            mahoraga_server_info = get_mahoraga_server_binary_info(exe)
            if mahoraga_server_info:
                entitlements_name = mahoraga_server_info.get("entitlements")
    ...
    # 6. Sign frameworks (except the main Mahoraga Framework)
    if components["frameworks"]:
        log_info("\n🔏 Signing frameworks...")
        # Sort to sign Sparkle.framework before Mahoraga Framework.framework
