#!/usr/bin/env python3
"""Standard single-architecture build module for Mahoraga"""
...
def compute_ninja_jobs(env: Optional[Mapping[str, str]] = None) -> Optional[int]:
    ...
    override = env.get("MAHORAGA_NINJA_JOBS")
    if override is not None:
        try:
            ...
            jobs = 0
        if jobs > 0:
            log_info(f"Ninja parallelism: -j {jobs} (MAHORAGA_NINJA_JOBS override)")
            return jobs
        log_warning(f"Ignoring invalid MAHORAGA_NINJA_JOBS={override!r}")
    ...
    log_info(
        ...
        f"{GB_PER_COMPILE_JOB} GB per job; override with MAHORAGA_NINJA_JOBS)"
    )
    return jobs
...
class CompileModule(Step):
    produces = ["built_app"]
    requires = []
    description = "Build Mahoraga using autoninja"

    def validate(self, ctx: Context) -> None:
        if not ctx.chromium_src.exists():
            raise ValidationError(f"Chromium source not found: {ctx.chromium_src}")

        if not ctx.mahoraga_chromium_version:
            raise ValidationError("Mahoraga chromium version not set")
    ...
    def execute(self, ctx: Context) -> None:
        log_info("\n🔨 Building Mahoraga (this will take a while)...")
    ...
    def _create_version_file(self, ctx: Context) -> None:
        parts = ctx.mahoraga_chromium_version.split(".")
        if len(parts) != 4:
            log_warning(f"Invalid version format: {ctx.mahoraga_chromium_version}")
            return
        ...
        log_info(f"Created VERSION file: {ctx.mahoraga_chromium_version}")
