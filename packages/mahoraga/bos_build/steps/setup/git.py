#!/usr/bin/env python3
"""Git operations module for Mahoraga build system"""
...
class GitSetupModule(Step):
    ...
    def execute(self, ctx: Context) -> None:
        ...
        self._checkout_mahoraga_branch(ctx)
    ...
    def _checkout_mahoraga_branch(self, ctx: Context) -> None:
        """Create/reset local `mahoraga` branch at the pinned tag, not detached HEAD."""
        ...
        log_info(
            f"🔀 Checking out tag {ctx.chromium_version} as branch: {MAHORAGA_BRANCH}"
        )
        run_command(
            ["git", "checkout", "-B", MAHORAGA_BRANCH, f"tags/{ctx.chromium_version}"],
            cwd=ctx.chromium_src,
        )
