...
from .git import GitSetupModule, MAHORAGA_BRANCH
...
class GitSetupValidateTest(unittest.TestCase):
    def test_missing_chromium_src_raises(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = MockMahoragaRoot(Path(tmp) / "root")
            ctx = Context(
                root_dir=root.root,
    ...
    def test_missing_chromium_version_raises(self):
        with (
            ...
            root = MockMahoragaRoot(Path(root_tmp))
    ...
    def test_passes_with_src_and_version(self):
        with (
            ...
            ctx = make_context(
                ...
                MockMahoragaRoot(Path(root_tmp)),
            )
            GitSetupModule().validate(ctx)
...
class GitSetupExecuteTest(unittest.TestCase):
    def setUp(self):
        ...
        self.ctx = make_context(
            ...
            MockMahoragaRoot(Path(self._root_tmp.name)),
        )

    def test_checks_out_tag_as_mahoraga_branch(self):
        commands = []
        ...
        self.assertEqual(
            ...
            [
                ...
                ["git", "checkout", "-B", MAHORAGA_BRANCH, tag_ref],
...
class EnsureGclientTargetCpusTest(unittest.TestCase):
    def setUp(self):
        ...
        self.ctx = make_context(
            self.chromium, MockMahoragaRoot(Path(self._root_tmp.name))
        )
