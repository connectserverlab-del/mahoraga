...
fn adding_feature_appends_without_rewriting_existing_comments() -> Result<()> {
    ...
    store.add_feature(
        "wallet",
        "feat: mahoraga wallet",
        vec!["chrome/browser/mahoraga/wallet/".to_string()],
    )?;
    store.save()?;
    ...
    assert_eq!(wallet.description, "feat: mahoraga wallet");
    assert_eq!(wallet.paths, vec!["chrome/browser/mahoraga/wallet/"]);
    Ok(())
}
...
fn feature_store_false_defaults_and_appends_preserve_yaml() -> Result<()> {
    ...
    store.append_feature_paths(
        "build-resources",
        vec!["chrome/MAHORAGA_VERSION".to_string()],
    )?;
    store.save()?;
    ...
    assert!(text.contains("      - \"chrome/MAHORAGA_VERSION\""));
...
fn canonical_store_loads_build_resources_as_unstored() -> Result<()> {
    ...
    let expected = vec![
        ...
        "chrome/browser/mahoraga/claw_server/resources/",
        "chrome/browser/mahoraga/server/resources/",
        "chrome/browser/mahoraga/onboarding/resources/",
    ...
    assert!(!store.stores_path("chrome/browser/mahoraga/server/resources/bin/mahoraga_server"));
    assert!(!store.stores_path("chrome/browser/mahoraga/claw_server/resources/bin/server"));
    assert!(store.stores_path("chrome/browser/mahoraga/claw_server/BUILD.gn"));
    Ok(())
}
...
fn feature_matching_exact_prefix_and_nearest_suggestions() -> Result<()> {
    ...
    assert_eq!(
        store.match_path("chrome/browser/mahoraga/wallet/service.cc"),
        FeatureMatch::Unmatched {
...
fn real_store_loads_when_seeded() -> Result<()> {
    ...
    assert!(store.features().features.contains_key("mahoraga-core"));
    Ok(())
}
