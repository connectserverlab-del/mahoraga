...
fn sim6_needs_feature_json_then_named_feature_extracts() -> Result<()> {
    ...
    let store_dir = seed_extract_store(&store, &base)?;
    store.commit("seed store")?;
    checkout.write_file("chrome/browser/mahoraga/wallet/service.cc", "wallet cc\n")?;
    checkout.write_file("chrome/browser/mahoraga/wallet/service.h", "wallet h\n")?;
    checkout.write_file("chrome/browser/mahoraga/wallet/BUILD.gn", "wallet build\n")?;
    checkout.write_file(
        "chrome/browser/mahoraga/BUILD.gn",
        "mahoraga build changed\n",
    )?;
...
fn sim7_annotate_commits_claimed_resources_and_leaves_leftovers_then_rest() -> Result<()> {
    ...
    checkout.write_file("chrome/browser/mahoraga/core/service.cc", "core changed\n")?;
    checkout.plant_untracked("chrome/browser/mahoraga/core/new_file.cc", "new core\n")?;
    checkout.write_file("chrome/MAHORAGA_VERSION", "148.0.7204.1-mahoraga\n")?;
    checkout.plant_untracked("chrome/app/theme/chromium/logo.png", "logo\n")?;
    checkout.plant_untracked("chrome/browser/mahoraga/wallet/service.cc", "wallet\n")?;
    ...
    assert!(
        annotated
            .stdout
            .contains("chrome/browser/mahoraga/wallet/service.cc")
    );
    ...
    assert!(
        subjects
            .lines()
            .any(|line| line == "chore: mahoraga core infrastructure")
    );
    assert!(
        checkout
            .git()
            .run_str(&[
                ...
                "HEAD",
                "--",
                "chrome/browser/mahoraga/core/new_file.cc",
            ])?
            .contains("chrome/browser/mahoraga/core/new_file.cc")
    );
    ...
    assert!(status.contains("chrome/browser/mahoraga/wallet"));
    assert!(!status.contains("chrome/browser/mahoraga/core/new_file.cc"));
    assert!(!status.contains("chrome/MAHORAGA_VERSION"));
...
fn sim8_annotate_triage_json_then_feature_add_round_trip() -> Result<()> {
    ...
    assert_eq!(
        ...
        "chrome/browser/mahoraga/wallet/service.cc"
    );
    assert!(
        json["unclaimed"][0]["command"]
            .as_str()
            .expect("command")
            .contains("bpatch feature add wallet chrome/browser/mahoraga/wallet/")
    );
    ...
    let add = run_bpatch(
        ...
        strs(&[
            ...
            "add",
            "wallet",
            "chrome/browser/mahoraga/wallet/",
            "--desc",
            "feat: wallet",
            "--json",
        ]),
...
fn sim9_store_false_resource_commit_is_clean_for_status_and_diff() -> Result<()> {
    ...
    checkout.write_file("chrome/MAHORAGA_VERSION", "resource changed\n")?;
    checkout.plant_untracked(".mahoraga-patch/state.json", "{}\n")?;
    let annotated = run_bpatch(
        checkout.path(),
        Some(&store_dir),
        strs(&["annotate", "--json"]),
...
fn sim10_feature_add_from_dirty_collapses_dirs_and_is_idempotent() -> Result<()> {
    ...
    assert!(features.contains("      - \"chrome/MAHORAGA_VERSION\""));
...
fn fixture_store_visible_files_are_patch_files_and_nested_dot_patches_load() -> Result<()> {
    ...
    checkout.write_file("chrome/browser/ui/llmchat/panel.cc", "panel changed\n")?;
    checkout.write_file(
        "chrome/browser/mahoraga/bundled_extensions/.gitignore",
        "bundle-cache\n",
    )?;
    checkout.git().run(&["add", "-A"])?;
    commit_store_from_index(
    ...
    assert!(
        model
            .patches()
            .contains_key("chrome/browser/mahoraga/bundled_extensions/.gitignore")
    );
    Ok(())
}
...
fn write_annotate_base(repo: &FixtureRepo) -> Result<String> {
    ...
    repo.write_file("chrome/browser/mahoraga/core/service.cc", "core base\n")?;
    repo.write_file("chrome/MAHORAGA_VERSION", "base resource\n")?;
...
features:
  mahoraga-core:
    description: "chore: mahoraga core infrastructure"
    files:
      - chrome/browser/mahoraga/core/
  build-resources:
    description: "resource: bos_build outputs"
    store: false
    files:
      - chrome/MAHORAGA_VERSION
